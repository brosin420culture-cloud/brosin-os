...
/* ---------- i18n (multi-idioma) ----------
   Los textos en ESPAÑOL son la clave. tr("Guardar") busca la traducción en el
...
const LANGS = [
  ...
  { k: "en", label: "English", flag: "🇬🇧", locale: "en-GB" },
  { k: "fr", label: "Français", flag: "🇫🇷", locale: "fr-FR" },
  { k: "de", label: "Deutsch", flag: "🇩🇪", locale: "de-DE" },
  { k: "it", label: "Italiano", flag: "🇮🇹", locale: "it-IT" },
  { k: "pt", label: "Português", flag: "🇵🇹", locale: "pt-PT" },
  { k: "nl", label: "Nederlands", flag: "🇳🇱", locale: "nl-NL" },
  { k: "pl", label: "Polski", flag: "🇵🇱", locale: "pl-PL" },
  { k: "sv", label: "Svenska", flag: "🇸🇪", locale: "sv-SE" },
  { k: "da", label: "Dansk", flag: "🇩🇰", locale: "da-DK" },
  { k: "no", label: "Norsk", flag: "🇳🇴", locale: "nb-NO" },
  { k: "fi", label: "Suomi", flag: "🇫🇮", locale: "fi-FI" },
  { k: "cs", label: "Čeština", flag: "🇨🇿", locale: "cs-CZ" },
  { k: "sk", label: "Slovenčina", flag: "🇸🇰", locale: "sk-SK" },
  { k: "hu", label: "Magyar", flag: "🇭🇺", locale: "hu-HU" },
  { k: "ro", label: "Română", flag: "🇷🇴", locale: "ro-RO" },
  { k: "el", label: "Ελληνικά", flag: "🇬🇷", locale: "el-GR" },
  { k: "bg", label: "Български", flag: "🇧🇬", locale: "bg-BG" },
  { k: "hr", label: "Hrvatski", flag: "🇭🇷", locale: "hr-HR" },
  { k: "sl", label: "Slovenščina", flag: "🇸🇮", locale: "sl-SI" },
  { k: "lt", label: "Lietuvių", flag: "🇱🇹", locale: "lt-LT" },
  { k: "lv", label: "Latviešu", flag: "🇱🇻", locale: "lv-LV" },
  { k: "et", label: "Eesti", flag: "🇪🇪", locale: "et-EE" },
  { k: "ga", label: "Gaeilge", flag: "🇮🇪", locale: "ga-IE" },
  { k: "mt", label: "Malti", flag: "🇲🇹", locale: "mt-MT" },
  { k: "uk", label: "Українська", flag: "🇺🇦", locale: "uk-UA" },
];
let I18N = {};            // diccionario activo (es -> traducción)
let CUR_LANG = "es";      // idioma activo resuelto
let LOCALE = "es-ES";     // locale para fechas y números
const tr = (es) => (I18N && Object.prototype.hasOwnProperty.call(I18N, es) ? I18N[es] : es);
// tri("Te faltan {n} días", { n: 3 }) — traduce la frase completa y luego rellena los huecos.
const tri = (es, vars) => { let s = tr(es); for (const k in vars) s = s.split("{" + k + "}").join(String(vars[k])); return s; };
const resolveLang = (pref) => {
  if (pref && pref !== "auto") return LANGS.some((l) => l.k === pref) ? pref : "es";
  try {
    const nav = (typeof navigator !== "undefined" && (navigator.language || (navigator.languages || [])[0])) || "es";
    const two = String(nav).slice(0, 2).toLowerCase();
    return LANGS.some((l) => l.k === two) ? two : "en"; // fuera de los soportados → inglés
  } catch (e) { return "es"; }
};
async function loadLang(lang) {
  const meta = LANGS.find((l) => l.k === lang) || LANGS[1];
  if (lang === "es") { const changed = CUR_LANG !== "es"; I18N = {}; CUR_LANG = "es"; LOCALE = "es-ES"; return changed; }
  let dict = null;
  try { const cached = await store.get("i18n_" + lang + "_v" + I18N_VER); if (cached && typeof cached === "object") dict = cached; } catch (e) {}
  if (dict) { I18N = dict; CUR_LANG = lang; LOCALE = meta.locale; }
  try {
    const r = await fetch(`./i18n/${lang}.json?v=${I18N_VER}`, { cache: "no-cache" });
    if (r.ok) {
      const fresh = await r.json();
      if (fresh && typeof fresh === "object") { I18N = fresh; CUR_LANG = lang; LOCALE = meta.locale; store.set("i18n_" + lang + "_v" + I18N_VER, fresh); return true; }
    }
  } catch (e) {}
  return !!dict;
}
...
/* nombre del idioma para las instrucciones de la IA */
const LANG_NAMES = { es: "español de España", en: "English", fr: "français", de: "Deutsch", it: "italiano", pt: "português", nl: "Nederlands", pl: "polski", sv: "svenska", da: "dansk", no: "norsk (bokmål)", fi: "suomi", cs: "čeština", sk: "slovenčina", hu: "magyar", ro: "română", el: "ελληνικά (griego)", bg: "български (búlgaro)", hr: "hrvatski", sl: "slovenščina", lt: "lietuvių", lv: "latviešu", et: "eesti", ga: "Gaeilge (irlandés)", mt: "Malti (maltés)", uk: "українська (ucraniano)" };
const aiLangRule = (mode) => {
  if (CUR_LANG === "es") return "";
  const n = LANG_NAMES[CUR_LANG] || "English";
  return mode === "json"
    ? `\nIDIOMA: el usuario usa la app en ${n}. El campo "reply" va SIEMPRE en ${n}; las claves del JSON y los valores fijos (tipos, categorías, kinds) se quedan tal cual en el formato indicado.`
    : `\nIDIOMA: responde SIEMPRE en ${n} (el usuario usa la app en ese idioma).`;
};
...
/* Temas desbloqueables por racha (recompensas de constancia) */
const MID = { ...DARK, mode: "mid",
  bg0: "#070b16", ink2: "#101728", ink3: "#16203a", line: "#1f2b4a", well: "#0a101f", outer: "#020409",
  textLo: "#8b95b5", glass: "rgba(6,10,22,.82)",
  surf: "linear-gradient(180deg,#141c31 0%,#0e1526 100%)",
  surfDeep: "linear-gradient(160deg,#16203a 0%,#0c1220 100%)",
  surfSheet: "linear-gradient(180deg,#151e35 0%,#0d1424 100%)",
  surfBtn: "linear-gradient(180deg,#1c2745 0%,#131b31 100%)",
  tileTool: "linear-gradient(180deg,#192341 0%,#111a2f 100%)",
  btnDark: "linear-gradient(180deg,#1e2949 0%,#141d35 60%,#101728 100%)",
  keyNum: "linear-gradient(180deg,#1a2440 0%,#111830 100%)",
  keyOp: "linear-gradient(180deg,#243056 0%,#182142 100%)",
  keyPin: "linear-gradient(180deg,#1b2543 0%,#121a32 100%)",
  edge: "rgba(160,190,255,.09)", edgeHi: "rgba(160,190,255,.16)", edgeSoft: "rgba(160,190,255,.04)",
  tintY: "linear-gradient(135deg,#22200a 0%,#0e1526 70%)",
  tintP: "linear-gradient(135deg,#1e1140 0%,#0e1526 65%)",
  tintG: "linear-gradient(135deg,#0e2420 0%,#0e1526 70%)",
};
const HONEY = { ...LIGHT, mode: "honey",
  bg0: "#f9eecd", ink2: "#f5e6ba", ink3: "#eeda9f", line: "#e0c67e", well: "#f2e0ac", outer: "#eeddab",
  textHi: "#231a04", textLo: "#7d6a35", cream: "#f9eecd",
  surf: "linear-gradient(180deg,#fff9e3 0%,#f7e9bd 100%)",
  surfDeep: "linear-gradient(160deg,#fffbe8 0%,#f3e2ac 100%)",
  surfSheet: "linear-gradient(180deg,#fdf6da 0%,#f4e5b3 100%)",
  surfBtn: "linear-gradient(180deg,#fffce9 0%,#f1dfa6 100%)",
  tileTool: "linear-gradient(180deg,#fffbe4 0%,#f0dda1 100%)",
  btnDark: "linear-gradient(180deg,#fbf2cf 0%,#ecd897 100%)",
  keyNum: "linear-gradient(180deg,#fffdf0 0%,#f4e5b0 100%)",
  keyOp: "linear-gradient(180deg,#f7ecc4 0%,#ebd691 100%)",
  keyPin: "linear-gradient(180deg,#fffbe6 0%,#f1dfa4 100%)",
  glass: "rgba(250,240,205,.87)", edge: "rgba(120,90,10,.13)", edgeHi: "rgba(120,90,10,.2)", edgeSoft: "rgba(120,90,10,.05)",
  rail: "rgba(120,90,10,.15)", keycapIn: "rgba(120,90,10,.16)", dropBtn: "rgba(120,90,10,.18)", dropSheet: "rgba(120,90,10,.22)",
  wellShadow: "0 2px 5px rgba(120,90,10,.12) inset, 0 1px 0 rgba(255,255,255,.75)",
  shadCard: "0 1px 0 rgba(255,255,255,.85) inset, 0 1px 2px rgba(120,90,10,.08), 0 10px 26px rgba(120,90,10,.12)",
  shadBtn: "0 1px 0 rgba(255,255,255,.9) inset, 0 -1.5px 0 rgba(120,90,10,.1) inset, 0 3px 8px rgba(120,90,10,.14)",
  tintY: "linear-gradient(135deg,#ffefad 0%,#faf0cd 70%)",
  tintP: "linear-gradient(135deg,#f0e0fb 0%,#f9f0ce 65%)",
  tintG: "linear-gradient(135deg,#def3d3 0%,#f9f1cf 70%)",
  accentText: "#7d5e00",
};
const GOLD = { ...DARK, mode: "gold",
  bg0: "#000000", ink2: "#12100a", ink3: "#1a1708", line: "#2e2708", well: "#0a0903", outer: "#000000",
  textLo: "#a89c6a", glass: "rgba(6,5,0,.82)",
  surf: "linear-gradient(180deg,#181405 0%,#0f0d03 100%)",
  surfDeep: "linear-gradient(160deg,#1a1606 0%,#0c0a02 100%)",
  surfSheet: "linear-gradient(180deg,#191504 0%,#0e0c03 100%)",
  surfBtn: "linear-gradient(180deg,#241e08 0%,#161204 100%)",
  tileTool: "linear-gradient(180deg,#1f1a06 0%,#131003 100%)",
  btnDark: "linear-gradient(180deg,#262008 0%,#181405 60%,#12100a 100%)",
  keyNum: "linear-gradient(180deg,#1e1906 0%,#121004 100%)",
  keyOp: "linear-gradient(180deg,#2b2409 0%,#1c1706 100%)",
  keyPin: "linear-gradient(180deg,#211b07 0%,#141105 100%)",
  edge: "rgba(255,212,0,.12)", edgeHi: "rgba(255,212,0,.2)", edgeSoft: "rgba(255,212,0,.05)",
  wellShadow: "0 2px 5px rgba(0,0,0,.5) inset, 0 1px 0 rgba(255,212,0,.06)",
  shadCard: "0 1px 0 rgba(255,212,0,.07) inset, 0 1px 2px rgba(0,0,0,.55), 0 10px 26px rgba(255,180,0,.07)",
  shadBtn: "0 1px 0 rgba(255,212,0,.1) inset, 0 -1.5px 0 rgba(0,0,0,.4) inset, 0 3px 8px rgba(0,0,0,.4)",
  tintY: "linear-gradient(135deg,#2c2400 0%,#0f0d03 70%)",
  tintP: "linear-gradient(135deg,#1c0d31 0%,#0f0d03 65%)",
  tintG: "linear-gradient(135deg,#12200f 0%,#0f0d03 70%)",
};
...
/* ---------- Liquid Glass ----------
   Lenguaje visual del kit de Almar, traído a los átomos de estilo de la app.
   La receta: desenfoque + saturación + un punto de brillo sobre el fondo,
   relleno translúcido con la luz cayendo desde arriba-izquierda, y un canto
   con grosor. El desenfoque solo empaña; lo que hace que parezca CRISTAL y
   no humo es la saturación.

   Se DERIVA del tema activo en vez de fijarse a mano, así los cinco temas
   (noche, claro, medianoche, miel, oro) salen coherentes solos.

   El bisel va como box-shadow inset y no como pseudoelemento porque estos
   átomos son objetos de estilo en línea y no admiten ::after. */
const glassTokens = () => (C.base === "light"
  ? { tint: "rgba(255,255,255,.58)", edge: "rgba(255,255,255,.70)", sheen: "rgba(255,255,255,.55)",
      hi: "rgba(255,255,255,.75)", lo: "rgba(31,45,61,.10)", drop: "0 6px 14px rgba(31,45,61,.14)" }
  : { tint: "rgba(14,14,16,.44)", edge: "rgba(255,255,255,.16)", sheen: "rgba(255,255,255,.10)",
      hi: "rgba(255,255,255,.28)", lo: "rgba(0,0,0,.32)", drop: "0 6px 14px rgba(0,0,0,.30)" });

// blur 16 equilibrado · saturate 1.8 · brightness 1.08 (el trío del kit)
const GLASS_FILTER = "blur(16px) saturate(1.8) brightness(1.08)";

/* El lienzo sobre el que flota todo. Sin esto el cristal no existe: el
   brightness del filtro multiplica lo que hay detrás, y sobre negro puro
   (#000) multiplicar no ilumina nada — los paneles quedan muertos.
   Focos de marca muy difusos, dentro del marco de la app para que el
   contenido se deslice por delante de ellos al desplazar.

   OJO: nada de background-attachment:fixed aquí. Fixed posiciona el degradado
   respecto al VIEWPORT, y como el marco solo mide 440 px, los focos se iban
   fuera de la app y no se veía nada. Va relativo al propio marco. */
const ambientBg = () => (C.base === "light"
  ? `radial-gradient(80% 34% at 4% 0%, rgba(255,196,0,.34), transparent 62%),
     radial-gradient(75% 32% at 100% 10%, rgba(144,19,254,.18), transparent 64%),
     radial-gradient(95% 38% at 50% 100%, rgba(61,220,132,.20), transparent 66%),
     ${C.bg0}`
  : `radial-gradient(80% 34% at 4% 0%, rgba(255,212,0,.26), transparent 62%),
     radial-gradient(75% 32% at 100% 10%, rgba(144,19,254,.34), transparent 64%),
     radial-gradient(95% 38% at 50% 100%, rgba(61,220,132,.18), transparent 66%),
     ${C.bg0}`);

function glassSurface(radius, blur) {
  const g = glassTokens();
  const f = blur ? `blur(${blur}px) saturate(1.8) brightness(1.08)` : GLASS_FILTER;
  return {
    background: `linear-gradient(135deg, ${g.sheen}, ${g.tint})`,
    backdropFilter: f,
    WebkitBackdropFilter: f,
    border: `1px solid ${g.edge}`,
    borderRadius: radius,
    boxShadow: `inset 1px 1px 0 ${g.hi}, inset -1px -1px 0 ${g.lo}, ${g.drop}`,
  };
}
...
function applyTheme(mode) {
  ...
  Object.assign(C, T);
  Object.assign(card, glassSurface(22));
  Object.assign(iconBtn, glassSurface(14, 10), { color: C.textHi });
  ...
  Object.assign(settingRow, glassSurface(18));
  // Los colores de tipo de la inversión compartida capturan C al cargar el módulo
  // (paleta noche); sin esto quedarían lavados/incorrectos en modo claro.
  const shareCols = { meter: "good", aporte: "good", sacar: "bad", gasto: "bad", ganancia: "yellow", reparto: "purple", reinversion: "purple" };
  SHARE_KINDS.forEach((k) => { if (shareCols[k.k]) k.col = C[shareCols[k.k]]; });
  Object.keys(SHARE_META).forEach((k) => { if (shareCols[k]) SHARE_META[k].col = C[shareCols[k]]; });
}
...
/* "Hoy" es TU hoy, no el de Greenwich. Con toISOString() en España, entre las
   00:00 y las 02:00 el día seguía siendo el de ayer: marcabas un hábito a la
   una de la mañana y se apuntaba en el día anterior, la misión salía ya hecha
   y el cierre del día se equivocaba de fecha. La cabecera ya usaba la fecha
   local, así que además se contradecían entre ellas. */
const todayISO = () => {
...
const money = (n) =>
  (Number(n) || 0).toLocaleString(LOCALE, { style: "currency", currency: "EUR", maximumFractionDigits: 2 });
...
const fmtDateTime = (iso) => {
  if (!iso) return "";
  try { const d = new Date(iso); return d.toLocaleDateString(LOCALE, { day: "numeric", month: "short" }) + " · " + d.toLocaleTimeString(LOCALE, { hour: "2-digit", minute: "2-digit" }); } catch (e) { return ""; }
};
...
const leadLabel = (min) =>
  min === 0 ? tr("a la hora") : min < 60 ? tri("{n} min antes", { n: min }) : min < 1440 ? tri("{n} h antes", { n: min / 60 }) : (min / 1440 > 1 ? tri("{n} días antes", { n: min / 1440 }) : tr("1 día antes"));
...
const bdayLabel = (bday) => {
  ...
  const dd = Number(parts.length === 3 ? parts[2] : parts[1]);
  if (!mm || !dd) return "";
  return new Date(2020, mm - 1, dd).toLocaleDateString(LOCALE, { day: "numeric", month: "short" });
};
...
const store = {
  ...
  async set(key, value) {
    ...
    try {
      ...
      // Casi siempre es que el almacén del móvil está lleno (fotos ocupan mucho).
      // Antes esto se tragaba en silencio y parecía que había guardado. Ahora
      // tiramos las copias automáticas viejas y reintentamos: es mejor perder
      // copias antiguas que no poder guardar lo de hoy.
      if (key !== AUTOBAK_KEY) {
        try {
          await window.storage.set(AUTOBAK_KEY, [], { shared: false });
          await window.storage.set(key, value, { shared: false });
          return true;
        } catch (e2) {}
      }
...
async function cloudLoad(sb, userId) {
  try {
    // updated_at también: sin él no hay forma de saber si la nube es más vieja que el móvil.
    const { data, error } = await sb.from("app_state").select("data, updated_at").eq("user_id", userId).maybeSingle();
    if (error || !data) return null;
    const d = data.data;
    // copias de antes de la v34 no llevan sello dentro: usamos el de la fila
    if (d && typeof d === "object" && !d.savedAt && data.updated_at) d.savedAt = data.updated_at;
    return d;
  } catch (e) { return null; }
}
...
/* Sello de guardado. Sin esto no se puede decidir qué copia es la buena. */
const stamp = (persist) => ({ ...persist, savedAt: nowISO() });
...
function computeReminders(state) {
  ...
  (state.events || []).forEach((e) => {
    ...
    leads.forEach((lead) => push(`ev:${e.id}:${lead}`, new Date(start - lead * 60000), lead === 0 ? `${tr("Ahora")}: ${e.title}` : `${tr("Pronto")}: ${e.title}`, `${fmtDate(e.date)}${e.time ? ` · ${e.time}` : ""}`));
  });
  (state.people || []).forEach((p) => {
    if (p.remind === false || !p.birthday) return;
    const nb = nextBirthdayDate(p.birthday); if (!nb) return;
    [7, 1, 0].forEach((d) => { const fire = new Date(nb.getTime() - d * 24 * 3600 * 1000); fire.setHours(9, 0, 0, 0); push(`bd:${p.id}:${d}`, fire, d === 0 ? `🎂 ${tr("Hoy cumple")} ${p.name}` : d === 1 ? `🎂 ${tr("Mañana cumple")} ${p.name}` : `🎂 ${p.name} ${tr("cumple en")} ${d} ${tr("días")}`, (p.giftIdeas || []).some((g) => !g.done) ? tr("Tienes ideas de regalo apuntadas") : tr("Prepárale un detalle")); });
  });
  (state.goals || []).forEach((g) => {
    if (!g.deadline || g.notif === false) return;
    if (g.steps && g.steps.length && g.steps.every((s) => s.done)) return;
    [3, 1, 0].forEach((d) => { const gd = new Date(g.deadline + "T09:00:00"); if (isNaN(gd.getTime())) return; push(`gl:${g.id}:${d}`, new Date(gd.getTime() - d * 24 * 3600 * 1000), d === 0 ? `${tr("Hoy es la meta")}: ${g.title}` : `${tr("Se acerca")}: ${g.title}`, g.why ? tr("Recuerda") + ": " + g.why : tr("A por ello.")); });
  });
  // "Tu día en 30 segundos": aviso matinal diario a la hora elegida (próximos 7 días)
  const prof = state.profile || {};
  if (prof.briefOn !== false) {
    const parts = String(prof.briefTime || "08:30").split(":");
    const bh = Number(parts[0]), bm = Number(parts[1]);
    for (let d = 0; d < 7; d++) {
      const day = new Date(); day.setDate(day.getDate() + d); day.setHours(isFinite(bh) ? bh : 8, isFinite(bm) ? bm : 30, 0, 0);
      const iso = `${day.getFullYear()}-${String(day.getMonth() + 1).padStart(2, "0")}-${String(day.getDate()).padStart(2, "0")}`;
      const evs = (state.events || []).filter((e) => !e.done && sameDay(e.date, iso));
      const bds = (state.people || []).filter((p) => { if (!p.birthday || p.remind === false) return false; const nb = nextBirthdayDate(p.birthday); return nb && nb.getMonth() === day.getMonth() && nb.getDate() === day.getDate(); });
      const hb = (state.habits || []).length;
      const bits = [];
      if (evs.length) bits.push(`${evs.length} ${evs.length === 1 ? tr("evento") : tr("eventos")}`);
      if (hb) bits.push(`${hb} ${hb === 1 ? tr("hábito") : tr("hábitos")}`);
      bds.forEach((p) => bits.push(`🎂 ${p.name}`));
      push(`brief:${iso}`, day, "🐝 " + tr("Tu día en 30 segundos"), bits.length ? bits.join(" · ") : tr("Día despejado — elige tu jugada."));
    }
  }
  return out;
}
...
/* ---------- Racha de la colmena (racha global de días activos) ----------
   Un día cuenta si completaste al menos 1 hábito o 1 evento ese día.
   Congeladores: perdonan automáticamente un día fallado (se ganan por constancia). */
const dayISOOf = (d) => { const p = (n) => String(n).padStart(2, "0"); return `${d.getFullYear()}-${p(d.getMonth() + 1)}-${p(d.getDate())}`; };
const wasActiveOn = (state, iso) => {
  if ((state.habits || []).some((h) => (h.done || []).includes(iso))) return true;
  if ((state.events || []).some((e) => e.done && sameDay(e.date, iso))) return true;
  return false;
};
function hiveStreak(state, now = new Date()) {
  const saved = new Set(((state.profile || {}).streakSaved) || []);
  const todayIso = dayISOOf(now);
  const activeToday = wasActiveOn(state, todayIso);
  let count = 0;
  const cursor = new Date(now);
  if (activeToday) { count = 1; }
  cursor.setDate(cursor.getDate() - 1); // ayer hacia atrás (hoy aún está "en juego" si no hay actividad)
  for (let i = 0; i < 3700; i++) {
    const iso = dayISOOf(cursor);
    if (wasActiveOn(state, iso) || saved.has(iso)) { count++; cursor.setDate(cursor.getDate() - 1); }
    else break;
  }
  return { count, activeToday, todayIso };
}
function hiveBestStreak(state) {
  const days = new Set(((state.profile || {}).streakSaved) || []);
  (state.habits || []).forEach((h) => (h.done || []).forEach((d) => days.add(d)));
  (state.events || []).forEach((e) => { if (e.done && e.date) days.add(String(e.date).slice(0, 10)); });
  const arr = [...days].sort();
  let best = 0, run = 0, prev = null;
  arr.forEach((d) => {
    if (prev) { const diff = (new Date(d + "T00:00:00") - new Date(prev + "T00:00:00")) / 86400000; run = diff === 1 ? run + 1 : 1; }
    else run = 1;
...
function makeRecognizer() {
  try {
    ...
    if (!SR) return null;
    const r = new SR();
    r.lang = LOCALE; r.interimResults = true; r.continuous = true; r.maxAlternatives = 1;
    return r;
  } catch (e) { return null; }
}
const fmtNum = (v) => {
  ...
  return r.toLocaleString(LOCALE, { maximumFractionDigits: 8 });
};
...
/* Tasación de coleccionables. El prompt de arriba pide "precio de mercado del activo",
   pensado para oro/cripto/bolsa: con un vinilo o una carta la IA no devolvía número y
   la estimación se quedaba en nada. Este va a segunda mano y siempre da una cifra. */
async function aiEstimateCollectible(name, note) {
  const sys = `Eres tasador de objetos de coleccionismo: vinilos, cartas, cómics, monedas, sellos, figuras, videojuegos, relojes, etc. Estima el valor de mercado de SEGUNDA MANO en euros, el que se pagaría hoy en eBay o Wallapop en estado normal de uso. Devuelve SOLO un JSON {"precio": number, "moneda": "EUR", "confianza": "alta"|"media"|"baja"}. Sin markdown y sin texto extra. Aunque tengas poca información, da SIEMPRE una cifra orientativa y pon confianza "baja"; nunca respondas que no puedes estimarlo.`;
  const txt = await callClaude([{ role: "user", content: `Objeto de colección: ${name}${note ? ". Detalles: " + note : ""}. ¿Cuánto vale hoy en EUR?` }], sys);
  const j = parseClaudeJson(txt);
  if (j && typeof j.precio === "number" && isFinite(j.precio)) return { precio: j.precio, confianza: j.confianza || "media" };
  // red de seguridad: si contestó en prosa, rescatar el primer número
  const m = String(txt || "").match(/\d+(?:[.,]\d+)?/);
  const n = m ? Number(m[0].replace(",", ".")) : NaN;
  if (isFinite(n) && n > 0) return { precio: n, confianza: "baja" };
  return null;
}

/* ============================================================
   PRECIO REAL DE OBJETOS — todo desde el navegador, sin claves.
   Las tres fuentes permiten CORS (comprobado), así que no hace falta
   ni GitHub Action ni fichero de precios ni función intermedia.
   ============================================================ */
const OZ_EN_GRAMOS = 31.1034768;
const LEYES = [
  { k: 0.999, l: "999 (puro)" },
  { k: 0.916, l: "22k" },
  { k: 0.75, l: "18k" },
...
const FUENTES_PRECIO = [
  { k: "", l: "Manual" },
  { k: "metal", l: "Oro / plata al peso" },
  { k: "pokemon", l: "Carta Pokémon" },
  { k: "mtg", l: "Carta Magic" },
  { k: "ia", l: "Estimación con IA" },
];

/* pokemontcg.io suelta 500 de vez en cuando con el mismo id que luego funciona.
   Sin reintento, el precio "falla" por nada. */
async function fetchReintento(url, opts, intentos = 3) {
  let ultimo = null;
  for (let i = 0; i < intentos; i++) {
    try {
      const r = await fetch(url, opts);
      if (r.ok || r.status === 404) return r; // el 404 no mejora reintentando
      ultimo = new Error("HTTP " + r.status);
    } catch (e) { ultimo = e; }
    await new Promise((res) => setTimeout(res, 400 * (i + 1)));
  }
  throw ultimo || new Error("net");
}

async function precioPokemon(ref) {
  const id = ref && ref.cardId;
  if (!id) throw new Error("Falta el id de la carta (p. ej. base1-4)");
  const r = await fetchReintento(`https://api.pokemontcg.io/v2/cards/${encodeURIComponent(id)}`, { headers: { Accept: "application/json" } });
  if (!r.ok) throw new Error(r.status === 404 ? "No existe esa carta" : "No responde pokemontcg.io");
  const j = await r.json();
  const cm = j && j.data && j.data.cardmarket && j.data.cardmarket.prices;
  const p = cm && (cm.trendPrice || cm.averageSellPrice || cm.avg30);
  if (p) return { value: Math.round(p * 100) / 100, fuente: "Cardmarket" };
  throw new Error("Esa carta no tiene precio publicado");
}

async function precioMagic(ref) {
  const name = ref && ref.name;
  if (!name) throw new Error("Falta el nombre exacto de la carta");
  const u = new URL("https://api.scryfall.com/cards/named");
  u.searchParams.set("exact", name);
  if (ref.set) u.searchParams.set("set", ref.set);
  const r = await fetchReintento(u, { headers: { Accept: "application/json" } });
  if (!r.ok) throw new Error(r.status === 404 ? "No encuentro esa carta" : "No responde Scryfall");
  const d = await r.json();
  const p = d && d.prices && d.prices.eur;
  if (p) return { value: Math.round(parseFloat(p) * 100) / 100, fuente: "Cardmarket" };
  throw new Error("Esa carta no tiene precio en euros");
}

async function precioMetal(ref) {
  const sym = (ref && ref.metal) || "XAU";
  const g = Number((ref && ref.grams) || 0);
  const ley = Number(ref && ref.purity != null ? ref.purity : 0.999);
  if (!g) throw new Error("Pon cuántos gramos tienes");
  const rates = await fetchForex();
  const eurusd = rates && rates.USD ? rates.USD : null;
  if (!eurusd) throw new Error("No pude traer el cambio EUR/USD");
  const usdOz = await fetchMetalUsd(sym);
  if (!usdOz) throw new Error("No pude traer el precio del metal");
  const eurGramo = (usdOz / eurusd) / OZ_EN_GRAMOS;
  return { value: Math.round(eurGramo * g * ley * 100) / 100, fuente: (sym === "XAU" ? "Oro" : "Plata") + " · " + (Math.round(eurGramo * ley * 100) / 100) + " €/g" };
}

async function precioDeObjeto(item) {
  if (item.src === "pokemon") return precioPokemon(item.ref);
  if (item.src === "mtg") return precioMagic(item.ref);
  if (item.src === "metal") return precioMetal(item.ref);
  if (item.src === "ia") {
    const r = await aiEstimateCollectible(item.name, item.note);
    if (!r) throw new Error("La IA no supo ponerle precio");
    return { value: r.precio, fuente: "IA · confianza " + r.confianza };
  }
  throw new Error("Este objeto no tiene fuente de precio");
}
...
/* Nivel global: la suma de todo lo que has hecho, mires el área que mires.
   Es el número grande de la cabecera del juego. */
function nivelGlobal(state) {
  const r = rpgDe(state);
  const total = Object.values(r.xp || {}).reduce((s, v) => s + Number(v || 0), 0);
  return { ...nivelDeXp(total), total };
}

/* Radiografía de tus áreas: cuánto tienes en cada una, cuál destaca y cuál
   está pidiendo atención. Es lo que dibuja el radar. */
function radiografia(state) {
  const r = rpgDe(state);
  const areas = (r.areas && r.areas.length) ? r.areas : AREAS_DEFECTO;
  const filas = areas.map((a) => {
    const xp = Number((r.xp || {})[a.k] || 0);
    const n = nivelDeXp(xp);
    return { ...a, xp, nivel: n.nivel, dentro: n.dentro, paraSiguiente: n.paraSiguiente };
  });
  const tope = Math.max(1, ...filas.map((f) => f.xp));
  const conValor = filas.map((f) => ({ ...f, ratio: f.xp / tope }));
  const orden = [...conValor].sort((a, b) => b.xp - a.xp);
  const hayDatos = orden.length > 1 && orden[0].xp > 0;
  return {
    filas: conValor,
    fuerte: hayDatos ? orden[0] : null,
    flojo: hayDatos ? orden[orden.length - 1] : null,
    media: conValor.reduce((s, f) => s + f.nivel, 0) / (conValor.length || 1),
  };
}
...
/* ============================================================
   FUSIÓN DE DOS ESTADOS (este móvil + la nube)
   La v34 evitó que la nube pisara lo local eligiendo la copia más nueva,
   pero elegir una copia ENTERA sigue perdiendo lo que hiciste en el otro
   sitio. Esto fusiona pieza a pieza: gana la versión más reciente de cada
   cosa, y un borrado gana si es posterior a esa versión.
   ============================================================ */
const CLAVE_DE_COL = { sleepLog: "date" };
const SIN_FUSION = ["chat"]; // sin id estable: se queda la copia más nueva

function _fechaDe(it, porDefecto) {
  const t = Date.parse((it && (it.updatedAt || it.createdAt)) || "");
  return isNaN(t) ? (porDefecto || 0) : t;
}
function fusionarLista(col, a, b, aAt, bAt, tumbas) {
  const clave = CLAVE_DE_COL[col] || "id";
  const mapa = new Map();
  const meter = (arr, cuando) => (arr || []).forEach((it) => {
    const k = it && it[clave];
    if (k == null) return;
    const t = _fechaDe(it, cuando);
    const prev = mapa.get(k);
    if (!prev || t >= prev.t) mapa.set(k, { it, t });
  });
  meter(a, aAt);
  meter(b, bAt);
  const out = [];
  mapa.forEach((v, k) => {
    const borradoEn = tumbas && tumbas[k] ? Date.parse(tumbas[k]) : NaN;
    if (!isNaN(borradoEn) && borradoEn >= v.t) return; // el borrado es posterior: sigue borrado
    out.push(v.it);
  });
  return out;
}
function fusionarTumbas(a, b) {
  ...
  [a || {}, b || {}].forEach((src) => {
    Object.keys(src).forEach((col) => {
      out[col] = out[col] || {};
      Object.keys(src[col] || {}).forEach((id) => {
        const prev = Date.parse(out[col][id] || "");
        const nuevo = Date.parse(src[col][id] || "");
        if (isNaN(prev) || (!isNaN(nuevo) && nuevo > prev)) out[col][id] = src[col][id];
      });
    });
  });
  // podar: pasados 120 días una marca de borrado ya no aporta y solo ocupa
  const limite = Date.now() - 120 * 24 * 3600 * 1000;
  Object.keys(out).forEach((col) => {
    Object.keys(out[col]).forEach((id) => { const t = Date.parse(out[col][id]); if (!isNaN(t) && t < limite) delete out[col][id]; });
  });
  return out;
}
function fusionarEstados(local, nube) {
  const lAt = Date.parse((local && local.savedAt) || "") || 0;
  const nAt = Date.parse((nube && nube.savedAt) || "") || 0;
  const nubeGana = nAt > lAt;
  const base = nubeGana ? nube : local;   // para lo que no se puede fusionar
  const otro = nubeGana ? local : nube;
  const tumbas = fusionarTumbas(local && local._tomb, nube && nube._tomb);
  const out = { ...otro, ...base, _tomb: tumbas };
  Object.keys(EMPTY).forEach((col) => {
    if (!Array.isArray(EMPTY[col])) return;
    if (SIN_FUSION.indexOf(col) > -1) { out[col] = Array.isArray(base[col]) ? base[col] : []; return; }
    out[col] = fusionarLista(col, local && local[col], nube && nube[col], lAt, nAt, tumbas[col]);
  });
  return out;
}
...
function reducer(state, a) {
  switch (a.type) {
    case "hydrate": {
      // Nunca dejar una colección en undefined: los datos guardados de versiones
      // anteriores no traen las colecciones nuevas (p. ej. "fixeds" de la v33).
      const next = { ...state, ...a.payload, ready: true };
      for (const k in EMPTY) if (Array.isArray(EMPTY[k]) && !Array.isArray(next[k])) next[k] = [];
      return next;
    }
    case "ready":
    ...
    case "add": {
      // Sello de alta para TODO lo que se crea (deudas, objetivos, huchas, colecciones…).
      // Se pone aquí y no en cada pantalla para que no se olvide en ningún sitio.
      const ahora = nowISO();
      const item = { ...a.item, createdAt: (a.item && a.item.createdAt) || ahora, updatedAt: ahora };
      return { ...state, [a.col]: [item, ...(state[a.col] || [])] };
    }
    case "update":
      // updatedAt en cada cambio: es lo que decide quién gana al fusionar
      return {
        ...state,
        [a.col]: (state[a.col] || []).map((x) => (x.id === a.id ? { ...x, ...a.patch, updatedAt: nowISO() } : x)),
      };
    case "remove": {
      // Dejar marca de borrado para que no reviva desde el otro dispositivo
      const tomb = { ...(state._tomb || {}) };
      tomb[a.col] = { ...(tomb[a.col] || {}), [a.id]: nowISO() };
      return { ...state, _tomb: tomb, [a.col]: (state[a.col] || []).filter((x) => x.id !== a.id) };
    }
    case "setCol": {
      const ahora2 = nowISO();
      const clave = CLAVE_DE_COL[a.col] || "id";
      const antes = state[a.col] || [];
      const items = (a.items || []).map((x) => (x && typeof x === "object" && !x.updatedAt ? { ...x, updatedAt: ahora2 } : x));
      const vivos = new Set(items.map((x) => x && x[clave]).filter((k) => k != null));
      const tomb2 = { ...(state._tomb || {}) };
      const delCol = { ...(tomb2[a.col] || {}) };
      let hubo = false;
      antes.forEach((x) => { const k = x && x[clave]; if (k != null && !vivos.has(k)) { delCol[k] = ahora2; hubo = true; } });
      if (hubo) tomb2[a.col] = delCol;
      return { ...state, _tomb: hubo ? tomb2 : state._tomb, [a.col]: items };
    }
    case "drive":
  ...
  return (
    <img src={BROSIN_EYE} alt="Brosin" draggable={false}
      style={{ width: size, height: size, objectFit: "contain", display: "block", userSelect: "none", pointerEvents: "none",
        opacity: muted ? 0.42 : 1, filter: muted ? (C.base === "light" ? "grayscale(1) brightness(.75)" : "grayscale(1) brightness(1.7)") : "none" }} />
  );
}
...
// Panel de cristal. Antes era una placa opaca con degradado; ahora deja ver
// el fondo con desenfoque y saturación. Se re-tokeniza en applyTheme().
const card = glassSurface(22);
...
/* Bottom sheet modal */
/* Apilado de paneles: lo ÚLTIMO que se abre queda encima, siempre.
   Con números fijos no hay manera: si el chat está por encima de los paneles,
   tapa lo que abras desde él; y si está por debajo, se lo comen los avisos.
   Cada panel coge un número nuevo al abrirse y así el orden es el real. */
let _zCounter = 90;
function useTopZ(active) {
  const zRef = useRef(90);
  const prev = useRef(false);
  if (active && !prev.current) {
    if (_zCounter > 800) _zCounter = 90; // no dejar que crezca sin fin
    _zCounter += 2;
    zRef.current = _zCounter;
  }
  prev.current = active;
  return zRef.current;
}

/* Alto del teclado virtual (iOS/Android). Devuelve 0 si no hay teclado abierto.
   Sin esto, en iPhone el teclado tapa el campo de escribir de los paneles. */
    ...
    >
      <div
        ...
        style={{
          width: "100%",
          maxWidth: 440,
          // Hoja de cristal: más cuerpo que una tarjeta porque lleva formularios
          // encima y el texto tiene que ganar al fondo.
          background: `linear-gradient(180deg, ${glassTokens().sheen}, ${C.base === "light" ? "rgba(255,255,255,.72)" : "rgba(12,12,14,.62)"})`,
          backdropFilter: "blur(24px) saturate(1.8) brightness(1.06)",
          WebkitBackdropFilter: "blur(24px) saturate(1.8) brightness(1.06)",
          borderTopLeftRadius: 28,
          borderTopRightRadius: 28,
          borderTop: `1px solid ${glassTokens().edge}`,
          boxShadow: `inset 0 1px 0 ${glassTokens().hi}, 0 -18px 50px ${C.dropSheet}`,
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
      ...
      <div style={{ fontSize: 12.5, color: C.textLo }}>
        {value ? (
          <button onClick={() => onChange(null)} style={{ ...iconBtn, width: "auto", paddingInline: 12, gap: 6, fontSize: 13, color: C.textLo }}><X size={14} /> {tr("Quitar foto")}</button>
        ) : (
...
function SpeakBtn({ text, size = 32, tone = C.yellow }) {
  ...
  return (
    <button type="button" onClick={toggle} title={on ? tr("Parar") : tr("Escuchar")} style={{
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 10 }}>
        <button className="b3d-dark" onClick={() => press("back")} style={{ ...iconBtn, width: "auto", paddingInline: 16, height: 40, gap: 6, fontWeight: 800, fontSize: 13 }}><Delete size={16} /> {tr("Borrar")}</button>
      </div>
...
function QuickIdea({ onSave }) {
  ...
  return (
    <div>
      <Field label={tr("Tu idea (dicta o escribe)")}>
        <textarea style={{ ...inputStyle, minHeight: 120, resize: "vertical" }} value={text} onChange={(e) => setText(e.target.value)} placeholder={tr("Pulsa el micro y habla; lo paso a texto…")} />
      </Field>
      <div style={{ display: "flex", gap: 12, alignItems: "center", marginBottom: 16 }}>
        <MicButton onResult={setText} base={text} size={54} />
        <span style={{ color: C.textLo, fontSize: 12.5, lineHeight: 1.4 }}>
          {sttSupported() ? tr("Pulsa el micro y habla — se transcribe solo. Vuelve a pulsar para parar.") : tr("Este navegador no deja dictar; escribe la idea a mano.")}
        </span>
      </div>
      <PrimaryBtn full color={C.purple} onClick={() => text.trim() && onSave({ title, body: text.trim() })}>
        <Check size={18} strokeWidth={3} color="#fff" /> <span style={{ color: "#fff" }}>{tr("Guardar como nota")}</span>
      </PrimaryBtn>
    </div>
  );
}
...
function MonthlyBars({ tx }) {
  ...
  for (let i = 5; i >= 0; i--) {
    ...
    months.push({ key: `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}`, label: d.toLocaleDateString(LOCALE, { month: "short" }) });
  }
  ...
  return (
    <div style={{ ...card, padding: 16 }}>
      <ChartHead icon={BarChart3} title={tr("Ingresos vs gastos")} sub={`Neto ${net >= 0 ? "+" : ""}${money(net)}`} />
...
function CategoryDonut({ tx }) {
  ...
  return (
    <div style={{ ...card, padding: 16 }}>
      <ChartHead icon={PieChart} title={tr("Gastos por categoría")} sub={money(total)} />
...
function BalanceLine({ tx }) {
  ...
  return (
    <div style={{ ...card, padding: 16 }}>
      <ChartHead icon={TrendingUp} title={tr("Evolución del balance")} sub={money(pts[pts.length - 1])} />
...
function Toasts({ items, onDismiss }) {
  return (
    <div style={{ position: "fixed", left: 0, right: 0, bottom: "calc(84px + env(safe-area-inset-bottom))", zIndex: 88, padding: "0 12px", display: "grid", gap: 8, pointerEvents: "none" }}>
...
function LockScreen({ mode, onUnlock, onSet }) {
  ...
  const label = mode === "set" ? (confirm === null ? tr("Crea tu PIN") : tr("Repite el PIN")) : "Hola otra vez";
      ...
      <div key={step} style={{ flex: 1, animation: "bslide .35s ease" }}>
        {step === 0 && (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", marginTop: 30 }}>
            ...
            <p style={{ color: C.textHi, fontWeight: 800, fontSize: 18, margin: "20px 0 0" }}>{tr("Tu segundo cerebro.")}</p>
        ...
        {step === 1 && (
          <div style={{ marginTop: 20 }}>
            <Title kicker={tr("Empecemos")}>{tr("¿Cómo te llamo?")}</Title>
            <p style={{ color: C.textLo, fontSize: 13.5, marginBottom: 18 }}>{tr("Para hablarte de tú a tú, como un colega.")}</p>
            <input style={{ ...inputStyle, fontSize: 18, padding: "16px 16px" }} placeholder={tr("Tu nombre o apodo")} value={name} onChange={(e) => setName(e.target.value)} autoFocus />
          </div>
        )}
        {step === 2 && (
          <div style={{ marginTop: 20 }}>
            <Title kicker={tr("A medida")}>{tr("¿Qué quieres dominar?")}</Title>
            <p style={{ color: C.textLo, fontSize: 13.5, marginBottom: 18 }}>{tr("Elige lo que más te importa. Lo tendrás todo, pero esto va arriba.")}</p>
        ...
        {step === 3 && (
          <div style={{ marginTop: 20 }}>
            <Title kicker={tr("Tu gente")}>{tr("¿Algún cumple que no quieras olvidar?")}</Title>
            <p style={{ color: C.textLo, fontSize: 13.5, marginBottom: 18 }}>{tr("Apunta a tu familia o amigos y Brosin te avisará con tiempo para preparar el regalo. Puedes saltarte esto y añadirlos luego.")}</p>
            <div style={{ display: "grid", gap: 10 }}>
              {ppl.map((p) => (
                <div key={p.id} style={{ display: "flex", gap: 8 }}>
                  <input style={{ ...inputStyle, flex: 1 }} value={p.name} onChange={(e) => setP(p.id, "name", e.target.value)} placeholder={tr("Nombre")} />
                  <input type="date" style={{ ...inputStyle, width: 150 }} value={p.birthday} onChange={(e) => setP(p.id, "birthday", e.target.value)} />
                </div>
              ))}
              <button onClick={addP} style={{ ...iconBtn, width: "auto", paddingInline: 14, height: 42, gap: 8, fontWeight: 700, fontSize: 13.5 }}><UserPlus size={16} /> {tr("Añadir otra persona")}</button>
            </div>
          </div>
        )}
        {step === 4 && (
        ...
        <div style={{ flex: 1 }}>
          {step < 4 ? (
            <PrimaryBtn full onClick={next}>
              {step === 3 ? tr("Casi está") : tr("Siguiente")} <ChevronRight size={18} strokeWidth={3} />
            </PrimaryBtn>
          ) : (
...
function DayRing({ state, todayL }) {
  ...
  if (totalItems === 0) {
    return (
      <div style={{ ...card, padding: 16, marginBottom: 14, display: "flex", alignItems: "center", gap: 12 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ color: C.textHi, fontWeight: 800, fontSize: 14.5 }}>{tr("Tu día, en un anillo")}</div>
          <div style={{ color: C.textLo, fontSize: 12.5, marginTop: 2, lineHeight: 1.4 }}>{tr("Añade un evento o marca un hábito y verás tu progreso llenarse.")}</div>
        </div>
      </div>
    );
  }
  ...
  const status = allDone ? "¡Día completado!" : overall >= 66 ? "Casi lo tienes" : overall >= 33 ? "Vas en marcha" : overall > 0 ? tr("Buen comienzo") : tr("Empieza tu día");
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
        <div style={{ position: "relative", flexShrink: 0, width: 122, height: 122 }}>
          ...
          <svg width={122} height={122} viewBox="0 0 122 122" style={{ display: "block" }}>
            ...
            <text x="61" y="74" textAnchor="middle" fill={C.textLo} fontSize="9" fontWeight="700" letterSpacing="0.5">{tr("DE HOY")}</text>
          </svg>
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: "grid", gap: 9 }}>
            <RingLegend color={C.yellow} label={tr("Agenda")} done={evDone} total={evTotal} />
            <RingLegend color={C.purple} label={tr("Hábitos")} done={hbDone} total={hbTotal} />
          </div>
        </div>
      </div>
    </div>
...
function ToolButton({ icon: Ico, label, accent, onClick, active, anim }) {
  ...
  // si el usuario navega a otra pestaña antes de que acabe la animación, el timer
  // muere con el componente (sin esto, el onClick diferido "secuestraba" la navegación)
  ...
  const fire = () => {
    ...
    if (playing) return;
    setPlaying(true);
    timerRef.current = setTimeout(() => { setPlaying(false); onClick && onClick(); }, 430);
  };
  return (
...
/* "Tu día en 30 segundos" — el ritual matinal: todo lo de hoy en una tarjeta */
function DailyBrief({ state, dispatch, streak, ai, go, onClose }) {
  const t = todayISO();
  const [aiLine, setAiLine] = useState(null);
  useEffect(() => {
    if (!ai) return;
    let alive = true;
    callClaude([{ role: "user", content: "Dame UNA sola frase (máximo 22 palabras) de enfoque para mi día de hoy, concreta a mis datos, sin comillas ni markdown." }],
      `Eres la mente de Brosin. Hoy es ${fmtLong()}. Sé concreto y útil, tono sereno.\n=== DATOS ===\n${buildMemory(state)}` + aiLangRule())
      .then((txt) => { if (alive && txt) setAiLine(String(txt).trim().slice(0, 220)); })
      .catch(() => {});
    return () => { alive = false; };
  }, []);
  const evs = (state.events || []).filter((e) => sameDay(e.date, t)).sort((a, b) => (a.time || "").localeCompare(b.time || ""));
  const habits = state.habits || [];
  const hbPend = habits.filter((h) => !(h.done || []).includes(t));
  const yesterday = (() => { const d = new Date(); d.setDate(d.getDate() - 1); return dayISOOf(d); })();
  const spentY = (state.tx || []).filter((x) => x.type === "gasto" && sameDay(x.date, yesterday)).reduce((s, x) => s + Number(x.amount || 0), 0);
  ...
  const Row = ({ icon, children }) => (
    <div style={{ display: "flex", gap: 10, alignItems: "flex-start", padding: "9px 0", borderBottom: `1px solid ${C.line}` }}>
      ...
      <div style={{ flex: 1, minWidth: 0, color: C.textHi, fontSize: 13.5, lineHeight: 1.5 }}>{children}</div>
    </div>
  );
  return (
    <div>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
        <div style={{ flex: 1 }}>
          <div style={{ color: C.textLo, fontSize: 12, fontWeight: 700, textTransform: "capitalize" }}>{fmtLong()}</div>
          <div style={{ color: C.textHi, fontWeight: 900, fontSize: 19 }}>Hola, {state.profile.name || "Bro"} — {tr("tu día en 30s")}</div>
        </div>
        <span style={{ fontSize: 13, fontWeight: 900, color: streak.activeToday ? C.yellow : C.textLo }}>🐝 {streak.count}</span>
      </div>

      {aiLine && <div style={{ ...card, padding: "11px 13px", marginBottom: 10, background: C.tintP || "rgba(144,19,254,.08)", border: "1px solid rgba(144,19,254,.45)", color: C.textHi, fontSize: 13.5, lineHeight: 1.5 }}>✨ {aiLine}</div>}

      <div style={{ ...card, padding: "4px 14px 6px", marginBottom: 12 }}>
        <Row icon="📅">
          {evs.length === 0 ? <span style={{ color: C.textLo }}>{tr("Sin eventos hoy — día despejado.")}</span> : evs.map((e) => (
            <div key={e.id} style={{ display: "flex", gap: 8, alignItems: "baseline" }}>
              <span style={{ color: C.yellow, fontWeight: 800, fontSize: 12.5, fontVariantNumeric: "tabular-nums", minWidth: 42 }}>{e.time || "—"}</span>
              <span style={{ textDecoration: e.done ? "line-through" : "none", opacity: e.done ? 0.55 : 1 }}>{e.title}</span>
            </div>
          ))}
        </Row>
        <Row icon="🎯">
          {habits.length === 0 ? <span style={{ color: C.textLo }}>{tr("Sin hábitos aún — crea el primero en Objetivos.")}</span> : hbPend.length === 0 ? <span style={{ color: C.good, fontWeight: 700 }}>{tr("Todos los hábitos hechos ✓")}</span> : (
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
              {habits.map((h) => { const done = (h.done || []).includes(t); return (
                <button key={h.id} onClick={() => toggleHabit(h)} style={{ borderRadius: 999, padding: "6px 12px", fontSize: 12.5, fontWeight: 800, cursor: "pointer", border: `1.5px solid ${done ? C.good : C.line}`, background: done ? "rgba(61,220,132,.15)" : "transparent", color: done ? C.good : C.textHi }}>
                  {done ? "✓ " : ""}{h.name}
                </button>
              ); })}
            </div>
        ...
        {(spentY > 0 || owedToMe > 0) && (
          <Row icon="💶">
            {spentY > 0 && <span>{tr("Ayer gastaste")} <b style={{ color: C.textHi }}>{money(spentY)}</b>. </span>}
            {owedToMe > 0 && <span>{tr("Te deben")} <b style={{ color: C.yellow }}>{money(owedToMe)}</b>.</span>}
          </Row>
        )}
        {bds.length > 0 && (
          <Row icon="🎂">{bds.map(({ p, d }) => <div key={p.id}>{d === 0 ? <b style={{ color: C.purple }}>{tr("¡Hoy cumple")} {p.name}!</b> : d === 1 ? tri("Mañana cumple {nombre}", { nombre: p.name }) : tri("{nombre} cumple en {n} días", { nombre: p.name, n: d })}</div>)}</Row>
        )}
        {nearGoal.length > 0 && (
          <Row icon="🐷">{nearGoal.map(({ h, pct }) => <div key={h.id}>{tr("La hucha")} <b style={{ color: C.textHi }}>{h.name}</b> {tr("va al")} <b style={{ color: C.good }}>{Math.round(pct)}%</b> {tr("— el empujón final.")}</div>)}</Row>
        )}
      </div>

      <div style={{ display: "flex", gap: 8 }}>
        <button onClick={() => { onClose && onClose(); go && go("agenda"); }} style={miniBtn(C.ink3, C.textHi)}>{tr("Ver agenda")}</button>
        <button onClick={onClose} style={miniBtn(C.yellow, C.ink)}>{tr("¡A por el día! 🐝")}</button>
      </div>
    </div>
  );
}

/* Mapa de calor de constancia (estilo GitHub/TickTick): 18 semanas de actividad */
function ActivityHeatmap({ state }) {
  const counts = {};
  (state.habits || []).forEach((h) => (h.done || []).forEach((d) => { counts[d] = (counts[d] || 0) + 1; }));
  (state.events || []).forEach((e) => { if (e.done && e.date) { const k = String(e.date).slice(0, 10); counts[k] = (counts[k] || 0) + 1; } });
  ...
  for (let w = 0; w < WEEKS; w++) {
    ...
    for (let d = 0; d < 7; d++) {
      ...
      const iso = dayISOOf(dt);
      col.push({ iso, n: counts[iso] || 0, today: iso === dayISOOf(today) });
    }
    cols.push(col);
  }
  const colOf = (n) => (n === 0 ? C.ink3 : n === 1 ? "rgba(255,212,0,.35)" : n === 2 ? "rgba(255,212,0,.65)" : C.yellow);
  return (
    <div style={{ ...card, padding: 14, marginBottom: 14, background: C.ink }}>
      <div style={{ color: C.textHi, fontWeight: 800, fontSize: 13 }}>{tr("Tu constancia")} · 18 {tr("semanas")}</div>
      <div style={{ display: "flex", gap: 3, justifyContent: "center", marginTop: 10 }}>
        {cols.map((col, i) => (
          <div key={i} style={{ display: "grid", gap: 3 }}>
            {col.map((c, j) => (
              <div key={j} style={{ width: 11, height: 11, borderRadius: 3, background: c ? colOf(c.n) : "transparent", outline: c && c.today ? `1.5px solid ${C.textHi}` : "none", outlineOffset: -1 }} />
            ))}
          </div>
        ))}
      </div>
      <div style={{ display: "flex", alignItems: "center", gap: 5, justifyContent: "flex-end", marginTop: 10 }}>
        <span style={{ color: C.textLo, fontSize: 10.5 }}>{tr("Menos")}</span>
        {[0, 1, 2, 3].map((n) => <span key={n} style={{ width: 10, height: 10, borderRadius: 3, background: colOf(n), display: "inline-block" }} />)}
        <span style={{ color: C.textLo, fontSize: 10.5 }}>{tr("Más")}</span>
      </div>
    </div>
  );
}

function StreakPanel({ state, streak }) {
  const p = state.profile || {};
  const best = Math.max(p.streakBest || 0, streak.count, hiveBestStreak(state));
  const freezes = p.streakFreezes || 0;
  const savedCount = (p.streakSaved || []).length;
  const toNext = 7 - (streak.count % 7 || 7) + (streak.count % 7 === 0 ? 7 : 0);
  return (
    <div>
      <div style={{ textAlign: "center", padding: "6px 0 16px" }}>
        <div style={{ fontSize: 46, lineHeight: 1 }}>🐝</div>
        <div style={{ color: C.textHi, fontWeight: 900, fontSize: 44, lineHeight: 1.1, marginTop: 6, fontVariantNumeric: "tabular-nums" }}>{streak.count}</div>
        <div style={{ color: C.textLo, fontSize: 13, fontWeight: 700 }}>{streak.count === 1 ? tr("día de racha") : tr("días de racha")}</div>
        <div style={{ marginTop: 10, display: "inline-block", borderRadius: 999, padding: "6px 14px", fontSize: 12.5, fontWeight: 800, background: streak.activeToday ? "rgba(61,220,132,.14)" : "rgba(255,212,0,.12)", border: `1px solid ${streak.activeToday ? C.good : C.yellow}`, color: streak.activeToday ? C.good : C.yellow }}>
          {streak.activeToday ? tr("Hoy ya cuenta ✓") : tr("Hoy aún en juego — completa 1 hábito o evento")}
        </div>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginBottom: 14 }}>
        <MiniStat label={tr("MEJOR RACHA")} val={String(best)} col={C.yellow} />
        <MiniStat label={tr("CONGELADORES")} val={`${freezes}/2`} col={C.textHi} />
        <MiniStat label={tr("DÍAS SALVADOS")} val={String(savedCount)} col={C.purple} />
      </div>
      {(() => { const nx = THEME_LIST.find((th) => th.req > 0 && best < th.req); return nx ? (
        <div style={{ ...card, padding: "10px 14px", marginBottom: 14, display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ fontSize: 22 }}>{nx.emoji}</span>
          <div style={{ flex: 1 }}>
            <div style={{ color: C.textLo, fontSize: 10.5, fontWeight: 800, letterSpacing: 0.6, textTransform: "uppercase" }}>{tr("Próxima recompensa")}</div>
            <div style={{ color: C.textHi, fontSize: 13, fontWeight: 700 }}>{tr("Tema de la app")}: {tr(nx.name)} · {nx.req} {tr("días de racha")}</div>
          </div>
          <span style={{ color: C.yellow, fontWeight: 900, fontSize: 13, fontVariantNumeric: "tabular-nums" }}>{best}/{nx.req}</span>
        </div>
      ) : null; })()}
      <ActivityHeatmap state={state} />
      <div style={{ ...card, padding: 14, background: C.ink, fontSize: 12.5, color: C.textLo, lineHeight: 1.6 }}>
        <b style={{ color: C.textHi }}>{tr("Cómo funciona:")}</b> {tr("cada día que completas al menos")} <b style={{ color: C.textHi }}>{tr("1 hábito o 1 evento")}</b>{tr(", la racha suma. Cada")} <b style={{ color: C.yellow }}>{tr("7 días")}</b> {tr("ganas un")} <b style={{ color: C.textHi }}>{tr("congelador")}</b> {tr("(máx. 2): si un día no puedes, se gasta solo y tu racha sobrevive.")} {streak.count > 0 ? (toNext === 1 ? tr("Te falta 1 día para el próximo congelador.") : tri("Te faltan {n} días para el próximo congelador.", { n: toNext })) : tr("Empieza hoy: marca cualquier hábito o evento.")}
      </div>
    </div>
  );
}
...
function HomeScreen({ state, dispatch, go, ai, openMarkets, toast }) {
  ...
  const streak = hiveStreak(state);
  /* ritual matinal: se abre solo la primera vez del día (desde las 5:00), una vez */
  useEffect(() => {
    if (!state.ready) return;
    const p = state.profile || {};
    if (p.briefOn === false) return;
    const t = todayISO();
    if (p.briefSeen === t || new Date().getHours() < 5) return;
    const id = setTimeout(() => { setBriefOpen(true); dispatch({ type: "profile", payload: { briefSeen: t } }); }, 650);
    return () => clearTimeout(id);
  }, [state.ready]);
  const [homeTab, setHomeTab] = useState("vistazo"); // vistazo | hoy | semana — se abre en Vistazo
  ...
  const greet = hour < 6 ? tr("De madrugada") : hour < 14 ? tr("Buenos días") : hour < 21 ? tr("Buenas tardes") : tr("Buenas noches");
  ...
  const tabs = [{ k: "vistazo", label: tr("Vistazo") }, { k: "hoy", label: tr("Hoy") }, { k: "semana", label: tr("Semana") }];
  ...
  return (
    <div style={{ padding: "18px 18px 0" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 18 }}>
        ...
        <button onClick={() => setStreakOpen(true)} style={{ ...iconBtn, width: "auto", paddingInline: 11, gap: 5, borderColor: streak.activeToday ? C.yellow : C.line, background: streak.activeToday ? "rgba(255,212,0,.14)" : C.ink3 }} title={tr("Racha de la colmena")}>
          <span style={{ fontSize: 14, filter: streak.activeToday ? "none" : "grayscale(1)", opacity: streak.activeToday ? 1 : 0.65 }}>🐝</span>
          <span style={{ color: streak.activeToday ? C.yellow : C.textLo, fontWeight: 900, fontSize: 13.5, fontVariantNumeric: "tabular-nums" }}>{streak.count}</span>
        </button>
        <button onClick={() => go("perfil")} style={iconBtn}><Settings size={18} /></button>
      </div>

      {/* big agenda date — tócala para abrir "Tu día en 30 segundos" */}
      <button onClick={() => setBriefOpen(true)} style={{ display: "block", width: "100%", textAlign: "left", background: "none", border: "none", padding: 0, cursor: "pointer", marginBottom: 18 }}>
        <div style={{ color: C.accentText, fontSize: 11.5, fontWeight: 800, letterSpacing: 3, textTransform: "uppercase", textShadow: C.base === "dark" ? "0 0 12px rgba(255,212,0,.35)" : "none" }}>{weekdayLong}</div>
        <div style={{ display: "flex", alignItems: "baseline", gap: 8, marginTop: 2 }}>
          ...
          <span style={{ flex: 1 }} />
          <span style={{ color: C.textLo, fontSize: 12.5, fontWeight: 600 }}>{todays.length ? `${todays.length} evento${todays.length === 1 ? "" : "s"} hoy` : tr("día libre")} ›</span>
        </div>
      </button>
      ...
      {ai && (
        <button onClick={() => setAiOpen(true)} style={{ ...card, width: "100%", display: "flex", alignItems: "center", gap: 10, padding: "12px 14px", marginBottom: 16, cursor: "pointer", border: "1px solid rgba(144,19,254,.5)", background: C.tintP, boxShadow: "0 1px 0 rgba(255,255,255,.07) inset, 0 6px 18px rgba(144,19,254,.16), 0 10px 26px rgba(0,0,0,.35)", textAlign: "left" }}>
          ...
          <span style={{ flex: 1, color: C.textLo, fontSize: 13.5 }}>{tr("Dile a Brosin y lo apunto…")}</span>
      ...
      <div key={homeTab} style={{ animation: "bfade .25s ease" }}>
          ...
          <>
            ...
            {todays.length === 0 ? (
              <div style={{ ...card, padding: "30px 20px", textAlign: "center" }}>
                ...
                <div style={{ color: C.textHi, fontWeight: 800, fontSize: 16 }}>{tr("Día limpio")}</div>
                <div style={{ color: C.textLo, fontSize: 13, marginTop: 4, maxWidth: 240, marginInline: "auto", lineHeight: 1.5 }}>{tr("No tienes nada para hoy. Disfrútalo o planifica algo.")}</div>
                <button onClick={() => go("agenda")} style={{ ...iconBtn, width: "auto", margin: "16px auto 0", paddingInline: 16, gap: 6, fontWeight: 800, fontSize: 13, background: C.yellow, color: C.ink, borderColor: C.yellow }}><Plus size={15} strokeWidth={3} /> {tr("Planificar")}</button>
              </div>
            ) : (
        ...
        {homeTab === "semana" && (
          <div>
                ...
                <div key={iso} style={{ marginBottom: 16 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                    <div style={{ width: 42, textAlign: "center", flexShrink: 0 }}>
                      <div style={{ color: isToday ? C.yellow : C.textLo, fontSize: 10, fontWeight: 800, textTransform: "uppercase", letterSpacing: 0.5 }}>{d.toLocaleDateString(LOCALE, { weekday: "short" }).replace(".", "")}</div>
                      <div style={{ color: isToday ? C.textHi : C.textLo, fontSize: 20, fontWeight: 900, lineHeight: 1 }}>{d.getDate()}</div>
                    </div>
                    <div style={{ flex: 1, height: 1, background: C.line }} />
                    {isToday && <span style={{ color: C.ink, background: C.yellow, fontSize: 9.5, fontWeight: 800, borderRadius: 999, padding: "2px 8px", letterSpacing: 0.5 }}>{tr("HOY")}</span>}
                  </div>
                  {evs.length === 0 ? (
                    <div style={{ color: C.textLo, fontSize: 12.5, paddingLeft: 52, opacity: 0.55 }}>{tr("Sin planes")}</div>
                  ) : (
            ...
            <button onClick={() => go("agenda")} style={{ ...iconBtn, width: "100%", height: 46, gap: 8, fontWeight: 800, fontSize: 13.5, background: C.ink2 }}><CalendarDays size={16} /> {tr("Abrir agenda completa")}</button>
          </div>
        )}
        ...
        {homeTab === "vistazo" && (
          <>
            ...
            <MisionDeHoy state={state} dispatch={dispatch} toast={toast} onVerBanco={() => go("juego")} />
            ...
            {birthday && (
              <button onClick={() => go("personas")} style={{ ...card, width: "100%", textAlign: "left", padding: "12px 14px", marginBottom: 14, display: "flex", alignItems: "center", gap: 10, cursor: "pointer", borderColor: C.purple, background: "rgba(144,19,254,.08)" }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ color: C.textLo, fontSize: 12 }}>{(birthday.p.giftIdeas || []).some((g) => !g.done) ? tr("Tienes ideas de regalo pendientes") : tr("Prepara un detalle")}</div>
                </div>
      ...
      <Sheet open={briefOpen} onClose={() => setBriefOpen(false)} title={tr("Tu día en 30 segundos")}>
        {briefOpen && <DailyBrief state={state} dispatch={dispatch} streak={streak} ai={ai} go={go} onClose={() => setBriefOpen(false)} />}
      </Sheet>
      <Sheet open={streakOpen} onClose={() => setStreakOpen(false)} title={tr("Racha de la colmena")}>
        <StreakPanel state={state} streak={streak} />
      </Sheet>
      <Sheet open={aiOpen} onClose={() => setAiOpen(false)} title={tr("Dile a Brosin")}>
        <HomeAI ai={ai} state={state} dispatch={dispatch} toast={toast} />
      </Sheet>
      <Sheet open={calcOpen} onClose={() => setCalcOpen(false)} title={tr("Calculadora")}>
        <Calc />
      </Sheet>
      <Sheet open={ideaOpen} onClose={() => setIdeaOpen(false)} title={tr("Guardar una idea")}>
...
function DailyQuote() {
  ...
  return (
    <div style={{ ...card, padding: "12px 14px", marginBottom: 16, background: C.tintY, border: "1px solid rgba(255,212,0,.22)", display: "flex", alignItems: "center", gap: 10 }}>
      ...
      <button onClick={() => setI((x) => (x + 1) % MOTIVATION.length)} title={tr("Otra frase")} style={{ ...iconBtn, width: 28, height: 28, flexShrink: 0 }}><RefreshCcw size={13} /></button>
    </div>
  );
}
...
function PeopleScreen({ state, dispatch, back }) {
  ...
  return (
    <div style={{ padding: "20px 18px 0" }}>
      <button onClick={back} style={{ ...linkBtn, marginBottom: 10 }}><ChevronLeft size={16} /> {tr("Inicio")}</button>
      <Title kicker={tr("Tu gente")}>{tr("Cumpleaños")}</Title>
      {people.length === 0 ? (
        <EmptyState face="happy" title={tr("No olvides a los tuyos")} sub={tr("Añade los cumpleaños de tu familia y amigos, apunta ideas de regalo y ten los detalles listos. Brosin te avisa con tiempo.")} cta={tr("Añadir persona")} onCta={openNew} />
      ) : (
      ...
      <FAB onClick={openNew} label={tr("Persona")} />
      <Sheet open={sheet} onClose={() => setSheet(false)} title={edit ? tr("Editar persona") : tr("Nueva persona")}>
  ...
  return (
    <div style={{ ...card, padding: 16, borderColor: isToday ? C.yellow : soon ? "#3a3200" : C.line }}>
      ...
      <div style={{ marginTop: 12 }}>
        <div style={{ color: C.textLo, fontSize: 11.5, fontWeight: 700, marginBottom: 6, display: "flex", alignItems: "center", gap: 6 }}><Gift size={13} color={C.purple} /> {tr("Regalos / detalles")}</div>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
          <input style={{ ...inputStyle, padding: "8px 12px", fontSize: 13 }} value={newIdea} onChange={(e) => setNewIdea(e.target.value)} onKeyDown={(e) => e.key === "Enter" && addIdea()} placeholder={tr("Idea de regalo o detalle…")} />
      ...
      <button onClick={genMsg} disabled={msgBusy} style={{ ...iconBtn, width: "100%", height: 40, marginTop: 12, gap: 8, fontWeight: 800, fontSize: 13, background: "rgba(144,19,254,.14)", borderColor: C.purple, color: C.purple }}>
        ...
        {msgBusy ? "Escribiendo…" : msg ? tr("Otra felicitación") : tr("Felicitación con IA")}
      </button>
      {msg && (
        <div style={{ ...card, padding: 12, marginTop: 8, background: C.well }}>
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
            <button onClick={copyMsg} style={miniBtn(copied ? C.good : C.ink3, copied ? C.ink : C.textHi)}>{copied ? tr("¡Copiado!") : tr("Copiar")}</button>
...
function PersonForm({ initial, onSave, onDelete }) {
  ...
  return (
    <div>
      <Field label={tr("Nombre")}><input style={inputStyle} value={name} onChange={(e) => setName(e.target.value)} placeholder={tr("Nombre")} autoFocus /></Field>
      <Field label={tr("¿Quién es?")}><div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>{rels.map((r) => <Chip key={r} active={relation === r} onClick={() => setRelation(relation === r ? "" : r)}>{r}</Chip>)}</div></Field>
      <Field label={tr("Cumpleaños (el año es opcional, para la edad)")}><input type="date" style={inputStyle} value={birthday} onChange={(e) => setBirthday(e.target.value)} /></Field>
      <button onClick={() => setRemind((r) => !r)} style={{
        ...
        <span style={{ flex: 1, textAlign: "left", color: C.textHi, fontWeight: 700, fontSize: 14 }}>{tr("Avisarme antes del cumple")}</span>
      ...
      <Field label={tr("Notas (gustos, tallas, alergias…)")}><textarea style={{ ...inputStyle, minHeight: 70, resize: "vertical" }} value={notes} onChange={(e) => setNotes(e.target.value)} placeholder={tr("Lo que quieras recordar de esta persona")} /></Field>
      <Field label={tr("Foto (opcional)")}><PhotoPicker value={photo} onChange={setPhoto} /></Field>
      <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
        ...
        <div style={{ flex: 1 }}>
          <PrimaryBtn full onClick={() => name.trim() && onSave({ name: name.trim(), relation, birthday, notes, photo, remind })}><Check size={18} strokeWidth={3} /> {tr("Guardar")}</PrimaryBtn>
        </div>
      </div>
    </div>
  );
...
- {"type":"event","title":"...","date":"YYYY-MM-DD","time":"HH:MM"|null,"kind":"Cita"|"Recordatorio"|"Tarea"|"Evento"|"Pago"|"Cumpleaños"|"Entrenamiento"|"Reunión"|"Trabajo"|"Viaje"|"Médico"|"Ocio"|null}
...
- {"type":"debt","dir":"meDeben"|"yoDebo","person":"...","amount":number,"note":"..."}  (alguien me debe / yo debo)
- {"type":"saving","name":"...","amount":number}  (meter dinero en una hucha; amount negativo = sacar)
- {"type":"habit","name":"..."}  (marcar un hábito como hecho HOY)
Reglas: interpreta fechas relativas ("mañana","el día 20","el viernes que viene") a fecha real a partir de hoy. No inventes importes, fechas ni horas que el usuario no haya dicho (si falta la hora, usa null). DESCOMPÓN frases con varias cosas en VARIAS acciones: "pagué 40 de la cena con Marcos y me debe la mitad" => tx(gasto 40) + debt(meDeben, Marcos, 20). Para saving/habit usa el nombre tal cual lo diga (se buscará el más parecido). Si no hay ninguna acción clara, devuelve "actions":[] y una "reply" pidiendo el detalle que falte. Sé conciso.
=== CONTEXTO ACTUAL ===
${buildMemory(state)}` + aiLangRule("json");
    try {
      ...
      acts.forEach((a) => {
        ...
        } else if (a.type === "debt" && a.person && a.amount) {
          dispatch({ type: "add", col: "debts", item: { id: uid(), date: todayISO(), settled: false, dir: a.dir === "yoDebo" ? "yoDebo" : "meDeben", person: String(a.person), amount: Number(a.amount), note: a.note || "" } });
          done++; labels.push(`🤝 ${a.dir === "yoDebo" ? tr("Debo a") : tr("Me debe")} ${a.person}: ${money(Number(a.amount))}`);
        } else if (a.type === "saving" && a.amount) {
          const norm = (s) => String(s || "").toLowerCase().trim();
          const target = (state.savings || []).find((h) => norm(h.name) === norm(a.name)) || (state.savings || []).find((h) => norm(h.name).includes(norm(a.name)) || norm(a.name).includes(norm(h.name)));
          const entry = { id: uid(), date: todayISO(), amount: Number(a.amount) };
          if (target) {
            dispatch({ type: "update", col: "savings", id: target.id, patch: { log: [entry, ...(target.log || [])] } });
            done++; labels.push(`🐷 ${target.name}: ${Number(a.amount) >= 0 ? "+" : ""}${money(Number(a.amount))}`);
          } else if (a.name) {
            dispatch({ type: "add", col: "savings", item: { id: uid(), name: String(a.name), target: 0, deadline: "", log: [entry] } });
            done++; labels.push(`🐷 Nueva hucha "${a.name}": ${money(Number(a.amount))}`);
          }
      ...
      setReply({ text: j.reply || (done ? tr("Hecho.") : tr("No pillé una acción clara, dime algo más concreto.")), labels });
  ...
  return (
    <div style={{ ...card, padding: 12, marginTop: 12, border: "1px solid rgba(144,19,254,.5)", background: C.tintP, boxShadow: "0 1px 0 rgba(255,255,255,.07) inset, 0 6px 18px rgba(144,19,254,.14), 0 10px 26px rgba(0,0,0,.35)" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
        ...
        <span style={{ color: C.purple, fontWeight: 800, fontSize: 12, letterSpacing: 0.8, textTransform: "uppercase", flex: 1 }}>{tr("Dile a Brosin y lo apunta")}</span>
      </div>
...
function SleepCard({ state, dispatch, toast }) {
  ...
  const logHours = (h) => {
    ...
    setOpen(false);
    if (h < min && toast) toast("A descansar, bro", `Solo ${h} h ${d === t ? tr("esta noche") : tr("ese día")}. Tu cuerpo necesita más — cuídate.`);
  };
  ...
  return (
    <div style={{ ...card, padding: 16, marginBottom: 18, borderColor: low ? C.bad : C.line, background: low ? "rgba(255,92,92,.06)" : C.ink2 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
        ...
        <span style={{ color: C.textHi, fontWeight: 800, fontSize: 14.5, flex: 1 }}>{tr("Descanso")}</span>
      ...
      <div style={{ display: "flex", alignItems: "flex-end", gap: 14 }}>
        <div style={{ minWidth: 92 }}>
          ...
          <div style={{ color: C.textLo, fontSize: 11.5, marginTop: 3 }}>{lastIsToday ? "hoy" : last ? tr("última noche") : tr("sin registrar")}</div>
        </div>
        {/* 7-day bars */}
        <div style={{ flex: 1, display: "flex", alignItems: "flex-end", gap: 5, height: 52 }}>
          {bars.map((b, i) => (
            <button key={i} onClick={() => openFor(b.d)} title={tr("Tocar para registrar/editar ese día")} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 3, background: "none", border: "none", padding: 0, cursor: "pointer" }}>
      ...
      {open ? (
        <div style={{ marginTop: 12 }}>
          <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 8, flexWrap: "wrap" }}>
            <span style={{ color: C.textLo, fontSize: 12, fontWeight: 700 }}>{tr("Día:")}</span>
          ...
          <button onClick={() => setOpen(false)} style={{ ...iconBtn, width: "auto", paddingInline: 12, height: 34, marginTop: 10, gap: 6, fontSize: 12.5, color: C.textLo }}><X size={13} /> {tr("Cerrar")}</button>
        </div>
      ) : (
        <button onClick={() => openFor(todayISO())} style={{ ...iconBtn, width: "100%", height: 42, marginTop: 12, gap: 8, fontWeight: 800, fontSize: 13.5, background: C.ink3 }}>
          <Moon size={15} /> {lastIsToday ? tr("Editar sueño · registrar otro día") : tr("Registrar tu sueño")}
        </button>
      )}

      {/* Historial completo: las barras solo enseñan 7 días, pero se guarda todo desde siempre */}
      {log.length > 0 && (
        <button onClick={() => setHistOpen(!histOpen)} style={{ ...iconBtn, width: "100%", height: 34, marginTop: 8, gap: 6, fontWeight: 700, fontSize: 12, background: "none", color: C.textLo }}>
          {histOpen ? tr("Ocultar historial") : tri("Ver todo el historial ({n} noches)", { n: log.length })}
        </button>
      )}
      {histOpen && (
        <div style={{ marginTop: 8, maxHeight: 260, overflowY: "auto", display: "grid", gap: 6 }}>
          {[...log].sort((a, b) => b.date.localeCompare(a.date)).map((e) => (
            <div key={e.date} style={{ display: "flex", alignItems: "center", gap: 10, background: C.ink3, borderRadius: 10, padding: "8px 10px" }}>
              <span style={{ flex: 1, color: C.textHi, fontSize: 12.5, textTransform: "capitalize" }}>{fmtDate(e.date)}</span>
              <span style={{ color: e.hours < min ? C.bad : C.yellow, fontWeight: 800, fontSize: 13.5 }}>{e.hours} h</span>
              <button onClick={() => { setHistOpen(false); openFor(e.date); }} title={tr("Corregir este día")} style={{ ...iconBtn, width: 30, height: 30, color: C.textLo }}><Pencil size={13} /></button>
              <button onClick={() => dispatch({ type: "setCol", col: "sleepLog", items: log.filter((x) => x.date !== e.date) })} title={tr("Borrar este día")} style={{ ...iconBtn, width: 30, height: 30, color: C.textLo }}><Trash2 size={13} /></button>
            </div>
          ))}
        </div>
      )}
  ...
  return (
    <div style={{ ...card, padding: 16, background: C.tintY, border: "1px solid rgba(255,212,0,.25)", boxShadow: "0 1px 0 rgba(255,255,255,.06) inset, 0 8px 22px rgba(255,212,0,.08), 0 10px 26px rgba(0,0,0,.35)" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
        ...
        <span style={{ color: C.accentText, fontWeight: 800, fontSize: 12.5, letterSpacing: 1.6, textTransform: "uppercase", flex: 1 }}>{tr("El parte del día")}</span>
      ...
      {loading ? (
        ...
        <div>
          ...
          <button onClick={run} style={{ ...iconBtn, width: "auto", paddingInline: 14, gap: 6, fontWeight: 700, fontSize: 13 }}>
            <RefreshCcw size={14} /> {tr("Reintentar")}
          </button>
        </div>
      ) : out ? (
...
function AgendaScreen({ state, dispatch }) {
  ...
  // Cumpleaños de las personas -> eventos virtuales (no viven en state.events, no se editan aquí)
  const bdayEvents = (state.people || []).flatMap((p) => {
    if (!p.birthday) return [];
    const nb = nextBirthdayDate(p.birthday);
    if (!nb) return [];
    const iso = nb.getFullYear() + "-" + String(nb.getMonth() + 1).padStart(2, "0") + "-" + String(nb.getDate()).padStart(2, "0");
    return [{ id: "bday:" + p.id, title: tri("Cumpleaños de {n}", { n: p.name || tr("alguien") }), date: iso, time: "", type: "Cumpleaños", done: false, remind: false, virtual: true }];
  });
  const allEvents = [...state.events, ...bdayEvents];

  const sorted = [...allEvents].sort((a, b) => (a.date + (a.time || "")).localeCompare(b.date + (b.time || "")));
  const grouped = {};
  sorted.forEach((e) => {
    const k = e.date;
  ...
  return (
    <div style={{ padding: "20px 18px 0" }}>
      <Title kicker={tr("Tu tiempo")}>{tr("Agenda")}</Title>
      <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
        <Chip active={view === "lista"} onClick={() => setView("lista")}>{tr("Próximos")}</Chip>
        <Chip active={view === "mes"} onClick={() => setView("mes")}>{tr("Mes")}</Chip>
      </div>

      {view === "mes" ? (
        <MonthView events={allEvents} onPick={(e) => { if (!e.virtual) openEdit(e); }} onAddDay={openNewOn} />
      ) : keys.length === 0 ? (
        <EmptyState face="sleep" title={tr("Agenda en blanco")} sub={tr("Añade tu primer evento o recordatorio y Brosin te avisa cuando toque.")} cta={tr("Crear evento")} onCta={openNew} />
      ) : (
            ...
            return (
              <div key={k}>
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                  ...
                  {d < 0 && <span style={{ color: C.textLo, fontSize: 11 }}>{tr("pasado")}</span>}
                </div>
                <div style={{ display: "grid", gap: 10 }}>
                  {grouped[k].sort((a, b) => (a.time || "").localeCompare(b.time || "")).map((e) => (
                    <div key={e.id} onClick={() => { if (!e.virtual) openEdit(e); }} style={{ cursor: e.virtual ? "default" : "pointer" }}>
                      <EventRow e={e} onToggle={e.virtual ? null : () => dispatch({ type: "update", col: "events", id: e.id, patch: { done: !e.done } })} />
                    </div>
                  ))}
                </div>
              </div>
      ...
      <div style={{ height: 122 }} />
      <FAB onClick={openNew} label={tr("Evento")} />

      <Sheet open={sheet} onClose={closeSheet} title={edit ? tr("Editar evento") : tr("Nuevo evento")}>
...
function EventForm({ initial, onSave, onDelete }) {
  ...
  const types = ["Cita", "Recordatorio", "Tarea", "Evento", "Pago", "Cumpleaños", "Entrenamiento", "Reunión", "Trabajo", "Viaje", "Médico", "Ocio"];
  const leadOpts = [
  ...
  return (
    <div>
      <Field label={tr("¿Qué es?")}><input style={inputStyle} value={title} onChange={(e) => setTitle(e.target.value)} placeholder={tr("Título del evento")} autoFocus /></Field>
      <div style={{ display: "flex", gap: 10 }}>
        <div style={{ flex: 1 }}><Field label={tr("Fecha")}><input type="date" style={inputStyle} value={date} onChange={(e) => setDate(e.target.value)} /></Field></div>
        <div style={{ flex: 1 }}><Field label={tr("Hora")}><input type="time" style={inputStyle} value={time} onChange={(e) => setTime(e.target.value)} /></Field></div>
      </div>
      <Field label={tr("Tipo")}>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {types.map((t) => <Chip key={t} active={type === t} onClick={() => setType(type === t ? "" : t)}>{tr(t)}</Chip>)}
        </div>
      </Field>
      ...
      <button onClick={() => setRemind((r) => !r)} style={{
        ...
        <BellRing size={18} color={remind ? C.yellow : C.textLo} />
        <span style={{ flex: 1, textAlign: "left", color: C.textHi, fontWeight: 700, fontSize: 14 }}>{tr("Avisos de este evento")}</span>
        <span style={{ width: 44, height: 26, borderRadius: 999, background: remind ? C.yellow : C.line, position: "relative", transition: "all .2s" }}>
          <span style={{ position: "absolute", top: 3, left: remind ? 21 : 3, width: 20, height: 20, borderRadius: 999, background: remind ? C.ink : "#777", transition: "all .2s" }} />
        </span>
      </button>
      {remind && (
        <>
          <Field label={tr("① Aviso único (una vez, antes del evento)")}>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {leadOpts.map((o) => <Chip key={String(o.v)} active={lead === o.v} onClick={() => setLead(o.v)}>{tr(o.l)}</Chip>)}
            </div>
          </Field>
          <Field label={tr("② Insistir (repetido hasta que lo marques hecho)")}>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {nagOpts.map((o) => <Chip key={o.v} active={nag === o.v} onClick={() => setNag(o.v)}>{tr(o.l)}</Chip>)}
            </div>
          </Field>
        </>
      )}
      <Field label={tr("Foto (opcional)")}><PhotoPicker value={photo} onChange={setPhoto} /></Field>
      <Field label={tr("Notas")}><textarea style={{ ...inputStyle, minHeight: 70, resize: "vertical" }} value={notes} onChange={(e) => setNotes(e.target.value)} placeholder={tr("Opcional")} /></Field>
      <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
        {onDelete && <button onClick={onDelete} style={{ ...iconBtn, width: 54, height: 54, color: C.bad, borderRadius: 16 }}><Trash2 size={20} /></button>}
        <div style={{ flex: 1 }}>
...
function MonthView({ events, onPick, onAddDay }) {
  ...
  const nextName = new Date(nextY, nextM, 1).toLocaleDateString(LOCALE, { month: "long" });
    ...
    <div style={{ display: "grid", gap: 12 }}>
      <div style={{ ...card, padding: 14 }}>
        ...
        {sel && (
          <div style={{ marginTop: 12, borderTop: `1px solid ${C.line}`, paddingTop: 12 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
              ...
              <button onClick={() => onAddDay && onAddDay(sel)} style={{ ...iconBtn, width: "auto", paddingInline: 12, height: 32, gap: 6, fontSize: 12.5, fontWeight: 800, background: C.yellow, color: C.ink, borderColor: C.yellow }}><Plus size={14} color={C.ink} strokeWidth={3} /> {tr("Añadir")}</button>
            </div>
            {selEvs.length === 0 ? (
              <div style={{ color: C.textLo, fontSize: 12.5 }}>{tr("Nada este día. Pulsa \"Añadir\" para crear algo.")}</div>
            ) : (
        ...
        <div style={{ display: "flex", gap: 14, justifyContent: "center", marginTop: 12, flexWrap: "wrap" }}>
          <span style={{ color: C.textLo, fontSize: 10.5, display: "flex", alignItems: "center", gap: 5 }}><span style={{ width: 12, height: 12, borderRadius: 4, background: C.yellow }} /> {tr("hoy")}</span>
          <span style={{ color: C.textLo, fontSize: 10.5, display: "flex", alignItems: "center", gap: 5 }}><span style={{ width: 12, height: 12, borderRadius: 6, border: `2px solid ${C.yellow}` }} /> {tr("con evento")}</span>
          <span style={{ color: C.textLo, fontSize: 10.5, textDecoration: "line-through" }}>{tr("día pasado")}</span>
        </div>
      </div>
...
/* ============================================================
   ...
   ============================================================ */
/* ============================================================
   FIJOS — gastos e ingresos que se repiten cada mes o cada semana
   ============================================================ */
const toMonthly = (amount, freq) => Number(amount || 0) * (freq === "semanal" ? 52 / 12 : 1);

function FixedPanel({ state, dispatch }) {
  const [sheet, setSheet] = useState(null); // null | "new" | objeto a editar
  const list = state.fixeds || [];
  const sum = (kind, freq) => list.filter((f) => f.kind === kind && f.freq === freq).reduce((s, f) => s + Number(f.amount || 0), 0);
  const gMesRaw = sum("gasto", "mensual"), gSem = sum("gasto", "semanal");
  const iMesRaw = sum("ingreso", "mensual"), iSem = sum("ingreso", "semanal");
  const gMes = gMesRaw + toMonthly(gSem, "semanal");
  const iMes = iMesRaw + toMonthly(iSem, "semanal");
  const neto = iMes - gMes;
  const save = (d) => {
    if (sheet && sheet !== "new") dispatch({ type: "update", col: "fixeds", id: sheet.id, patch: d });
    else dispatch({ type: "add", col: "fixeds", item: { id: uid(), ...d } });
    setSheet(null);
  };
  const Stat = ({ label, value, color }) => (
    <div style={{ ...card, padding: 12 }}>
      <div style={{ color: C.textLo, fontSize: 10.5, fontWeight: 700 }}>{label}</div>
      <div style={{ color, fontWeight: 900, fontSize: 17, marginTop: 3 }}>{money(value)}</div>
    </div>
  );
  const gastos = list.filter((f) => f.kind === "gasto");
  const ingresos = list.filter((f) => f.kind === "ingreso");
  return (
    <div>
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 12 }}>
        <Stat label={tr("Gastos fijos mensuales")} value={gMesRaw} color={C.bad} />
        <Stat label={tr("Ganancias fijas mensuales")} value={iMesRaw} color={C.good} />
        <Stat label={tr("Gastos fijos semanales")} value={gSem} color={C.bad} />
        <Stat label={tr("Ganancias fijas semanales")} value={iSem} color={C.good} />
      </div>

      <div style={{ ...card, padding: 16, marginBottom: 16 }}>
        <div style={{ color: C.textLo, fontSize: 11.5, fontWeight: 700 }}>{tr("Te queda al mes, contando todo lo fijo")}</div>
        <div style={{ color: neto >= 0 ? C.good : C.bad, fontWeight: 900, fontSize: 28, marginTop: 4 }}>{money(neto)}</div>
        <div style={{ color: C.textLo, fontSize: 11, marginTop: 4 }}>
          {tri("Entran {i} y salen {g} al mes. Lo semanal cuenta ×4,33.", { i: money(iMes), g: money(gMes) })}
        </div>
      </div>

      {list.length === 0 ? (
        <EmptyState face="sleep" title={tr("Sin fijos todavía")} sub={tr("Mete el alquiler, la nómina, el gimnasio o lo que se repita, y sabrás con cuánto cuentas de verdad cada mes.")} cta={tr("Añadir fijo")} onCta={() => setSheet("new")} />
      ) : (
        <div style={{ display: "grid", gap: 16 }}>
          {ingresos.length > 0 && (
            <div>
              <div style={{ color: C.good, fontWeight: 900, fontSize: 13, marginBottom: 8 }}>{tr("Entra")}</div>
              <div style={{ display: "grid", gap: 10 }}>
                {ingresos.map((f) => <FixedCard key={f.id} f={f} onEdit={() => setSheet(f)} onDel={() => dispatch({ type: "remove", col: "fixeds", id: f.id })} />)}
              </div>
            </div>
          )}
          {gastos.length > 0 && (
            <div>
              <div style={{ color: C.bad, fontWeight: 900, fontSize: 13, marginBottom: 8 }}>{tr("Sale")}</div>
              <div style={{ display: "grid", gap: 10 }}>
                {gastos.map((f) => <FixedCard key={f.id} f={f} onEdit={() => setSheet(f)} onDel={() => dispatch({ type: "remove", col: "fixeds", id: f.id })} />)}
              </div>
            </div>
          )}
        </div>
      ...
      <div style={{ height: 122 }} />
      <FAB onClick={() => setSheet("new")} label={tr("Fijo")} />
      <Sheet open={!!sheet} onClose={() => setSheet(null)} title={sheet && sheet !== "new" ? tr("Editar fijo") : tr("Nuevo fijo")}>
        <FixedForm initial={sheet && sheet !== "new" ? sheet : null} onSave={save} />
      </Sheet>
    </div>
  );
}

function FixedCard({ f, onEdit, onDel }) {
  const esGasto = f.kind === "gasto";
  return (
    <div style={{ ...card, padding: 13 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        <div style={{ width: 36, height: 36, borderRadius: 11, background: esGasto ? "rgba(255,92,92,.15)" : "rgba(61,220,132,.15)", display: "grid", placeItems: "center", flexShrink: 0 }}>
          {esGasto ? <ArrowDown size={17} color={C.bad} /> : <ArrowUp size={17} color={C.good} />}
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ color: C.textHi, fontWeight: 700, fontSize: 14.5, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{f.name}</div>
          <div style={{ color: C.textLo, fontSize: 12 }}>
            {f.freq === "semanal" ? tr("Cada semana") : tr("Cada mes")}
            {f.freq === "semanal" ? ` · ${tri("{n} al mes", { n: money(toMonthly(f.amount, "semanal")) })}` : ""}
          </div>
          <CreatedAt at={f.createdAt} />
        </div>
        <div style={{ color: esGasto ? C.bad : C.good, fontWeight: 900, fontSize: 16, flexShrink: 0 }}>{money(f.amount)}</div>
      </div>
      <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
        <button onClick={onEdit} style={miniBtn(C.ink3, C.textHi)}>{tr("Editar")}</button>
        <button onClick={onDel} style={{ ...iconBtn, width: 36, height: 36, color: C.textLo }}><Trash2 size={14} /></button>
      </div>
    </div>
  );
}

function FixedForm({ initial, onSave }) {
  const [name, setName] = useState(initial?.name || "");
  const [amount, setAmount] = useState(initial?.amount != null ? String(initial.amount) : "");
  const [kind, setKind] = useState(initial?.kind || "gasto");
  const [freq, setFreq] = useState(initial?.freq || "mensual");
  const ok = name.trim() && Number(amount) > 0;
  return (
    <div>
      <Field label={tr("¿Qué es?")}>
        <input style={inputStyle} value={name} onChange={(e) => setName(e.target.value)} placeholder={tr("Ej. Alquiler, nómina, gimnasio…")} autoFocus />
      </Field>
      <Field label={tr("Cantidad")}>
        <input type="number" inputMode="decimal" style={inputStyle} value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="€" />
      </Field>
      <Field label={tr("¿Entra o sale?")}>
        <div style={{ display: "flex", gap: 8 }}>
          <Chip active={kind === "gasto"} onClick={() => setKind("gasto")}>{tr("Sale (gasto)")}</Chip>
          <Chip active={kind === "ingreso"} onClick={() => setKind("ingreso")}>{tr("Entra (ganancia)")}</Chip>
        </div>
      </Field>
      <Field label={tr("¿Cada cuánto?")}>
        <div style={{ display: "flex", gap: 8 }}>
          <Chip active={freq === "mensual"} onClick={() => setFreq("mensual")}>{tr("Cada mes")}</Chip>
          <Chip active={freq === "semanal"} onClick={() => setFreq("semanal")}>{tr("Cada semana")}</Chip>
        </div>
      </Field>
      <PrimaryBtn full color={kind === "gasto" ? C.yellow : C.good} onClick={() => ok && onSave({ name: name.trim(), amount: Number(amount), kind, freq })}>
        <Check size={18} strokeWidth={3} /> {tr("Guardar")}
      </PrimaryBtn>
    </div>
  );
...
/* ============================================================
   ...
   ============================================================ */
const GASTO_TIPOS = [
  { k: "inversion", n: "Inversión inicial", d: "Se paga una vez: la máquina, las herramientas." },
...
function MoneyScreen({ state, dispatch, tab, setTab, toast, ai }) {
  ...
  // Lo vendido deja de contar como cartera: pasa a "ganancia realizada".
  const invActivas = state.investments.filter((i) => !i.sold);
  const invVendidas = state.investments.filter((i) => i.sold);
  const invTotal = invActivas.reduce((s, i) => s + curVal(i), 0);
  const invCost = invActivas.reduce((s, i) => s + Number(i.invested || 0), 0);
  const invPnl = invTotal - invCost;
  const invRealizado = invVendidas.reduce((s, i) => s + (Number(i.soldFor || 0) - Number(i.invested || 0)), 0);

  return (
    <div style={{ padding: "20px 18px 0" }}>
      <Title kicker={tr("Tus cuentas")}>{tr("Dinero")}</Title>
      ...
      <div style={{
        ...
        <div style={{ color: "rgba(0,0,0,.62)", fontSize: 11.5, fontWeight: 800, letterSpacing: 2, textTransform: "uppercase" }}>{tr("Balance total")}</div>
      ...
      {tab === "graficas" && (
        state.tx.length === 0 ? (
          <EmptyState face="neutral" title={tr("Sin datos para graficar")} sub={tr("Apunta algunos movimientos y aquí verás tu dinero en gráficas.")} cta={tr("Añadir movimiento")} onCta={() => setSheet("tx")} />
        ) : (
          <div style={{ display: "grid", gap: 12 }}>
            <MonthCloseCard state={state} ai={ai} />
            ...
            {state.investments.length > 0 && (
              <ExpandableChart onExpand={() => setChartDetail("inversiones")}>
                <HBars icon={Landmark} title={tr("Inversiones (valor actual)")} sub={money(invTotal)} accent={C.yellow}
      ...
      {tab === "movimientos" && (
        <Section
          ...
          emptyT={tr("Sin movimientos")}
          emptyS={tr("Apunta ingresos y gastos para ver tu balance real.")}
          onAdd={() => setSheet("tx")}
        >
          {[...state.tx].sort((a, b) => (b.date || "").localeCompare(a.date || "")).map((x) => (
            <div key={x.id} style={{ ...card, padding: "13px 14px", display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ color: C.textHi, fontWeight: 700, fontSize: 14.5 }}>{x.note || (x.type === "ingreso" ? tr("Ingreso") : tr("Gasto"))}</div>
      ...
      {tab === "deudas" && (
        <Section
          ...
          emptyT={tr("Cuentas claras")}
          emptyS={tr("Registra quién te debe y a quién debes tú.")}
          onAdd={() => setSheet("debt")}
        >
          {state.debts.map((d) => (
      ...
      {tab === "inversiones" && (
        <Section
          ...
          emptyT={tr("Aún sin inversiones")}
          emptyS={tr("Sigue tus inversiones y mira cómo evolucionan.")}
          onAdd={() => setSheet("inv")}
          header={
            state.investments.length > 0 && (
              <div style={{ ...card, padding: 14, marginBottom: 12, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <div style={{ textAlign: "right" }}>
                  <div style={{ color: C.textLo, fontSize: 11, fontWeight: 700, letterSpacing: 1.2 }}>{tr("SIN VENDER")}</div>
                  ...
                  {invVendidas.length > 0 && (
                    <>
                      <div style={{ color: C.textLo, fontSize: 10.5, fontWeight: 700, letterSpacing: 1.2, marginTop: 6 }}>{tr("YA GANADO")}</div>
                      <div style={{ color: invRealizado >= 0 ? C.good : C.bad, fontWeight: 900, fontSize: 15, fontVariantNumeric: "tabular-nums" }}>{invRealizado >= 0 ? "+" : ""}{money(invRealizado)}</div>
                    </>
                  )}
                </div>
              </div>
        ...
        >
          {state.investments.map((i) => {
            ...
            return (
              <button key={i.id} onClick={() => { setEditInv(i); setSheet("inv"); }} style={{ ...card, padding: 14, width: "100%", textAlign: "left", cursor: "pointer", opacity: i.sold ? 0.6 : 1, borderColor: i.sold ? C.good : C.line }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
      ...
      {(tab === "movimientos" || tab === "deudas" || tab === "inversiones") && (
        <FAB onClick={() => { setEditInv(null); setEditDebt(null); setSheet(tab === "deudas" ? "debt" : tab === "inversiones" ? "inv" : "tx"); }} label={tab === "deudas" ? tr("Deuda") : tab === "inversiones" ? tr("Inversión") : tr("Movimiento")} />
      )}

      <Sheet open={!!sheet} onClose={() => { setSheet(null); setEditInv(null); setEditDebt(null); }} title={sheet === "debt" ? (editDebt ? tr("Editar deuda") : tr("Nueva deuda")) : sheet === "inv" ? (editInv ? tr("Editar inversión") : tr("Nueva inversión")) : tr("Nuevo movimiento")}>
        ...
        {sheet === "inv" && <InvForm initial={editInv} livePrices={invPrices}
          ...
          onSell={editInv ? (v) => {
            dispatch({ type: "update", col: "investments", id: editInv.id, patch: { sold: true, soldAt: v.fecha, soldFor: v.precio } });
            const g = v.precio - Number(editInv.invested || 0);
            let apuntado = "";
            if (v.registro === "total") {
              dispatch({ type: "add", col: "tx", item: { id: uid(), type: "ingreso", amount: v.precio, note: "Venta: " + (editInv.name || ""), category: "Negocio", date: v.fecha } });
              apuntado = " · " + money(v.precio) + " en Movimientos";
            } else if (v.registro === "ganancia" && g !== 0) {
              // solo el beneficio (o la pérdida): es lo correcto si la compra nunca se apuntó
              dispatch({ type: "add", col: "tx", item: { id: uid(), type: g > 0 ? "ingreso" : "gasto", amount: Math.abs(g), note: (g > 0 ? "Ganancia: " : "Pérdida: ") + (editInv.name || ""), category: "Negocio", date: v.fecha } });
              apuntado = " · " + money(Math.abs(g)) + " en Movimientos";
            }
            toast && toast(g >= 0 ? "Venta apuntada ✓" : "Venta apuntada", `${editInv.name}: ${g >= 0 ? "+" : ""}${money(g)}${apuntado}`);
            setSheet(null); setEditInv(null);
          } : null}
          onDelete={editInv ? () => { dispatch({ type: "remove", col: "investments", id: editInv.id }); setSheet(null); setEditInv(null); } : null} />}
      </Sheet>

      <Sheet open={!!chartDetail} onClose={() => setChartDetail(null)} title={chartDetail === "meses" ? "Por meses" : chartDetail === "categorias" ? "Por categorías" : chartDetail === "balance" ? tr("Movimiento a movimiento") : tr("Tus inversiones")}>
...
function ChartDetailSheet({ which, tx, investments }) {
  if (which === "meses") {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      rows.push({ label: d.toLocaleDateString(LOCALE, { month: "long", year: "2-digit" }), ing, gas, net: ing - gas });
    }
...
function MarketsPanel({ state, dispatch, toast }) {
  ...
  const checkAlert = (asset, v) => {
    ...
    if (hit) {
      ...
      toast && toast(hit.dir === "up" ? tr("Alerta de mercado ↑") : tr("Alerta de mercado ↓"), hit.txt);
    }
  };
    ...
    <div>
      <div style={{ display: "flex", gap: 10, marginBottom: 12 }}>
        <button onClick={refreshAll} disabled={busy} style={{ ...iconBtn, width: "auto", flex: 1, height: 44, gap: 8, fontWeight: 800, fontSize: 14, background: C.ink2 }}>
          ...
          {busy ? tr("Actualizando…") : tr("Actualizar precios en vivo")}
        </button>
      ...
      {updated === "ok" && <div style={{ color: C.good, fontSize: 12, marginBottom: 10 }}>{tr("Precios actualizados en vivo ✓")}</div>}
      {updated === "err" && <div style={{ color: C.textLo, fontSize: 12, marginBottom: 10 }}>{tr("No pude conectar a algún feed. Cripto/oro/plata/forex son en vivo; el resto, a mano o con IA.")}</div>}

      {state.markets.length === 0 ? (
        <EmptyState face="happy" title={tr("Tu watchlist vacía")} sub={tr("Añade activos: Bitcoin, oro, plata, gasolina, S&P500, forex… y sigue cómo suben o bajan.")} cta={tr("Añadir activo")} onCta={() => setSheet(true)} />
      ) : (
        <div className="bstagger" style={{ display: "grid", gap: 10 }}>
      ...
      <div style={{ ...card, padding: 12, marginTop: 12, background: C.well, fontSize: 11.5, color: C.textLo, lineHeight: 1.5 }}>
        <b style={{ color: C.textHi }}>{tr("En vivo (reales):")}</b> {tr("cripto, oro, plata y forex (EUR/USD).")} <b style={{ color: C.textHi }}>{tr("A mano o con IA:")}</b> gasolina, S&P 500 y otros índices (no hay feed gratuito abierto fiable). Toca 🔔 en un activo para ponerle una alerta de subida/bajada.
      </div>

      <Sheet open={sheet} onClose={() => setSheet(false)} title={tr("Añadir activo")}>
        <div style={{ display: "grid", gap: 8 }}>
          {PRESET_ASSETS.map((p) => {
            ...
            return (
              <button key={p.sym} onClick={() => addAsset(p)} disabled={added} style={{
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ color: C.textLo, fontSize: 12 }}>{p.sym} · {p.type === "crypto" ? tr("en vivo") : tr("manual / IA")}</div>
                </div>
    ...
    <div style={{ ...card, padding: 14 }}>
      ...
      {alertOpen ? (
        <div style={{ marginTop: 10, borderTop: `1px solid ${C.line}`, paddingTop: 10 }}>
          <div style={{ color: C.textLo, fontSize: 11.5, marginBottom: 8 }}>{tr("Avísame cuando el precio…")}</div>
          <div style={{ display: "flex", gap: 8 }}>
            <div style={{ flex: 1 }}>
              <div style={{ color: C.good, fontSize: 11, fontWeight: 700, marginBottom: 4 }}>{tr("≥ suba a (€)")}</div>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ color: C.bad, fontSize: 11, fontWeight: 700, marginBottom: 4 }}>{tr("≤ baje a (€)")}</div>
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
            <button onClick={saveAlert} style={miniBtn(C.yellow, C.ink)}>{tr("Guardar alerta")}</button>
            <button onClick={() => { onSetAlert(a, null); setAbove(""); setBelow(""); setAlertOpen(false); }} style={miniBtn(C.ink3, C.textLo)}>{tr("Quitar")}</button>
          </div>
          <div style={{ color: C.textLo, fontSize: 10.5, marginTop: 8, lineHeight: 1.4 }}>{tr("Te avisa al actualizar precios (con la app abierta). Útil para entrar a tu app externa y vender o mover.")}</div>
        </div>
      ) : editing ? (
        <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
          <input type="number" inputMode="decimal" autoFocus style={{ ...inputStyle, padding: "9px 12px" }} value={val} onChange={(e) => setVal(e.target.value)} placeholder={tr("Precio en €")} />
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
          {!live && <button onClick={() => setEditing(true)} style={miniBtn(C.ink3, C.textHi)}>{tr("A mano")}</button>}
          {!live && <button onClick={estimate} disabled={aiBusy} style={miniBtn("rgba(144,19,254,.18)", C.purple)}>{aiBusy ? "Estimando…" : "IA"}</button>}
          <button onClick={() => setAlertOpen(true)} style={miniBtn(hasAlert ? "rgba(144,19,254,.18)" : C.ink3, hasAlert ? C.purple : C.textHi)}>{tr("🔔 Alerta")}</button>
...
/* ---------- Cierre de mes (resumen financiero mensual) ---------- */
const ymOf = (iso) => String(iso || "").slice(0, 7);
const prevYm = (ym) => { const p = ym.split("-").map(Number); const d = new Date(p[0], p[1] - 2, 1); return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}`; };
function monthStats(state, ym) {
  const all = state.tx || [];
  const tx = all.filter((x) => ymOf(x.date) === ym);
  const prev = all.filter((x) => ymOf(x.date) === prevYm(ym));
  const sum = (a, t) => a.filter((x) => x.type === t).reduce((s, x) => s + Number(x.amount || 0), 0);
  const ing = sum(tx, "ingreso"), gas = sum(tx, "gasto");
  const pIng = sum(prev, "ingreso"), pGas = sum(prev, "gasto");
  const cats = {}; tx.filter((x) => x.type === "gasto").forEach((x) => { const k = x.category || "General"; cats[k] = (cats[k] || 0) + Number(x.amount || 0); });
  const pCats = {}; prev.filter((x) => x.type === "gasto").forEach((x) => { const k = x.category || "General"; pCats[k] = (pCats[k] || 0) + Number(x.amount || 0); });
  const topCats = Object.entries(cats).sort((a, b) => b[1] - a[1]).slice(0, 3).map((e) => ({ k: e[0], v: e[1], prev: pCats[e[0]] || 0 }));
  // recurrentes: misma nota normalizada, o misma categoría con importe ~igual (±10%), presente también el mes anterior
  const norm = (s) => String(s || "").toLowerCase().trim();
  const rec = [];
  tx.filter((x) => x.type === "gasto").forEach((x) => {
    const hit = prev.find((p) => p.type === "gasto" && ((norm(x.note) && norm(p.note) === norm(x.note)) || (p.category === x.category && Math.abs(Number(p.amount) - Number(x.amount)) <= Math.max(0.5, Number(x.amount) * 0.1))));
    if (hit && Number(x.amount) > 0 && !rec.some((r) => norm(r.note) === norm(x.note || x.category))) rec.push({ note: x.note || x.category || "—", amount: Number(x.amount) });
  });
  rec.sort((a, b) => b.amount - a.amount);
  const byDay = {}; tx.filter((x) => x.type === "gasto").forEach((x) => { const d = String(x.date).slice(0, 10); byDay[d] = (byDay[d] || 0) + Number(x.amount || 0); });
  const worstArr = Object.entries(byDay).sort((a, b) => b[1] - a[1]);
  const parts = ym.split("-").map(Number);
  const daysInMonth = new Date(parts[0], parts[1], 0).getDate();
  const savedM = (state.savings || []).reduce((s, h) => s + (h.log || []).filter((e) => ymOf(e.date) === ym).reduce((a, e) => a + Number(e.amount || 0), 0), 0);
  let activeDays = 0;
  for (let d = 1; d <= daysInMonth; d++) { if (wasActiveOn(state, `${ym}-${String(d).padStart(2, "0")}`)) activeDays++; }
  return {
    ym, ing, gas, net: ing - gas, pIng, pGas, pNet: pIng - pGas,
    topCats, recurring: rec.slice(0, 5), recurringTotal: rec.reduce((s, r) => s + r.amount, 0),
    worstDay: worstArr.length ? { date: worstArr[0][0], amount: worstArr[0][1] } : null,
    dailyAvg: gas / daysInMonth, savedM, activeDays, daysInMonth, txCount: tx.length,
  };
}

/* ---------- Grupos de gastos (repartir cuentas entre amigos) ----------
   Saldo por miembro = lo que pagó − su parte de cada gasto ± pagos hechos.
   simplifyDebts: transferencias mínimas (greedy mayor deudor → mayor acreedor). */
const groupBalances = (g) => {
  const bal = {};
  (g.members || []).forEach((m) => { bal[m.id] = 0; });
  (g.expenses || []).forEach((e) => {
    const ids = (e.split && e.split.length ? e.split : (g.members || []).map((m) => m.id)).filter((id) => bal[id] != null);
    if (!ids.length) return;
    const amt = Number(e.amount || 0);
    const share = amt / ids.length;
    if (bal[e.paidBy] != null) bal[e.paidBy] += amt;
    ids.forEach((id) => { bal[id] -= share; });
  });
  (g.settled || []).forEach((s) => {
    const a = Number(s.amount || 0);
    if (bal[s.from] != null) bal[s.from] += a;
    if (bal[s.to] != null) bal[s.to] -= a;
  });
  return bal;
};
const simplifyDebts = (bal) => {
  const EPS = 0.005;
  const debt = Object.entries(bal).filter(([, v]) => v < -EPS).map(([id, v]) => ({ id, v: -v })).sort((a, b) => b.v - a.v);
  const cred = Object.entries(bal).filter(([, v]) => v > EPS).map(([id, v]) => ({ id, v })).sort((a, b) => b.v - a.v);
  const out = [];
  let i = 0, j = 0;
  while (i < debt.length && j < cred.length) {
    const x = Math.min(debt[i].v, cred[j].v);
    out.push({ from: debt[i].id, to: cred[j].id, amount: Math.round(x * 100) / 100 });
    debt[i].v -= x; cred[j].v -= x;
    if (debt[i].v <= EPS) i++;
    if (cred[j].v <= EPS) j++;
  }
  return out.filter((t) => t.amount >= 0.01);
};
const groupTotal = (g) => (g.expenses || []).reduce((s, e) => s + Number(e.amount || 0), 0);
...
function SharedInvPanel({ state, dispatch }) {
  ...
  return (
    <div>
      {list.length === 0 ? (
        <EmptyState face="happy" title={tr("Inversión compartida")} sub={tr("Invierte con alguien y controla el bote al segundo: mete y saca dinero cuando quieras, apunta ganancias o pérdidas, reparte a cada socio y mira el resultado al momento.")} cta={tr("Nueva compartida")} onCta={() => setSheet("new")} />
      ) : (
        <>
          <div style={{ ...card, padding: 16, marginBottom: 12, background: C.tintP, border: "1px solid rgba(144,19,254,.35)", boxShadow: "0 1px 0 rgba(255,255,255,.07) inset, 0 8px 22px rgba(144,19,254,.14), 0 10px 26px rgba(0,0,0,.35)" }}>
            <div style={{ color: C.textLo, fontSize: 11.5, fontWeight: 700, letterSpacing: 1.6, textTransform: "uppercase" }}>{tr("En los botes ahora")}</div>
      ...
      <div style={{ height: 122 }} />
      <FAB onClick={() => setSheet("new")} label={tr("Compartida")} />
      <Sheet open={sheet === "new"} onClose={() => setSheet(null)} title={tr("Nueva inversión compartida")}>
        <SharedInvForm me={state.profile.name} onSave={addNew} />
      </Sheet>
    </div>
  );
}
/* Beneficio por periodo. Antes solo había el acumulado de toda la vida. */
const periodStart = (key) => {
  const d = new Date(); d.setHours(0, 0, 0, 0);
  if (key === "semana") { const wd = (d.getDay() + 6) % 7; d.setDate(d.getDate() - wd); return d; }
  if (key === "mes") return new Date(d.getFullYear(), d.getMonth(), 1);
  if (key === "trimestre") return new Date(d.getFullYear(), Math.floor(d.getMonth() / 3) * 3, 1);
  if (key === "semestre") return new Date(d.getFullYear(), d.getMonth() < 6 ? 0 : 6, 1);
  return new Date(d.getFullYear(), 0, 1);
};
const benefitSince = (log, from) => (log || []).reduce((s, e) => {
  const t = new Date(String(e.date || "").slice(0, 10) + "T00:00:00");
  if (isNaN(t.getTime()) || t < from) return s;
  const a = Number(e.amount || 0);
  if (e.kind === "ganancia") return s + a;
  if (e.kind === "perdida") return s - a;
  return s;
}, 0);
const PERIODS = [
  { k: "semana", l: "SEMANA" },
...
function PeriodBreakdown({ log }) {
  const [open, setOpen] = useState(false);
  const vals = PERIODS.map((p) => ({ ...p, v: benefitSince(log, periodStart(p.k)) }));
  const algo = vals.some((x) => x.v !== 0);
  return (
    <div style={{ marginTop: 8 }}>
      <button onClick={() => setOpen(!open)} style={{ ...iconBtn, width: "100%", height: 34, gap: 8, fontWeight: 800, fontSize: 12, background: C.ink3, color: C.textLo }}>
        {open ? tr("Ocultar beneficios por periodo") : tr("Beneficios por periodo")}
      </button>
      {open && (
        <>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginTop: 8 }}>
            {vals.map((x) => <MiniStat key={x.k} label={tr(x.l)} val={`${x.v >= 0 ? "+" : ""}${money(x.v)}`} col={x.v > 0 ? C.good : x.v < 0 ? C.bad : C.textLo} />)}
          </div>
          {!algo && <div style={{ color: C.textLo, fontSize: 11, marginTop: 6 }}>{tr("Todavía no hay ganancias ni pérdidas apuntadas en estos periodos.")}</div>}
        </>
      )}
    </div>
  );
...
function SharedInvCard({ si, dispatch }) {
  ...
  return (
    <div style={{ ...card, padding: 16 }}>
      ...
      {/* stats */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 8, marginTop: 8 }}>
        <MiniStat label={tr("METIDO")} val={money(t.aportado)} col={C.textHi} />
        <MiniStat label={tr("SACADO")} val={money(t.retirado)} col={C.textHi} />
        <MiniStat label={tr("BENEFICIO")} val={`${t.beneficio >= 0 ? "+" : ""}${money(t.beneficio)}`} col={t.beneficio >= 0 ? C.good : C.bad} />
      </div>

      <PeriodBreakdown log={log} />

      {/* quick meter / sacar — sin abrir otra pestaña */}
      <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
        <input type="number" inputMode="decimal" style={{ ...inputStyle, padding: "10px 12px" }} value={quick} onChange={(e) => setQuick(e.target.value)} placeholder={tr("Importe €")} />
        <button onClick={() => quickMove("meter")} style={miniBtn(C.good, C.ink)}>{tr("Meter")}</button>
        <button onClick={() => quickMove("sacar")} style={miniBtn(C.ink3, C.textHi)}>{tr("Sacar")}</button>
      </div>
      ...
      {more ? (
        ...
        <button onClick={() => setMore(true)} style={{ ...iconBtn, width: "100%", height: 38, marginTop: 8, gap: 8, fontWeight: 800, fontSize: 12.5, background: C.ink3 }}><Plus size={15} /> {tr("Ganancia · pérdida · reparto")}</button>
      )}
        ...
        <div style={{ marginTop: 12, display: "grid", gap: 6 }}>
          {partners.map((p) => (
            <div key={p.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", fontSize: 13 }}>
              ...
              <span style={{ color: C.textLo }}>{tr("se ha llevado")} <b style={{ color: C.yellow }}>{money(perPerson(p.name))}</b></span>
            </div>
          ))}
        </div>
      )}
      <button onClick={() => setEditPartners((v) => !v)} style={{ background: "none", border: "none", color: C.textLo, fontSize: 12, fontWeight: 700, cursor: "pointer", marginTop: 8, padding: 0, display: "flex", alignItems: "center", gap: 4 }}><Settings size={12} /> {tr("Editar socios")}</button>
      ...
      {open && (
        <div style={{ display: "grid", gap: 6, marginTop: 10 }}>
          {log.length === 0 && <div style={{ color: C.textLo, fontSize: 12.5, textAlign: "center", padding: "8px 0" }}>{tr("Sin movimientos todavía. Mete dinero para empezar.")}</div>}
  ...
  if (edit) {
    return (
      <div style={{ ...card, padding: 10, background: C.well, display: "grid", gap: 8 }}>
        ...
        <input type="number" inputMode="decimal" autoFocus style={{ ...inputStyle, padding: "9px 12px" }} value={amt} onChange={(ev) => setAmt(ev.target.value)} placeholder={tr("Importe €")} />
        {kind === "reparto" && (
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
            {whoOptions.map((w) => <Chip key={w} active={who === w} onClick={() => setWho(w)}>{w === "común" ? "Común" : w}</Chip>)}
          </div>
        )}
        <input style={{ ...inputStyle, padding: "9px 12px" }} value={note} onChange={(ev) => setNote(ev.target.value)} placeholder={tr("Nota (opcional)")} />
        <div style={{ display: "flex", gap: 8 }}>
          <button onClick={save} style={miniBtn(C.yellow, C.ink)}>{tr("Guardar")}</button>
          <button onClick={() => setEdit(false)} style={miniBtn(C.ink3, C.textLo)}>{tr("Cancelar")}</button>
...
function PartnersEditor({ partners, onSave, onCancel }) {
  ...
  return (
    <div style={{ ...card, padding: 12, background: C.well, marginTop: 10, display: "grid", gap: 8 }}>
      ...
      {ps.map((p) => (
        <div key={p.id} style={{ display: "flex", gap: 8 }}>
          <input style={{ ...inputStyle, flex: 1, padding: "9px 12px" }} value={p.name} onChange={(e) => setP(p.id, "name", e.target.value)} placeholder={tr("Nombre")} />
      ...
      <button onClick={add} style={{ ...iconBtn, width: "auto", paddingInline: 12, height: 36, gap: 6, fontSize: 12.5, fontWeight: 700 }}><UserPlus size={14} /> {tr("Añadir socio")}</button>
      <div style={{ display: "flex", gap: 8 }}>
        <button onClick={() => onSave(ps.filter((p) => p.name.trim()).map((p) => ({ id: p.id, name: p.name.trim(), share: p.share === "" ? null : Number(p.share) })))} style={miniBtn(C.purple, "#fff")}>{tr("Guardar socios")}</button>
        <button onClick={onCancel} style={miniBtn(C.ink3, C.textLo)}>{tr("Cancelar")}</button>
      </div>
    </div>
  );
}
function SharedInvForm({ me, onSave }) {
  ...
  return (
    <div>
      <Field label={tr("¿En qué invertís?")}><input style={inputStyle} value={name} onChange={(e) => setName(e.target.value)} placeholder={tr("Ej. reventa, cripto, un coche…")} autoFocus /></Field>
      <Field label={tr("Socios (nombre y % opcional)")}>
        <div style={{ display: "grid", gap: 8 }}>
          {partners.map((p) => (
            <div key={p.id} style={{ display: "flex", gap: 8 }}>
              <input style={{ ...inputStyle, flex: 1 }} value={p.name} onChange={(e) => setP(p.id, "name", e.target.value)} placeholder={tr("Nombre")} />
          ...
          <button onClick={addP} style={{ ...iconBtn, width: "auto", paddingInline: 12, height: 38, gap: 6, fontSize: 13, fontWeight: 700 }}><UserPlus size={15} /> {tr("Añadir socio")}</button>
        </div>
      </Field>
      <div style={{ display: "flex", gap: 10 }}>
        <div style={{ flex: 1 }}><Field label={tr("Invertido total (€)")}><input type="number" inputMode="decimal" style={inputStyle} value={invested} onChange={(e) => setInvested(e.target.value)} placeholder="0,00" /></Field></div>
        <div style={{ flex: 1 }}><Field label={tr("Valor hoy (€)")}><input type="number" inputMode="decimal" style={inputStyle} value={value} onChange={(e) => setValue(e.target.value)} placeholder="0,00" /></Field></div>
      </div>
      <PrimaryBtn full color={C.purple} onClick={() => { const ps = partners.filter((p) => p.name.trim()).map((p) => ({ id: p.id, name: p.name.trim(), share: p.share ? Number(p.share) : null })); if (name.trim() && ps.length) onSave({ name: name.trim(), partners: ps, invested: Number(invested || 0), value: Number(value || 0) }); }}><Check size={18} strokeWidth={3} color="#fff" /> <span style={{ color: "#fff" }}>{tr("Crear")}</span></PrimaryBtn>
    </div>
  );
}
function SharedMoveForm({ partners, onSave, onCancel }) {
  ...
  return (
    <div style={{ ...card, padding: 12, background: C.well, marginBottom: 6 }}>
      ...
      <input type="number" inputMode="decimal" autoFocus style={{ ...inputStyle, marginBottom: 8 }} value={amount} onChange={(e) => setAmount(e.target.value)} placeholder={tr("Importe €")} />
      {needsWho && (
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 8 }}>
          {whoOptions.map((w) => <Chip key={w} active={who === w} onClick={() => setWho(w)}>{w === "común" ? "Común" : w}</Chip>)}
        </div>
      )}
      <input style={{ ...inputStyle, marginBottom: 8 }} value={note} onChange={(e) => setNote(e.target.value)} placeholder={tr("Nota: en qué se reinvirtió / gastó…")} />
      <div style={{ display: "flex", gap: 8 }}>
        <button onClick={() => amount && onSave({ kind, amount: Math.abs(Number(amount)), who: needsWho ? who : "común", note })} style={miniBtn(C.purple, "#fff")}>{tr("Guardar")}</button>
        <button onClick={onCancel} style={miniBtn(C.ink3, C.textLo)}>{tr("Cancelar")}</button>
      </div>
    </div>
  );
...
/* ---------- Cierre de mes: UI ---------- */
function DeltaBadge({ now, prev, invert }) {
  if (!prev) return null;
  const pct = ((now - prev) / prev) * 100;
  if (!isFinite(pct) || Math.abs(pct) < 0.5) return null;
  const up = pct > 0;
  const good = invert ? !up : up;
  return <span style={{ fontSize: 10.5, fontWeight: 800, color: good ? C.good : C.bad }}>{up ? "▲" : "▼"} {Math.abs(pct).toFixed(0)}%</span>;
}
function MonthCloseCard({ state, ai }) {
  const now = new Date();
  ...
  const [aiBusy, setAiBusy] = useState(false);
  const s = monthStats(state, ym);
  const label = (() => { try { const p = ym.split("-").map(Number); return new Date(p[0], p[1] - 1, 1).toLocaleDateString(LOCALE, { month: "long", year: "numeric" }); } catch (e) { return ym; } })();
  const nav = (dir) => { const p = ym.split("-").map(Number); const d = new Date(p[0], p[1] - 1 + dir, 1); const next = `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, "0")}`; if (next <= curMonth) { setYm(next); setAiTxt(null); } };
  const analyze = async () => {
    if (aiBusy) return;
    setAiBusy(true); setAiTxt(null);
    try {
      const brief = `Mes ${label}: ingresos ${money(s.ing)}, gastos ${money(s.gas)}, neto ${money(s.net)} (mes anterior: neto ${money(s.pNet)}). Top categorías: ${s.topCats.map((c) => `${c.k} ${money(c.v)}${c.prev ? ` (antes ${money(c.prev)})` : ""}`).join(", ") || "—"}. Recurrentes detectados: ${s.recurring.map((r) => `${r.note} ${money(r.amount)}`).join(", ") || "ninguno"}. Metido en huchas: ${money(s.savedM)}. Gasto medio diario: ${money(s.dailyAvg)}. Días activos: ${s.activeDays}/${s.daysInMonth}.`;
      const txt = await callClaude([{ role: "user", content: "Analiza mi cierre de mes." }],
        `Eres la mente financiera de Brosin. Con estos datos reales, escribe 3-4 frases claras: qué fue lo importante del mes, una alerta si la hay (gasto disparado, recurrentes que suman mucho) y DOS acciones concretas para el mes que viene. Sin markdown, sin listas, prosa directa y cercana.\n=== DATOS ===\n${brief}` + aiLangRule());
      setAiTxt(String(txt).trim());
    } catch (e) { setAiTxt(null); }
    finally { setAiBusy(false); }
  };
  return (
    <div style={{ ...card, padding: 16 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
        <Receipt size={16} color={C.yellow} />
        <span style={{ color: C.textHi, fontWeight: 800, fontSize: 14, flex: 1 }}>{tr("Cierre de mes")}</span>
        <button onClick={() => nav(-1)} style={{ ...iconBtn, width: 28, height: 28 }}><ChevronLeft size={14} /></button>
        <span style={{ color: C.textHi, fontWeight: 800, fontSize: 12.5, textTransform: "capitalize", minWidth: 108, textAlign: "center" }}>{label}</span>
        <button onClick={() => nav(1)} style={{ ...iconBtn, width: 28, height: 28, opacity: ym >= curMonth ? 0.35 : 1 }}><ChevronRight size={14} /></button>
      </div>

      {s.txCount === 0 ? (
        <div style={{ color: C.textLo, fontSize: 13, textAlign: "center", padding: "14px 0" }}>{tr("Sin movimientos ese mes.")}</div>
      ) : (
        <>
          <div style={{ textAlign: "center", marginBottom: 12 }}>
            <div style={{ color: C.textLo, fontSize: 10.5, fontWeight: 800, letterSpacing: 1 }}>{tr("NETO DEL MES")}</div>
            <div style={{ color: s.net >= 0 ? C.good : C.bad, fontWeight: 900, fontSize: 30, lineHeight: 1.1, fontVariantNumeric: "tabular-nums" }}>{s.net >= 0 ? "+" : ""}{money(s.net)}</div>
            {s.pNet !== 0 && <div style={{ color: C.textLo, fontSize: 11.5, marginTop: 2 }}>{tr("vs mes anterior")}: {s.pNet >= 0 ? "+" : ""}{money(s.pNet)}</div>}
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 12 }}>
            <div style={{ ...card, padding: "9px 10px", background: C.ink, textAlign: "center" }}>
              <div style={{ color: C.textLo, fontSize: 10, fontWeight: 700 }}>{tr("INGRESOS")} <DeltaBadge now={s.ing} prev={s.pIng} /></div>
              <div style={{ color: C.good, fontWeight: 900, fontSize: 16, fontVariantNumeric: "tabular-nums" }}>{money(s.ing)}</div>
            </div>
            <div style={{ ...card, padding: "9px 10px", background: C.ink, textAlign: "center" }}>
              <div style={{ color: C.textLo, fontSize: 10, fontWeight: 700 }}>{tr("GASTOS")} <DeltaBadge now={s.gas} prev={s.pGas} invert /></div>
              <div style={{ color: C.bad, fontWeight: 900, fontSize: 16, fontVariantNumeric: "tabular-nums" }}>{money(s.gas)}</div>
            </div>
          </div>

          {s.topCats.length > 0 && (
            <div style={{ marginBottom: 12 }}>
              <div style={{ color: C.textLo, fontSize: 11, fontWeight: 800, letterSpacing: 0.6, textTransform: "uppercase", marginBottom: 6 }}>{tr("Top categorías")}</div>
              <div style={{ display: "grid", gap: 5 }}>
                {s.topCats.map((c, i) => (
                  <div key={c.k} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 12.5 }}>
                    <span style={{ width: 8, height: 8, borderRadius: 3, background: SLICE[i % SLICE.length], flexShrink: 0 }} />
                    <span style={{ color: C.textHi, flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{c.k}</span>
                    <DeltaBadge now={c.v} prev={c.prev} invert />
                    <span style={{ color: C.textHi, fontWeight: 800, fontVariantNumeric: "tabular-nums" }}>{money(c.v)}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          {s.recurring.length > 0 && (
            <div style={{ ...card, padding: 11, background: C.ink, marginBottom: 12 }}>
              <div style={{ color: C.yellow, fontSize: 11, fontWeight: 800, letterSpacing: 0.5, marginBottom: 6 }}>🔁 {tr("Recurrentes detectados")} · {money(s.recurringTotal)}/{tr("mes")}</div>
              <div style={{ display: "grid", gap: 3 }}>
                {s.recurring.map((r, i) => (
                  <div key={i} style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: C.textLo }}>
                    <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{r.note}</span>
                    <span style={{ color: C.textHi, fontWeight: 700, fontVariantNumeric: "tabular-nums" }}>{money(r.amount)}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 12, fontSize: 11.5, color: C.textLo }}>
            <span style={{ ...card, padding: "5px 10px", background: C.ink }}>{tr("Media diaria")}: <b style={{ color: C.textHi }}>{money(s.dailyAvg)}</b></span>
            {s.worstDay && <span style={{ ...card, padding: "5px 10px", background: C.ink }}>{tr("Día más caro")}: <b style={{ color: C.textHi }}>{fmtDate(s.worstDay.date)} · {money(s.worstDay.amount)}</b></span>}
            {s.savedM > 0 && <span style={{ ...card, padding: "5px 10px", background: C.ink }}>🐷 <b style={{ color: C.good }}>{money(s.savedM)}</b></span>}
            <span style={{ ...card, padding: "5px 10px", background: C.ink }}>🐝 {s.activeDays}/{s.daysInMonth} {tr("días activos")}</span>
          </div>

          {ai && (aiTxt ? (
            <div style={{ ...card, padding: "11px 13px", background: C.tintP || "rgba(144,19,254,.08)", border: "1px solid rgba(144,19,254,.45)", color: C.textHi, fontSize: 13, lineHeight: 1.55 }}>✨ {aiTxt}</div>
          ) : (
            <button onClick={analyze} style={{ ...iconBtn, width: "100%", height: 42, gap: 8, fontWeight: 800, fontSize: 13, background: "rgba(144,19,254,.16)", borderColor: C.purple, color: C.textHi, opacity: aiBusy ? 0.6 : 1 }}>
              {aiBusy ? <><span style={{ display: "inline-flex", animation: "bspin 1s linear infinite" }}><RefreshCcw size={15} /></span> {tr("Analizando…")}</> : <><Sparkles size={15} color={C.purple} /> {tr("Analizar mi mes con IA")}</>}
            </button>
          ))}
        </>
      )}
...
/* ---------- Grupos de gastos: UI ---------- */
function GroupsPanel({ state, dispatch, toast }) {
  const [sheet, setSheet] = useState(null); // 'new' | 'import'
  const list = state.groups || [];
  const addNew = (d) => { dispatch({ type: "add", col: "groups", item: { id: uid(), name: d.name, members: d.members, expenses: [], settled: [] } }); setSheet(null); };
  const doImport = (code) => {
    const obj = decodeShare(code);
    if (!obj || obj.kind !== "brosin-group" || !obj.group) { toast && toast("✗", tr("No reconozco ese código de grupo")); return; }
    const g = obj.group;
    dispatch({ type: "add", col: "groups", item: { id: uid(), name: g.name || "Grupo", members: (g.members || []).map((m) => ({ id: m.id || uid(), name: String(m.name || "?") })), expenses: (g.expenses || []).map((e) => ({ ...e, id: e.id || uid() })), settled: (g.settled || []).map((s) => ({ ...s, id: s.id || uid() })) } });
    setSheet(null); toast && toast("✓", tr("Grupo importado"));
  };
  return (
    <div>
      {list.length === 0 ? (
        <EmptyState face="happy" title={tr("Cuentas entre amigos")} sub={tr("Reparte los gastos de cenas, viajes o el piso: apunta quién pagó cada cosa y Brosin calcula quién debe cuánto a quién, con los mínimos pagos posibles.")} cta={tr("Nuevo grupo")} onCta={() => setSheet("new")} />
      ) : (
        <div style={{ display: "grid", gap: 12 }}>
          {list.map((g) => <GroupCard key={g.id} g={g} dispatch={dispatch} toast={toast} />)}
        </div>
      )}
      <button onClick={() => setSheet("import")} style={{ ...iconBtn, width: "100%", height: 42, marginTop: 12, gap: 8, fontWeight: 700, fontSize: 12.5, color: C.textLo }}>{tr("Importar grupo (pegar código)")}</button>
      <div style={{ height: 122 }} />
      <FAB onClick={() => setSheet("new")} label={tr("Grupo")} />
      <Sheet open={sheet === "new"} onClose={() => setSheet(null)} title={tr("Nuevo grupo de gastos")}>
        <GroupForm me={state.profile.name} onSave={addNew} />
      </Sheet>
      <Sheet open={sheet === "import"} onClose={() => setSheet(null)} title={tr("Importar grupo")}>
        <ImportGroupForm onImport={doImport} />
      </Sheet>
    </div>
  );
}
function GroupForm({ me, onSave }) {
  const [name, setName] = useState("");
  const [members, setMembers] = useState((me ? [me, ""] : ["", ""]).map((n) => ({ id: uid(), name: n })));
  const setM = (id, v) => setMembers((a) => a.map((m) => (m.id === id ? { ...m, name: v } : m)));
  return (
    <div>
      <Field label={tr("Nombre del grupo")}><input style={inputStyle} value={name} onChange={(e) => setName(e.target.value)} placeholder={tr("Ej. Viaje a Lisboa, piso, cena…")} autoFocus /></Field>
      <Field label={tr("Miembros")}>
        <div style={{ display: "grid", gap: 8 }}>
          {members.map((m) => (
            <div key={m.id} style={{ display: "flex", gap: 8 }}>
              <input style={{ ...inputStyle, flex: 1 }} value={m.name} onChange={(e) => setM(m.id, e.target.value)} placeholder={tr("Nombre")} />
              {members.length > 2 && <button onClick={() => setMembers((a) => a.filter((x) => x.id !== m.id))} style={{ ...iconBtn, width: 44, height: 44, color: C.textLo }}><X size={16} /></button>}
            </div>
          ))}
          <button onClick={() => setMembers((a) => [...a, { id: uid(), name: "" }])} style={{ ...iconBtn, width: "auto", paddingInline: 12, height: 38, gap: 6, fontSize: 13, fontWeight: 700 }}><UserPlus size={15} /> {tr("Añadir miembro")}</button>
        </div>
      </Field>
      <PrimaryBtn full color={C.purple} onClick={() => { const ms = members.filter((m) => m.name.trim()).map((m) => ({ id: m.id, name: m.name.trim() })); if (name.trim() && ms.length >= 2) onSave({ name: name.trim(), members: ms }); }}><Check size={18} strokeWidth={3} color="#fff" /> <span style={{ color: "#fff" }}>{tr("Crear")}</span></PrimaryBtn>
    </div>
  );
}
function ImportGroupForm({ onImport }) {
  const [code, setCode] = useState("");
  return (
    <div>
      <Field label={tr("Pega el código del grupo")}>
        <textarea style={{ ...inputStyle, minHeight: 90, resize: "vertical", fontFamily: "monospace", fontSize: 12 }} value={code} onChange={(e) => setCode(e.target.value)} placeholder="BRSN1:…" />
      </Field>
      <PrimaryBtn full onClick={() => code.trim() && onImport(code.trim())}><Download size={18} strokeWidth={2.6} /> {tr("Importar")}</PrimaryBtn>
    </div>
  );
}
function GroupCard({ g, dispatch, toast }) {
  const [open, setOpen] = useState(false);
  const [addOpen, setAddOpen] = useState(false);
  const [copied, setCopied] = useState(false);
  const bal = groupBalances(g);
  const plan = simplifyDebts(bal);
  const total = groupTotal(g);
  const nameOf = (id) => { const m = (g.members || []).find((x) => x.id === id); return m ? m.name : "?"; };
  const patch = (p) => dispatch({ type: "update", col: "groups", id: g.id, patch: p });
  const addExpense = (e) => { patch({ expenses: [{ id: uid(), date: todayISO(), ...e }, ...(g.expenses || [])] }); setAddOpen(false); };
  const delExpense = (id) => patch({ expenses: (g.expenses || []).filter((e) => e.id !== id) });
  const markPaid = (t) => patch({ settled: [{ id: uid(), date: todayISO(), from: t.from, to: t.to, amount: t.amount }, ...(g.settled || [])] });
  const shareCode = () => { try { const code = encodeShare({ kind: "brosin-group", v: 1, group: { name: g.name, members: g.members, expenses: g.expenses, settled: g.settled } }); navigator.clipboard && navigator.clipboard.writeText(code); setCopied(true); setTimeout(() => setCopied(false), 2000); toast && toast("✓", tr("Código copiado — pásaselo a tus amigos")); } catch (e) {} };
  const settledUp = plan.length === 0;
  return (
    <div style={{ ...card, padding: 16 }}>
      <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
        <div style={{ width: 38, height: 38, borderRadius: 11, background: "rgba(255,212,0,.14)", display: "grid", placeItems: "center", flexShrink: 0 }}><Users size={18} color={C.yellow} /></div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ color: C.textHi, fontWeight: 800, fontSize: 15.5, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{g.name}</div>
          <div style={{ color: C.textLo, fontSize: 12 }}>{(g.members || []).map((m) => m.name).join(" · ")}</div>
        </div>
        <button onClick={() => { if (typeof window === "undefined" || window.confirm(`${tr("¿Borrar el grupo")} "${g.name}"?`)) dispatch({ type: "remove", col: "groups", id: g.id }); }} style={{ ...iconBtn, width: 30, height: 30, color: C.textLo }}><Trash2 size={14} /></button>
      </div>

      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 12 }}>
        <span style={{ color: C.textLo, fontSize: 12, fontWeight: 700 }}>{tr("Total del grupo")}</span>
        <span style={{ color: C.textHi, fontWeight: 900, fontSize: 17, fontVariantNumeric: "tabular-nums" }}>{money(total)}</span>
      </div>

      {/* saldos por miembro */}
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 10 }}>
        {(g.members || []).map((m) => { const v = Math.round((bal[m.id] || 0) * 100) / 100; const pos = v > 0.004, neg = v < -0.004; return (
          <span key={m.id} style={{ borderRadius: 999, padding: "5px 11px", fontSize: 12, fontWeight: 800, background: pos ? "rgba(61,220,132,.13)" : neg ? "rgba(255,92,92,.12)" : C.ink3, color: pos ? C.good : neg ? C.bad : C.textLo, border: `1px solid ${pos ? C.good : neg ? C.bad : C.line}` }}>
            {m.name} {pos ? "+" : ""}{money(v)}
          </span>
        ); })}
      </div>

      {/* plan de pagos mínimos */}
      <div style={{ marginTop: 12 }}>
        {settledUp ? (
          <div style={{ ...card, padding: "10px 12px", background: "rgba(61,220,132,.08)", border: `1px solid ${C.good}`, color: C.good, fontSize: 13, fontWeight: 800, textAlign: "center" }}>{tr("Cuentas en paz")} ✓</div>
        ) : (
          <div style={{ display: "grid", gap: 6 }}>
            <div style={{ color: C.textLo, fontSize: 11, fontWeight: 800, letterSpacing: 0.8, textTransform: "uppercase" }}>{tr("Pagos mínimos para quedar en paz")} ({plan.length})</div>
            {plan.map((t, i) => (
              <div key={i} style={{ ...card, padding: "9px 12px", display: "flex", alignItems: "center", gap: 8, background: C.ink }}>
                <span style={{ flex: 1, color: C.textHi, fontSize: 13 }}><b>{nameOf(t.from)}</b> <span style={{ color: C.textLo }}>{tr("le paga a")}</span> <b>{nameOf(t.to)}</b></span>
                <span style={{ color: C.yellow, fontWeight: 900, fontSize: 13.5, fontVariantNumeric: "tabular-nums" }}>{money(t.amount)}</span>
                <button onClick={() => markPaid(t)} style={{ ...iconBtn, width: "auto", height: 30, paddingInline: 10, fontSize: 11.5, fontWeight: 800, background: "rgba(61,220,132,.13)", borderColor: C.good, color: C.good }}>{tr("Pagado")}</button>
              </div>
            ))}
          </div>
        )}
      ...
      {/* añadir gasto */}
      {addOpen ? (
        <GroupExpenseForm members={g.members || []} onSave={addExpense} onCancel={() => setAddOpen(false)} />
      ) : (
        <button onClick={() => setAddOpen(true)} style={{ ...iconBtn, width: "100%", height: 40, marginTop: 10, gap: 8, fontWeight: 800, fontSize: 13, background: C.purple, borderColor: C.purple, color: "#fff" }}><Plus size={16} color="#fff" strokeWidth={3} /> {tr("Añadir gasto")}</button>
      )}

      {/* gastos */}
      <button onClick={() => setOpen((o) => !o)} style={{ ...iconBtn, width: "100%", height: 36, marginTop: 10, gap: 8, fontWeight: 800, fontSize: 12.5, background: C.ink3 }}>
        <ChevronRight size={14} style={{ transform: open ? "rotate(90deg)" : "none", transition: "transform .2s" }} /> {tr("Gastos")} ({(g.expenses || []).length})
      </button>
      {open && (
        <div style={{ display: "grid", gap: 6, marginTop: 8 }}>
          {(g.expenses || []).length === 0 && <div style={{ color: C.textLo, fontSize: 12.5, textAlign: "center", padding: "6px 0" }}>{tr("Aún no hay gastos.")}</div>}
          {(g.expenses || []).map((e) => (
            <div key={e.id} style={{ ...card, padding: "9px 12px", display: "flex", alignItems: "center", gap: 10, background: C.ink }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ color: C.textHi, fontSize: 13, fontWeight: 700, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{e.desc || tr("Gasto")}</div>
                <div style={{ color: C.textLo, fontSize: 11 }}>{fmtDate(e.date)} · {tr("pagó")} {nameOf(e.paidBy)}</div>
              </div>
              <span style={{ color: C.textHi, fontWeight: 800, fontSize: 13, fontVariantNumeric: "tabular-nums" }}>{money(e.amount)}</span>
              <button onClick={() => delExpense(e.id)} style={{ background: "none", border: "none", color: C.textLo, cursor: "pointer", padding: 2 }}><X size={13} /></button>
            </div>
          ))}
        </div>
      )}

      <button onClick={shareCode} style={{ ...iconBtn, width: "100%", height: 36, marginTop: 10, gap: 8, fontWeight: 700, fontSize: 12.5, color: C.textLo }}>
        {copied ? <><Check size={14} color={C.good} /> {tr("Código copiado")}</> : <><Share2 size={14} /> {tr("Compartir grupo (código)")}</>}
      </button>
    </div>
  );
}
function GroupExpenseForm({ members, onSave, onCancel }) {
  const [desc, setDesc] = useState("");
  const [amount, setAmount] = useState("");
  const [paidBy, setPaidBy] = useState(members[0] ? members[0].id : "");
  const [split, setSplit] = useState(members.map((m) => m.id));
  const togg = (id) => setSplit((s) => (s.includes(id) ? s.filter((x) => x !== id) : [...s, id]));
  return (
    <div style={{ ...card, padding: 12, background: C.ink, marginTop: 10 }}>
      <input style={{ ...inputStyle, marginBottom: 8 }} value={desc} onChange={(e) => setDesc(e.target.value)} placeholder={tr("¿Qué fue? (cena, gasolina…)")} autoFocus />
      <input type="number" inputMode="decimal" style={{ ...inputStyle, marginBottom: 8 }} value={amount} onChange={(e) => setAmount(e.target.value)} placeholder={tr("Importe €")} />
      <div style={{ color: C.textLo, fontSize: 11, fontWeight: 800, letterSpacing: 0.5, marginBottom: 6 }}>{tr("PAGÓ")}</div>
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 8 }}>
        {members.map((m) => <Chip key={m.id} active={paidBy === m.id} onClick={() => setPaidBy(m.id)}>{m.name}</Chip>)}
      </div>
      <div style={{ color: C.textLo, fontSize: 11, fontWeight: 800, letterSpacing: 0.5, marginBottom: 6 }}>{tr("ENTRE")}</div>
      <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 10 }}>
        {members.map((m) => <Chip key={m.id} active={split.includes(m.id)} onClick={() => togg(m.id)} accent={C.purple}>{m.name}</Chip>)}
      </div>
      <div style={{ display: "flex", gap: 8 }}>
        <button onClick={() => { const v = Number(amount); if (v > 0 && paidBy && split.length) onSave({ desc: desc.trim(), amount: v, paidBy, split: split.length === members.length ? [] : split }); }} style={miniBtn(C.yellow, C.ink)}>{tr("Guardar")}</button>
        <button onClick={onCancel} style={miniBtn(C.ink3, C.textLo)}>{tr("Cancelar")}</button>
      </div>
    </div>
  );
...
function SavingsPanel({ state, dispatch }) {
  ...
  const add = (d) => { dispatch({ type: "add", col: "savings", item: { id: uid(), name: d.name, target: d.target, deadline: d.deadline || "", partners: d.partners || [], log: [] } }); setSheet(false); };
  return (
    <div>
      {list.length === 0 ? (
        <EmptyState face="happy" title={tr("Tus huchas")} sub={tr("Ponte metas de ahorro y ve llenando la hucha. Cada aporte suma y ves cuánto te queda para llegar.")} cta={tr("Nueva hucha")} onCta={() => setSheet(true)} />
      ) : (
        <div style={{ display: "grid", gap: 12 }}>{list.map((h) => <SavingsCard key={h.id} h={h} dispatch={dispatch} />)}</div>
      )}
      <div style={{ height: 122 }} />
      <FAB onClick={() => setSheet(true)} label={tr("Hucha")} />
      <Sheet open={sheet} onClose={() => setSheet(false)} title={tr("Nueva hucha")}><SavingsForm onSave={add} /></Sheet>
    </div>
  );
}
function SavingsCard({ h, dispatch }) {
  ...
  const conjunta = partners.length > 0;
  const quienes = conjunta ? ["Yo", ...partners] : [];
  const [who, setWho] = useState("Yo");
  const contribute = (sign) => { const v = Number(amt); if (!v) return; patch({ log: [{ id: uid(), date: todayISO(), amount: sign * Math.abs(v), who: conjunta ? who : undefined }, ...log] }); setAmt(""); setAddOpen(false); };
  const porPersona = quienes.map((q) => ({ q, v: log.filter((e) => (e.who || "Yo") === q).reduce((s, e) => s + Number(e.amount || 0), 0) }));
  ...
  return (
    <div style={{ ...card, padding: 16, borderColor: done ? C.good : C.line }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ color: C.textHi, fontWeight: 800, fontSize: 15.5, display: "flex", alignItems: "center", gap: 6 }}>
            ...
            {conjunta && <Users size={13} color={C.purple} />}
          </div>
          <div style={{ color: C.textLo, fontSize: 12 }}>{money(saved)} de {money(target)}{h.deadline ? ` · ${fmtDate(h.deadline)}` : ""}</div>
          {conjunta && <div style={{ color: C.purple, fontSize: 11.5, fontWeight: 700 }}>{tr("Conjunta")} · {partners.join(" · ")}</div>}
          <CreatedAt at={h.createdAt} />
        </div>
      ...
      <div style={{ color: C.textLo, fontSize: 12, marginTop: 6 }}>{done ? tr("¡Meta conseguida! 🎉") : tri("Te faltan {n}", { n: money(remaining) })}</div>
      {conjunta && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(84px,1fr))", gap: 8, marginTop: 10 }}>
          {porPersona.map((p) => <MiniStat key={p.q} label={p.q === "Yo" ? tr("YO") : p.q.toUpperCase()} val={money(p.v)} col={p.v >= 0 ? C.good : C.bad} />)}
        </div>
      )}
      {addOpen ? (
        <div style={{ marginTop: 10 }}>
          {conjunta && (
            <div style={{ display: "flex", gap: 6, marginBottom: 8, overflowX: "auto", paddingBottom: 2 }}>
              {quienes.map((q) => <Chip key={q} active={who === q} accent={C.purple} onClick={() => setWho(q)}>{q === "Yo" ? tr("Yo") : q}</Chip>)}
            </div>
          )}
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button onClick={() => contribute(1)} style={miniBtn(C.good, C.ink)}>{tr("Meter")}</button>
            <button onClick={() => contribute(-1)} style={miniBtn(C.ink3, C.textHi)}>{tr("Sacar")}</button>
          </div>
        </div>
      ) : (
        <button onClick={() => setAddOpen(true)} style={{ ...iconBtn, width: "100%", height: 40, marginTop: 10, gap: 8, fontWeight: 800, fontSize: 13, background: C.ink3 }}><Plus size={16} /> {tr("Aportar / sacar")}</button>
      )}
    </div>
  );
}
function SavingsForm({ onSave }) {
  ...
  const parsePartners = (s) => String(s || "").split(",").map((x) => x.trim()).filter(Boolean);
  return (
    <div>
      <Field label={tr("¿Para qué ahorras?")}><input style={inputStyle} value={name} onChange={(e) => setName(e.target.value)} placeholder={tr("Ej. Viaje, coche, colchón…")} autoFocus /></Field>
      <div style={{ display: "flex", gap: 10 }}>
        <div style={{ flex: 1 }}><Field label={tr("Meta (€)")}><input type="number" inputMode="decimal" style={inputStyle} value={target} onChange={(e) => setTarget(e.target.value)} placeholder="0,00" /></Field></div>
        <div style={{ flex: 1 }}><Field label={tr("Fecha (opcional)")}><input type="date" style={inputStyle} value={deadline} onChange={(e) => setDeadline(e.target.value)} /></Field></div>
      </div>
      <Field label={tr("¿Ahorras con alguien? (opcional)")}>
        <input style={inputStyle} value={partners} onChange={(e) => setPartners(e.target.value)} placeholder={tr("Nombres separados por comas")} />
      </Field>
      <PrimaryBtn full color={C.good} onClick={() => name.trim() && target && onSave({ name: name.trim(), target: Number(target), deadline, partners: parsePartners(partners) })}><Check size={18} strokeWidth={3} /> {tr("Crear hucha")}</PrimaryBtn>
    </div>
  );
}
...
/* ============================================================
   EL RADAR DE ÁREAS — de un vistazo, en qué destacas y en qué no
   SVG a pelo, sin librerías: son cuatro fórmulas de trigonometría y pesa cero.
   El polígono tiene tantos lados como áreas tengas (nueve por defecto), así
   que si añades o quitas una, se redibuja solo.
   ============================================================ */
function RadarAreas({ filas, size = 250 }) {
  const n = filas.length;
  if (n < 3) return null;
  const cx = size / 2, cy = size / 2 + 4;
  const R = size / 2 - 34;              // hueco para las etiquetas
  const ang = (i) => (Math.PI * 2 * i) / n - Math.PI / 2;   // arriba primero
  const punto = (i, r) => [cx + Math.cos(ang(i)) * R * r, cy + Math.sin(ang(i)) * R * r];
  const camino = (r) => filas.map((f, i) => punto(i, typeof r === "function" ? r(f) : r).join(",")).join(" ");
  const anillos = [0.25, 0.5, 0.75, 1];
  // Suelo del 12 % para que un área a cero siga viéndose y se pueda tocar
  const relleno = camino((f) => 0.12 + f.ratio * 0.88);

  return (
    <svg viewBox={`0 0 ${size} ${size + 8}`} style={{ width: "100%", height: "auto", display: "block", overflow: "visible" }}>
      <defs>
        <radialGradient id="radarFill" cx="50%" cy="50%" r="50%">
          <stop offset="0%" stopColor={C.yellow} stopOpacity="0.55" />
          <stop offset="100%" stopColor={C.yellow} stopOpacity="0.16" />
        </radialGradient>
      </defs>

      {/* telaraña */}
      {anillos.map((a, i) => (
        <polygon key={i} points={camino(a)} fill="none" stroke={C.line} strokeWidth={i === anillos.length - 1 ? 1.4 : 1} opacity={i === anillos.length - 1 ? 0.9 : 0.45} />
      ))}
      {filas.map((f, i) => {
        const [x, y] = punto(i, 1);
        return <line key={f.k} x1={cx} y1={cy} x2={x} y2={y} stroke={C.line} strokeWidth="1" opacity="0.45" />;
      })}

      {/* tu forma */}
      <polygon points={relleno} fill="url(#radarFill)" stroke={C.yellow} strokeWidth="2" strokeLinejoin="round" />
      {filas.map((f, i) => {
        const [x, y] = punto(i, 0.12 + f.ratio * 0.88);
        return <circle key={f.k} cx={x} cy={y} r="3.2" fill={C.yellow} stroke={C.ink} strokeWidth="1.2" />;
      })}

      {/* etiquetas: el emoji fuera del polígono y el nivel debajo */}
      {filas.map((f, i) => {
        const [x, y] = punto(i, 1.22);
        return (
          <g key={f.k}>
            <text x={x} y={y} textAnchor="middle" dominantBaseline="middle" style={{ fontSize: 15 }}>{f.e}</text>
            <text x={x} y={y + 13} textAnchor="middle" dominantBaseline="middle" style={{ fontSize: 8.5, fontWeight: 800, fill: C.textLo, letterSpacing: 0.2 }}>{f.nivel}</text>
          </g>
        );
      })}
    </svg>
...
/* ============================================================
   ...
   ============================================================ */
function AvatarPanel({ state, dispatch, toast, seccion }) {
  const [sheet, setSheet] = useState(null); // null | 'malo' | 'articulo' | 'areas' | 'reglas'
  const ver = (k) => !seccion || seccion === k;   // sin sección, se ve todo
  const radio = radiografia(state);
  const nivelG = nivelGlobal(state);
  ...
  return (
    <div>
      {/* --- El avatar: nivel, vida y monedas de un vistazo --- */}
      {ver("avatar") && (
      <div style={{ ...card, padding: 18, marginBottom: 12, borderColor: r.muerto ? C.bad : C.line }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 12 }}>
          <div style={{ position: "relative", flexShrink: 0 }}>
            ...
            <span style={{ position: "absolute", bottom: -4, right: -6, background: C.yellow, color: C.ink, fontSize: 10, fontWeight: 900, borderRadius: 999, padding: "1px 6px", boxShadow: "0 2px 6px rgba(0,0,0,.4)" }}>
              {tri("Nv {n}", { n: nivelG.nivel })}
            </span>
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 8 }}>
          <span style={{ color: C.textLo, fontSize: 11 }}>{tri("{n}/{m} XP para el nivel {s}", { n: nivelG.dentro, m: nivelG.paraSiguiente, s: nivelG.nivel + 1 })}</span>
          {(r.vidas || 0) > 0 && <span style={{ color: C.good, fontSize: 11.5, fontWeight: 800 }}>{tri("+{n} vida extra", { n: r.vidas })}</span>}
        </div>
      </div>
      )}

      {/* --- El radar: dónde estás fuerte y dónde flojeas --- */}
      {ver("avatar") && (
        <div style={{ ...card, padding: "16px 14px 12px", marginBottom: 12 }}>
          <div style={{ color: C.textHi, fontWeight: 900, fontSize: 14, marginBottom: 2 }}>{tr("Tu forma")}</div>
          <div style={{ color: C.textLo, fontSize: 11.5, marginBottom: 6 }}>{tr("Cuanto más lejos del centro, más nivel tienes en esa área.")}</div>
          <RadarAreas filas={radio.filas} />
          {radio.fuerte && radio.flojo && radio.fuerte.k !== radio.flojo.k && (
            <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
              <div style={{ ...filaDentro(false), flex: 1, padding: "9px 11px" }}>
                <div style={{ color: C.good, fontSize: 10, fontWeight: 800, letterSpacing: 0.5 }}>{tr("DESTACA")}</div>
                <div style={{ color: C.textHi, fontWeight: 700, fontSize: 13 }}>{radio.fuerte.e} {tr(radio.fuerte.n)}</div>
              </div>
              <div style={{ ...filaDentro(false), flex: 1, padding: "9px 11px" }}>
                <div style={{ color: C.bad, fontSize: 10, fontWeight: 800, letterSpacing: 0.5 }}>{tr("TE PIDE ATENCIÓN")}</div>
                <div style={{ color: C.textHi, fontWeight: 700, fontSize: 13 }}>{radio.flojo.e} {tr(radio.flojo.n)}</div>
              </div>
            </div>
          )}
          {!radio.fuerte && (
            <div style={{ color: C.textLo, fontSize: 12, marginTop: 8, lineHeight: 1.45 }}>
              {tr("Cumple hábitos y el radar se irá deformando hacia lo que trabajas. Ahí verás lo que tienes abandonado.")}
            </div>
          )}
        </div>
      )}
...
/* ============================================================
   EL JUEGO — su propia pestaña abajo
   ============================================================ */
function GameScreen({ state, dispatch, toast, ai }) {
  const [tab, setTab] = useState("avatar");
  const temp = temporadaDe(state);
  return (
    <div style={{ padding: "20px 18px 0" }}>
      <Title kicker={tr("Tu vida, jugada")}>{tr("Juego")}</Title>

      <div style={{ display: "flex", gap: 8, marginBottom: 16, overflowX: "auto", paddingBottom: 4, WebkitOverflowScrolling: "touch" }}>
        <Chip active={tab === "avatar"} onClick={() => setTab("avatar")}>{tr("Avatar")}</Chip>
        <Chip active={tab === "mision"} onClick={() => setTab("mision")}>{tr("Misión")}</Chip>
      ...
      {/* La temporada siempre a la vista: es el reloj del juego */}
      {temp && tab !== "temporada" && (
        <button onClick={() => setTab("temporada")} style={{ ...filaDentro(false), width: "100%", padding: "9px 12px", marginBottom: 14, display: "flex", alignItems: "center", gap: 9, cursor: "pointer", textAlign: "left" }}>
          <span style={{ fontSize: 14 }}>🏁</span>
          <span style={{ flex: 1, color: C.textHi, fontWeight: 700, fontSize: 12.5 }}>{temp.nombre}</span>
          <span style={{ color: temp.diasRestantes <= 3 ? C.bad : C.yellow, fontWeight: 800, fontSize: 12 }}>
            {temp.diasRestantes === 1 ? tr("último día") : tri("{n} días", { n: temp.diasRestantes })}
          </span>
        </button>
      )}
...
/* El banco de hábitos con su propio botón de crear (antes vivía en Cerebro) */
function HabitsPanel({ state, dispatch, toast }) {
  const [sheet, setSheet] = useState(false);
  return (
    <div>
      <HabitsList habits={state.habits} dispatch={dispatch} state={state} toast={toast} onAdd={() => setSheet(true)} />
      <div style={{ height: 122 }} />
      <FAB onClick={() => setSheet(true)} label={tr("Hábito")} />
      <Sheet open={sheet} onClose={() => setSheet(false)} title={tr("Nuevo hábito")}>
        <HabitForm areas={rpgDe(state).areas} onSave={(d) => { dispatch({ type: "add", col: "habits", item: { id: uid(), done: [], ...d } }); setSheet(false); }} />
      </Sheet>
    </div>
  );
}

function ChallengesPanelSuelto({ state, dispatch }) {
  const [sheet, setSheet] = useState(false);
  return (
    <div>
      <ChallengesPanel state={state} dispatch={dispatch} onAdd={() => setSheet(true)} />
      <div style={{ height: 122 }} />
      <FAB onClick={() => setSheet(true)} label={tr("Reto")} />
      <Sheet open={sheet} onClose={() => setSheet(false)} title={tr("Nuevo reto con amigos")}>
        <ChallengeForm onSave={(d) => { dispatch({ type: "add", col: "challenges", item: { id: uid(), ...d } }); setSheet(false); }} />
      </Sheet>
    </div>
  );
}
...
/* ============================================================
   TEMPORADAS — un principio y un final
   Una racha infinita no motiva: no se gana nunca. Una temporada de 30 o 90
   días sí, porque tiene meta. Al cerrarla se guarda el resultado y el XP se
   queda donde está: lo que has aprendido no se borra, solo empieza otra cuenta.
   ============================================================ */
const TEMP_DURACIONES = [
  { k: 30, n: "30 días", d: "Un mes. Para probar algo nuevo." },
  { k: 90, n: "90 días", d: "Un trimestre. Aquí cambian las cosas de verdad." },
];
const diasEntre = (a, b) => Math.round((Date.parse(b) - Date.parse(a)) / 86400000);

function temporadaDe(state) {
  const t = (state.temporadas || []).find((x) => !x.cerrada);
  if (!t) return null;
  const hoy = todayISO();
  const total = Number(t.dias || 30);
  const pasados = Math.max(0, diasEntre(t.inicio, hoy));
  return { ...t, total, pasados, diasRestantes: Math.max(0, total - pasados), pct: Math.min(100, (pasados / total) * 100), acabada: pasados >= total };
}

function TemporadaPanel({ state, dispatch, toast }) {
  const [sheet, setSheet] = useState(false);
  const t = temporadaDe(state);
  const hist = (state.temporadas || []).filter((x) => x.cerrada).sort((a, b) => (b.fin || "").localeCompare(a.fin || ""));
  const r = rpgDe(state);

  const cerrar = () => {
    if (!t) return;
    const marca = { cerrada: true, fin: todayISO(), nivelFin: nivelGlobal(state).nivel, coinsFin: r.coins || 0, xpFin: Object.values(r.xp || {}).reduce((s, v) => s + Number(v || 0), 0) };
    dispatch({ type: "update", col: "temporadas", id: t.id, patch: marca });
    toast && toast(tr("Temporada cerrada"), tri("Acabaste en nivel {n}. El XP se queda contigo.", { n: marca.nivelFin }));
  };

  const abrir = (dias, nombre) => {
    const g = nivelGlobal(state);
    dispatch({ type: "add", col: "temporadas", item: { id: uid(), nombre: nombre.trim(), dias, inicio: todayISO(), cerrada: false, nivelIni: g.nivel, xpIni: g.total, coinsIni: r.coins || 0 } });
    setSheet(false);
  };
  ...
  return (
    <div>
      {t ? (
        <div style={{ ...card, padding: 16, marginBottom: 14 }}>
          <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", marginBottom: 8 }}>
            <span style={{ color: C.textHi, fontWeight: 900, fontSize: 16 }}>{t.nombre}</span>
            <span style={{ color: t.diasRestantes <= 3 ? C.bad : C.yellow, fontWeight: 900, fontSize: 13 }}>
              {t.acabada ? tr("Terminada") : t.diasRestantes === 1 ? tr("Último día") : tri("Quedan {n} días", { n: t.diasRestantes })}
            </span>
          </div>
          <div style={{ height: 10, borderRadius: 999, background: C.rail, overflow: "hidden", marginBottom: 8 }}>
            <div style={{ width: `${t.pct}%`, height: "100%", background: t.acabada ? C.good : C.yellow, borderRadius: 999, transition: "width .4s" }} />
          </div>
          <div style={{ color: C.textLo, fontSize: 11.5, marginBottom: 12 }}>{tri("Día {n} de {m}", { n: Math.min(t.pasados + 1, t.total), m: t.total })}</div>

          <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
            <div style={{ ...filaDentro(false), flex: 1, padding: "9px 11px" }}>
              <div style={{ color: C.textLo, fontSize: 10, fontWeight: 800, letterSpacing: 0.5 }}>{tr("NIVEL")}</div>
              <div style={{ color: C.textHi, fontWeight: 900, fontSize: 15 }}>{t.nivelIni} → {nivelGlobal(state).nivel}</div>
            </div>
            <div style={{ ...filaDentro(false), flex: 1, padding: "9px 11px" }}>
              <div style={{ color: C.textLo, fontSize: 10, fontWeight: 800, letterSpacing: 0.5 }}>{tr("XP GANADO")}</div>
              <div style={{ color: C.yellow, fontWeight: 900, fontSize: 15 }}>+{Math.max(0, nivelGlobal(state).total - Number(t.xpIni || 0))}</div>
            </div>
          </div>

          <PrimaryBtn full color={t.acabada ? C.good : C.ink3} onClick={cerrar}>
            <Check size={18} strokeWidth={3} /> {t.acabada ? tr("Cerrar y guardar el resultado") : tr("Cerrarla antes de tiempo")}
          </PrimaryBtn>
        </div>
      ) : (
        <div style={{ ...card, padding: 18, marginBottom: 14, textAlign: "center" }}>
          <div style={{ fontSize: 30, marginBottom: 6 }}>🏁</div>
          <div style={{ color: C.textHi, fontWeight: 900, fontSize: 16, marginBottom: 5 }}>{tr("Sin temporada abierta")}</div>
          <div style={{ color: C.textLo, fontSize: 13, lineHeight: 1.5, marginBottom: 14 }}>
            {tr("Una racha infinita no se gana nunca. Ponle fecha de final y verás la diferencia.")}
          </div>
          <PrimaryBtn full onClick={() => setSheet(true)}><Plus size={17} strokeWidth={3} /> {tr("Empezar temporada")}</PrimaryBtn>
        </div>
      )}

      {hist.length > 0 && (
        <>
          <h3 style={{ ...sectionH, fontSize: 13, margin: "0 0 10px" }}>{tr("Temporadas cerradas")}</h3>
          <div style={{ display: "grid", gap: 8, marginBottom: 14 }}>
            {hist.map((h) => (
              <div key={h.id} style={{ ...card, padding: 13, display: "flex", alignItems: "center", gap: 11 }}>
                <span style={{ fontSize: 17 }}>🏆</span>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ color: C.textHi, fontWeight: 700, fontSize: 13.5 }}>{h.nombre}</div>
                  <div style={{ color: C.textLo, fontSize: 11.5 }}>{fmtDate(h.inicio)} → {fmtDate(h.fin)}</div>
                </div>
                <div style={{ textAlign: "right" }}>
                  <div style={{ color: C.yellow, fontWeight: 900, fontSize: 13 }}>{tri("Nv {n}", { n: h.nivelFin || h.nivelIni || 1 })}</div>
                  <div style={{ color: C.good, fontSize: 11, fontWeight: 700 }}>+{Math.max(0, Number(h.xpFin || 0) - Number(h.xpIni || 0))} XP</div>
                </div>
              </div>
            ))}
          </div>
      ...
      <div style={{ height: 100 }} />
      <Sheet open={sheet} onClose={() => setSheet(false)} title={tr("Nueva temporada")}>
        <TemporadaForm onSave={abrir} />
      </Sheet>
    </div>
  );
}

function TemporadaForm({ onSave }) {
  const [nombre, setNombre] = useState("");
  const [dias, setDias] = useState(30);
  return (
    <div>
      <Field label={tr("¿Cómo la llamas?")}>
        <input style={inputStyle} value={nombre} onChange={(e) => setNombre(e.target.value)} placeholder={tr("Ej. Verano en forma, Ahorrar para el coche…")} autoFocus />
      </Field>
      <Field label={tr("¿Cuánto dura?")}>
        <div style={{ display: "grid", gap: 8 }}>
          {TEMP_DURACIONES.map((d) => (
            <button key={d.k} onClick={() => setDias(d.k)} style={{ ...filaDentro(dias === d.k, C.yellow), padding: "11px 13px", cursor: "pointer", textAlign: "left" }}>
              <div style={{ color: C.textHi, fontWeight: 800, fontSize: 14 }}>{tr(d.n)}</div>
              <div style={{ color: C.textLo, fontSize: 12 }}>{tr(d.d)}</div>
            </button>
          ))}
        </div>
      </Field>
      <PrimaryBtn full onClick={() => nombre.trim() && onSave(dias, nombre)}>
        <Check size={18} strokeWidth={3} /> {tr("Empezar")}
      </PrimaryBtn>
    </div>
  );
}
...
/* ============================================================
   CLAN Y CLASIFICACIÓN — competir sin montar un servidor
   Aquí no hay backend: cada uno genera su CARTA (un código) y la pasa por
   WhatsApp. Al pegarla, entra en la clasificación del clan. Es exactamente el
   mismo truco que ya usan los Grupos de gastos, y tiene una ventaja seria: los
   datos de tus amigos no viven en ningún servidor mío.
   El bote es en Brosin Coins, nunca en euros: un bote con dinero real es juego
   de azar y necesita licencia. Eso es una conversación para el gestor.
   ============================================================ */
function miCarta(state) {
  const g = nivelGlobal(state);
  const r = rpgDe(state);
  const rachas = (state.habits || []).map((h) => streakOf(new Set(h.done || [])));
  return {
    id: state.profile.deviceId || (state.profile.name || "yo"),
    nombre: state.profile.name || tr("Yo"),
    nivel: g.nivel, xp: g.total, coins: r.coins || 0,
    racha: rachas.length ? Math.max(...rachas) : 0,
    at: nowISO(),
  };
}

function ClanPanel({ state, dispatch, toast }) {
  const [sheet, setSheet] = useState(null); // 'nuevo' | 'pegar' | 'carta'
  const clan = (state.clanes || [])[0] || null;
  const yo = miCarta(state);

  const gente = clan ? [yo, ...(clan.miembros || []).filter((m) => m.id !== yo.id)] : [yo];
  const tabla = [...gente].sort((a, b) => (b.nivel - a.nivel) || (b.xp - a.xp) || (b.racha - a.racha));
  const bote = tabla.reduce((s, m) => s + Number(m.apuesta || 0), 0);

  const pegar = (code) => {
    let obj = null;
    try { obj = decodeShare(code); } catch (e) {}
    if (!obj || obj.kind !== "brosin-carta" || !obj.carta) { toast && toast("✗", tr("Ese código no es una carta de Brosin")); return; }
    if (!clan) { toast && toast("✗", tr("Crea tu clan primero")); return; }
    const otros = (clan.miembros || []).filter((m) => m.id !== obj.carta.id);
    dispatch({ type: "update", col: "clanes", id: clan.id, patch: { miembros: [...otros, obj.carta] } });
    setSheet(null);
    toast && toast(tr("Dentro del clan"), tri("{n} ya está en la clasificación.", { n: obj.carta.nombre }));
  };
    ...
    <div>
      {!clan ? (
        <div style={{ ...card, padding: 18, marginBottom: 14, textAlign: "center" }}>
          ...
          <div style={{ color: C.textHi, fontWeight: 900, fontSize: 16, marginBottom: 5 }}>{tr("Sin clan")}</div>
          <div style={{ color: C.textLo, fontSize: 13, lineHeight: 1.5, marginBottom: 14 }}>
            {tr("Monta un clan con tus colegas: cada uno pasa su carta y veis quién va por delante. Sin servidor, sin cuentas, sin que nadie guarde vuestros datos.")}
          </div>
          <PrimaryBtn full onClick={() => setSheet("nuevo")}><Plus size={17} strokeWidth={3} /> {tr("Crear clan")}</PrimaryBtn>
        </div>
      ) : (
        <>
          <div style={{ ...card, padding: 16, marginBottom: 12 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
              ...
              <span style={{ flex: 1, color: C.textHi, fontWeight: 900, fontSize: 16 }}>{clan.nombre}</span>
              <button onClick={() => dispatch({ type: "remove", col: "clanes", id: clan.id })} style={{ ...iconBtn, width: 30, height: 30, color: C.textLo }}><Trash2 size={13} /></button>
            </div>
            <div style={{ color: C.textLo, fontSize: 12 }}>{tri("{n} en la clasificación", { n: tabla.length })}{bote > 0 ? tri(" · bote de {b} monedas", { b: bote }) : ""}</div>
          </div>
          ...
          <div style={{ display: "grid", gap: 8, marginBottom: 12 }}>
            {tabla.map((m, i) => {
              ...
              return (
                <div key={m.id + i} style={{ ...filaDentro(soyYo, C.yellow), padding: "11px 13px", display: "flex", alignItems: "center", gap: 11 }}>
                  <span style={{ width: 24, textAlign: "center", fontSize: i < 3 ? 17 : 13, fontWeight: 900, color: C.textLo }}>{medalla}</span>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ color: C.textHi, fontWeight: soyYo ? 900 : 700, fontSize: 14 }}>{m.nombre}{soyYo ? tr(" (tú)") : ""}</div>
                    <div style={{ color: C.textLo, fontSize: 11.5 }}>{tri("{x} XP · racha de {r}", { x: m.xp, r: m.racha })}</div>
                  </div>
                  <span style={{ color: C.yellow, fontWeight: 900, fontSize: 13.5 }}>{tri("Nv {n}", { n: m.nivel })}</span>
                </div>
              );
            })}
          </div>

          <div style={{ color: C.textLo, fontSize: 11.5, lineHeight: 1.45, marginBottom: 12 }}>
            {tr("Las cartas son una foto del momento: pídeles una nueva de vez en cuando para que la clasificación esté al día.")}
          </div>

          <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
            <button onClick={() => setSheet("carta")} style={miniBtn(C.yellow, C.ink)}>{tr("Mi carta")}</button>
            <button onClick={() => setSheet("pegar")} style={miniBtn(C.ink3, C.textHi)}>{tr("Pegar una carta")}</button>
          </div>
        </>
      )}
      ...
      <Sheet open={sheet === "nuevo"} onClose={() => setSheet(null)} title={tr("Nuevo clan")}>
        <SimpleNameForm placeholder={tr("Ej. Los de siempre")} onSave={(nombre) => { dispatch({ type: "add", col: "clanes", item: { id: uid(), nombre, miembros: [] } }); setSheet(null); }} />
      </Sheet>
      <Sheet open={sheet === "carta"} onClose={() => setSheet(null)} title={tr("Tu carta")}>
        <CartaPanel carta={yo} toast={toast} />
      </Sheet>
      <Sheet open={sheet === "pegar"} onClose={() => setSheet(null)} title={tr("Pegar la carta de un amigo")}>
        <ImportGroupForm onImport={pegar} />
      </Sheet>
    </div>
  );
}

function CartaPanel({ carta, toast }) {
  const code = encodeShare({ kind: "brosin-carta", v: 1, carta });
  const [copiado, setCopiado] = useState(false);
  const copiar = () => { try { navigator.clipboard && navigator.clipboard.writeText(code); setCopiado(true); setTimeout(() => setCopiado(false), 2000); } catch (e) {} };
  return (
    <div>
      <div style={{ ...card, padding: 16, marginBottom: 12, textAlign: "center" }}>
        <div style={{ color: C.textHi, fontWeight: 900, fontSize: 18 }}>{carta.nombre}</div>
        <div style={{ color: C.yellow, fontWeight: 900, fontSize: 26, margin: "4px 0" }}>{tri("Nv {n}", { n: carta.nivel })}</div>
        <div style={{ color: C.textLo, fontSize: 12.5 }}>{tri("{x} XP · racha de {r} · {c} monedas", { x: carta.xp, r: carta.racha, c: carta.coins })}</div>
      </div>
      <div style={{ ...card, padding: 12, background: C.well, maxHeight: 120, overflow: "auto", marginBottom: 12 }}>
        <code style={{ color: C.yellow, fontSize: 11, wordBreak: "break-all", fontFamily: "monospace" }}>{code}</code>
      </div>
      <PrimaryBtn full onClick={copiar}>
        {copiado ? <><Check size={18} strokeWidth={3} /> {tr("Copiado")}</> : <><Copy size={18} /> {tr("Copiar y mandar")}</>}
      </PrimaryBtn>
    </div>
  );
}
...
function challengeStats(c) {
  const people = c.people && c.people.length ? c.people : ["Yo"];
  const done = c.done || {};
  const total = Number(c.days || 21);
  const dias = [];
  for (let i = 0; i < total; i++) { const d = isoPlus(c.start, i); if (d) dias.push(d); }
  const t = todayISO();
  const doneBy = (p, d) => (done[p] || []).includes(d);
  const allDone = (d) => people.every((p) => doneBy(p, d));
  const pasados = dias.filter((d) => d <= t);
  const completos = pasados.filter(allDone).length;
  let racha = 0;
  for (let i = pasados.length - 1; i >= 0; i--) {
    if (allDone(pasados[i])) racha++;
    else if (i === pasados.length - 1) continue; // hoy todavía se puede salvar
    else break;
  }
  const porPersona = people.map((p) => ({ p, n: pasados.filter((d) => doneBy(p, d)).length }));
  const restantes = dias.filter((d) => d > t).length;
  const terminado = dias.length > 0 && t > dias[dias.length - 1];
  return { people, dias, pasados, completos, racha, porPersona, restantes, terminado, total: dias.length, hoy: t, doneBy, allDone };
}

function ChallengesPanel({ state, dispatch, onAdd }) {
  const list = state.challenges || [];
  if (list.length === 0) return <EmptyState face="happy" title={tr("Retos con amigos")} sub={tr("Un reto de 21 o 30 días con quien tú quieras. La racha solo sube si lo cumplís todos ese día — si uno falla, falla el grupo.")} cta={tr("Nuevo reto")} onCta={onAdd} />;
  return <div className="bstagger" style={{ display: "grid", gap: 12 }}>{list.map((c) => <ChallengeCard key={c.id} c={c} dispatch={dispatch} />)}</div>;
}

function ChallengeCard({ c, dispatch }) {
  const s = challengeStats(c);
  const done = c.done || {};
  const toggle = (p, d) => {
    const set = new Set(done[p] || []);
    if (set.has(d)) set.delete(d); else set.add(d);
    dispatch({ type: "update", col: "challenges", id: c.id, patch: { done: { ...done, [p]: [...set] } } });
  };
  const pct = s.total ? Math.round((s.completos / s.total) * 100) : 0;
  const hoyEnRango = s.dias.includes(s.hoy);
  const logrado = s.completos >= s.total && s.total > 0;
  return (
    <div style={{ ...card, padding: 16, borderColor: logrado ? C.good : C.line }}>
      <div style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
        <div style={{ width: 38, height: 38, borderRadius: 11, background: "rgba(144,19,254,.15)", display: "grid", placeItems: "center", flexShrink: 0 }}><Users size={18} color={C.purple} /></div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ color: C.textHi, fontWeight: 800, fontSize: 15.5 }}>{c.name}</div>
          <div style={{ color: C.textLo, fontSize: 12 }}>{tri("Reto de {n} días", { n: s.total })} · {s.people.join(" · ")}</div>
          <CreatedAt at={c.createdAt} />
        </div>
        <button onClick={() => dispatch({ type: "remove", col: "challenges", id: c.id })} style={{ ...iconBtn, width: 30, height: 30, color: C.textLo }}><Trash2 size={14} /></button>
      </div>

      <div style={{ ...card, padding: "12px 14px", background: C.well, boxShadow: C.wellShadow, marginTop: 12, textAlign: "center" }}>
        <div style={{ color: C.textLo, fontSize: 10.5, fontWeight: 700, letterSpacing: 1.5 }}>{tr("RACHA CONJUNTA")}</div>
        <div style={{ color: s.racha > 0 ? C.purple : C.textLo, fontWeight: 900, fontSize: 30, lineHeight: 1.1 }}>{s.racha}</div>
        <div style={{ color: C.textLo, fontSize: 11 }}>{s.racha === 1 ? tr("día seguido cumpliendo todos") : tr("días seguidos cumpliendo todos")}</div>
      </div>

      <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 12 }}>
        <div style={{ flex: 1, height: 10, borderRadius: 999, background: C.rail, boxShadow: "0 1px 3px rgba(0,0,0,.2) inset", overflow: "hidden" }}>
          <div style={{ width: `${pct}%`, height: "100%", background: logrado ? "linear-gradient(180deg,#5fe89c,#2eb96b)" : "linear-gradient(90deg,#a844ff,#8210e6)", borderRadius: 999, transition: "width .4s" }} />
        </div>
        <span style={{ color: logrado ? C.good : C.purple, fontWeight: 900, fontSize: 12.5 }}>{s.completos}/{s.total}</span>
      </div>

      {logrado ? (
        <div style={{ color: C.good, fontWeight: 800, fontSize: 13, marginTop: 10, textAlign: "center" }}>{tr("¡Reto completado! 🎉")}</div>
      ) : s.terminado ? (
        <div style={{ color: C.textLo, fontSize: 12.5, marginTop: 10, textAlign: "center" }}>{tri("Se acabó el plazo con {n} días conjuntos.", { n: s.completos })}</div>
      ) : !hoyEnRango ? (
        <div style={{ color: C.textLo, fontSize: 12.5, marginTop: 10, textAlign: "center" }}>{tri("Empieza el {d}", { d: fmtDate(c.start) })}</div>
      ) : (
        <div style={{ marginTop: 12 }}>
          <div style={{ color: C.textLo, fontSize: 11, fontWeight: 700, marginBottom: 6 }}>{tr("HOY")}</div>
          <div style={{ display: "grid", gap: 6 }}>
            {s.people.map((p) => {
              const hecho = s.doneBy(p, s.hoy);
              return (
                <button key={p} onClick={() => toggle(p, s.hoy)} style={{ display: "flex", alignItems: "center", gap: 10, width: "100%", background: hecho ? "rgba(144,19,254,.14)" : C.ink3, border: `1px solid ${hecho ? C.purple : C.line}`, borderRadius: 12, padding: "9px 12px", cursor: "pointer" }}>
                  <div style={{ width: 22, height: 22, borderRadius: 7, flexShrink: 0, border: `2px solid ${hecho ? C.purple : C.line}`, background: hecho ? C.purple : "transparent", display: "grid", placeItems: "center" }}>
                    {hecho && <Check size={13} color="#fff" strokeWidth={3.5} />}
                  </div>
                  <span style={{ flex: 1, textAlign: "left", color: hecho ? C.textHi : C.textLo, fontWeight: 700, fontSize: 13.5 }}>{p === "Yo" ? tr("Yo") : p}</span>
                  <span style={{ color: C.textLo, fontSize: 11.5, fontWeight: 700 }}>{(s.porPersona.find((x) => x.p === p) || { n: 0 }).n}/{s.total}</span>
                </button>
              );
            })}
          </div>
...
function ChallengeForm({ onSave }) {
  ...
  return (
    <div>
      <Field label={tr("¿Qué reto?")}>
        <input style={inputStyle} value={name} onChange={(e) => setName(e.target.value)} placeholder={tr("Ej. Gimnasio, leer 20 min, sin azúcar…")} autoFocus />
      </Field>
      <Field label={tr("¿Cuántos días?")}>
        <div style={{ display: "flex", gap: 8 }}>
          <Chip active={days === 21} accent={C.purple} onClick={() => setDays(21)}>{tr("21 días")}</Chip>
          <Chip active={days === 30} accent={C.purple} onClick={() => setDays(30)}>{tr("30 días")}</Chip>
          <Chip active={days === 66} accent={C.purple} onClick={() => setDays(66)}>{tr("66 días")}</Chip>
        </div>
      </Field>
      <Field label={tr("Empieza el")}>
        <input type="date" style={inputStyle} value={start} onChange={(e) => setStart(e.target.value)} />
      </Field>
      <Field label={tr("¿Con quién?")}>
        <input style={inputStyle} value={people} onChange={(e) => setPeople(e.target.value)} placeholder={tr("Nombres separados por comas")} />
      </Field>
      <PrimaryBtn full color={C.purple} onClick={() => name.trim() && onSave({ name: name.trim(), days, start: start || todayISO(), people: ["Yo", ...parse(people)], done: {} })}>
        <Check size={18} strokeWidth={3} color="#fff" /> <span style={{ color: "#fff" }}>{tr("Crear reto")}</span>
      </PrimaryBtn>
    </div>
  );
}
  ...
  return (
    <div style={{ ...card, padding: 16 }}>
      ...
      <button onClick={toggle} style={{ ...iconBtn, width: "100%", height: 44, marginTop: 12, gap: 8, fontWeight: 800, fontSize: 14, background: doneToday ? col : C.ink3, color: doneToday ? C.ink : C.textHi, borderColor: doneToday ? col : C.line }}>
        {doneToday ? <><Check size={16} strokeWidth={3} /> {tr("Hecho hoy")}</> : "Marcar hoy"}
      </button>
    </div>
  );
}
...
function Section({ children, empty, emptyT, emptyS, onAdd, header }) {
  if (empty) return <EmptyState title={emptyT} sub={emptyS} cta={tr("Añadir")} onCta={onAdd} />;
  ...
  return (
    <div>
      <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
        <button onClick={() => setType("gasto")} style={typeTab(type === "gasto", C.bad)}>{tr("Gasto")}</button>
        <button onClick={() => setType("ingreso")} style={typeTab(type === "ingreso", C.good)}>{tr("Ingreso")}</button>
      </div>
      <Field label={tr("Importe (€)")}><input type="number" inputMode="decimal" style={{ ...inputStyle, fontSize: 22, fontWeight: 800 }} value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="0,00" autoFocus /></Field>
      <Field label={tr("Concepto")}><input style={inputStyle} value={note} onChange={(e) => setNote(e.target.value)} placeholder={tr("¿En qué?")} /></Field>
      <Field label={tr("Categoría")}><div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>{cats.map((c) => <Chip key={c} active={category === c} onClick={() => setCategory(c)}>{c}</Chip>)}</div></Field>
      <PrimaryBtn full onClick={() => amount && onSave({ type, amount: Number(amount), note, category })}><Check size={18} strokeWidth={3} /> {tr("Guardar")}</PrimaryBtn>
    </div>
  );
}
...
function DebtForm({ initial, onSave, onDelete }) {
  ...
  const [date, setDate] = useState((initial?.date || todayISO()).slice(0, 10));
  return (
    <div>
      <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
        <button onClick={() => setDir("meDeben")} style={typeTab(dir === "meDeben", C.good)}>{tr("Me deben")}</button>
        <button onClick={() => setDir("yoDebo")} style={typeTab(dir === "yoDebo", C.bad)}>{tr("Yo debo")}</button>
      </div>
      <Field label={tr("¿Quién?")}><input style={inputStyle} value={person} onChange={(e) => setPerson(e.target.value)} placeholder={tr("Nombre")} autoFocus /></Field>
      <Field label={tr("Importe (€)")}><input type="number" inputMode="decimal" style={{ ...inputStyle, fontSize: 20, fontWeight: 800 }} value={amount} onChange={(e) => setAmount(e.target.value)} placeholder="0,00" /></Field>
      <Field label={tr("Día")}><input type="date" style={inputStyle} value={date} onChange={(e) => setDate(e.target.value)} /></Field>
      <Field label={tr("Concepto")}><input style={inputStyle} value={note} onChange={(e) => setNote(e.target.value)} placeholder={tr("Opcional")} /></Field>
      <PrimaryBtn full onClick={() => person.trim() && amount && onSave({ dir, person: person.trim(), amount: Number(amount), note, date: date || todayISO() })}><Check size={18} strokeWidth={3} /> {initial ? tr("Guardar cambios") : tr("Guardar")}</PrimaryBtn>
      {onDelete && <button onClick={onDelete} style={{ ...iconBtn, width: "100%", height: 44, marginTop: 10, gap: 8, fontWeight: 700, fontSize: 13.5, color: C.bad, borderColor: "rgba(255,92,92,.4)" }}><Trash2 size={16} /> {tr("Borrar deuda")}</button>}
    </div>
  );
}
/* Sello de alta. No pinta nada en lo creado antes de la v33, que no tiene createdAt. */
function CreatedAt({ at, style }) {
  if (!at) return null;
  return <div style={{ color: C.textLo, fontSize: 10.5, opacity: 0.75, marginTop: 3, ...style }}>{tri("Creado el {d}", { d: fmtDate(String(at).slice(0, 10)) })}</div>;
}
    ...
    <div style={{ ...card, padding: 14, opacity: d.settled ? 0.55 : 1 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ color: C.textLo, fontSize: 12 }}>{d.dir === "meDeben" ? tr("Te debe") : tr("Le debes")}{d.note ? ` · ${d.note}` : ""}</div>
          {d.date && <div style={{ color: C.textLo, fontSize: 11.5, marginTop: 2 }}>{fmtDate(String(d.date).slice(0, 10))}</div>}
      ...
      {adj ? (
        <div style={{ marginTop: 10 }}>
          <div style={{ color: C.textLo, fontSize: 11.5, marginBottom: 6 }}>{tr("Ajustar saldo: \"Abonó\" resta (te pagó parte), \"+ Sumó\" añade (debe más).")}</div>
          <div style={{ display: "flex", gap: 8 }}>
            <input type="number" inputMode="decimal" autoFocus style={{ ...inputStyle, padding: "9px 12px" }} value={amt} onChange={(e) => setAmt(e.target.value)} placeholder="€" />
            <button onClick={() => apply(-1)} style={miniBtn(C.good, C.ink)}>{tr("− Abonó")}</button>
            <button onClick={() => apply(1)} style={miniBtn(C.ink3, C.textHi)}>{tr("+ Sumó")}</button>
          </div>
          <button onClick={() => { setAdj(false); setAmt(""); }} style={{ ...iconBtn, width: "auto", paddingInline: 12, height: 32, marginTop: 8, fontSize: 12, color: C.textLo }}>{tr("Cancelar")}</button>
        </div>
      ) : (
        <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
          <button onClick={() => setAdj(true)} style={miniBtn(C.ink3, C.textHi)}>{tr("± Ajustar")}</button>
          <button onClick={() => dispatch({ type: "update", col: "debts", id: d.id, patch: { settled: !d.settled } })} style={miniBtn(d.settled ? C.line : C.yellow, d.settled ? C.textLo : C.ink)}>{d.settled ? tr("Reabrir") : tr("Saldada")}</button>
...
function InvForm({ initial, onSave, onDelete, onSell, livePrices }) {
  ...
  return (
    <div>
      <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
        <button onClick={() => setKind("manual")} style={typeTab(kind === "manual", C.yellow)}>{tr("Manual")}</button>
        <button onClick={() => setKind("crypto")} style={typeTab(kind === "crypto", C.yellow)}>{tr("Cripto (precio real)")}</button>
      </div>
      {kind === "crypto" ? (
        <>
          <Field label={tr("Activo")}>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          ...
          <div style={{ display: "flex", gap: 10 }}>
            ...
            <div style={{ flex: 1 }}><Field label={tr("Invertido (€)")}><input type="number" inputMode="decimal" style={inputStyle} value={invested} onChange={(e) => setInvested(e.target.value)} placeholder="0,00" /></Field></div>
          </div>
          <div style={{ ...card, padding: 12, marginBottom: 12, background: C.well }}>
            <div style={{ color: C.textLo, fontSize: 12, fontWeight: 700 }}>{tr("Valor en vivo")}</div>
        ...
        <>
          <Field label={tr("Nombre")}><input style={inputStyle} value={name} onChange={(e) => setName(e.target.value)} placeholder={tr("Ej. Fondo, oro, acciones…")} autoFocus /></Field>
          <div style={{ display: "flex", gap: 10 }}>
            <div style={{ flex: 1 }}><Field label={tr("Invertido (€)")}><input type="number" inputMode="decimal" style={inputStyle} value={invested} onChange={(e) => setInvested(e.target.value)} placeholder="0,00" /></Field></div>
            <div style={{ flex: 1 }}><Field label={tr("Valor hoy (€)")}><input type="number" inputMode="decimal" style={inputStyle} value={current} onChange={(e) => setCurrent(e.target.value)} placeholder="0,00" /></Field></div>
          </div>
        </>
      )}
      <PrimaryBtn full onClick={save}><Check size={18} strokeWidth={3} /> {initial ? tr("Guardar cambios") : tr("Guardar")}</PrimaryBtn>
      {onSell && initial && !initial.sold && <SellBlock initial={initial} onSell={onSell} />}
      {initial && initial.sold && (
        <div style={{ ...card, padding: 12, marginTop: 12, background: "rgba(61,220,132,.08)", borderColor: C.good }}>
          <div style={{ color: C.good, fontWeight: 800, fontSize: 13 }}>{tri("Vendida el {d} por {p}", { d: fmtDate(String(initial.soldAt || "").slice(0, 10)), p: money(initial.soldFor) })}</div>
          <div style={{ color: C.textLo, fontSize: 12, marginTop: 3 }}>{tri("Ganancia: {g}", { g: money(Number(initial.soldFor || 0) - Number(initial.invested || 0)) })}</div>
        </div>
      )}
      {onDelete && <button onClick={onDelete} style={{ ...iconBtn, width: "100%", height: 44, marginTop: 10, gap: 8, fontWeight: 700, fontSize: 13.5, color: C.bad, borderColor: "rgba(255,92,92,.4)" }}><Trash2 size={16} /> {tr("Borrar inversión")}</button>}
    </div>
  );
}

/* Vender una inversión. Lo que cobras entra como ingreso real en Movimientos,
   para que el dinero de la app cuadre con el del bolsillo. */
function SellBlock({ initial, onSell }) {
  const [abierto, setAbierto] = useState(false);
  const [precio, setPrecio] = useState("");
  const [fecha, setFecha] = useState(todayISO());
  // Qué apuntar en Movimientos. Por defecto SOLO la ganancia: si la compra nunca se
  // apuntó como gasto, meter el cobro entero infla el saldo (compras por 2800, vendes
  // por 3600 y parecería que has ingresado 3600 de la nada, cuando ganaste 800).
  const [registro, setRegistro] = useState("ganancia");
  const invertido = Number(initial.invested || 0);
  const gan = precio !== "" ? Number(precio) - invertido : null;
  if (!abierto) {
    return <button onClick={() => setAbierto(true)} style={{ ...iconBtn, width: "100%", height: 44, marginTop: 10, gap: 8, fontWeight: 700, fontSize: 13.5, color: C.good, borderColor: "rgba(61,220,132,.45)" }}><HandCoins size={16} /> {tr("Vender esta inversión")}</button>;
  }
  return (
    <div style={{ ...card, padding: 14, marginTop: 12 }}>
      <div style={{ color: C.textHi, fontWeight: 800, fontSize: 14, marginBottom: 10 }}>{tr("Vender")}</div>
      <Field label={tr("¿Por cuánto la vendes? (€)")}>
        <input type="number" inputMode="decimal" autoFocus style={inputStyle} value={precio} onChange={(e) => setPrecio(e.target.value)} placeholder="0,00" />
      </Field>
      <Field label={tr("Día de la venta")}>
        <input type="date" style={inputStyle} value={fecha} onChange={(e) => setFecha(e.target.value)} />
      </Field>
      {gan != null && !isNaN(gan) && (
        <div style={{ color: gan >= 0 ? C.good : C.bad, fontWeight: 800, fontSize: 13.5, marginBottom: 10 }}>
          {tri("Invertiste {i} · {r} de {g}", { i: money(invertido), r: gan >= 0 ? tr("ganancia") : tr("pérdida"), g: money(Math.abs(gan)) })}
        </div>
      )}
      <Field label={tr("¿Qué apunto en Movimientos?")}>
        <div style={{ display: "grid", gap: 6 }}>
          {[
            { k: "ganancia", l: tr("Solo la ganancia"), d: tr("La compra no estaba apuntada como gasto") },
            { k: "total", l: tr("El cobro entero"), d: tr("Solo si apuntaste la compra como gasto") },
            { k: "nada", l: tr("Nada"), d: tr("Que quede solo dentro de Inversiones") },
          ].map((o) => (
            <button key={o.k} onClick={() => setRegistro(o.k)} style={{ display: "flex", alignItems: "center", gap: 10, width: "100%", background: C.ink3, border: `1px solid ${registro === o.k ? C.good : C.line}`, borderRadius: 12, padding: "9px 12px", cursor: "pointer", textAlign: "left" }}>
              <div style={{ width: 18, height: 18, borderRadius: 999, flexShrink: 0, border: `2px solid ${registro === o.k ? C.good : C.line}`, background: registro === o.k ? C.good : "transparent" }} />
              <span style={{ flex: 1 }}>
                <span style={{ display: "block", color: C.textHi, fontSize: 12.5, fontWeight: 700 }}>
                  {o.l}{o.k === "ganancia" && gan != null && !isNaN(gan) ? " · " + money(Math.abs(gan)) : ""}{o.k === "total" && precio !== "" ? " · " + money(Number(precio)) : ""}
                </span>
                <span style={{ display: "block", color: C.textLo, fontSize: 10.5 }}>{o.d}</span>
              </span>
            </button>
          ))}
        </div>
      </Field>
      <PrimaryBtn full color={C.good} onClick={() => { const p = Number(precio); if (!isFinite(p) || precio === "") return; onSell({ precio: p, fecha: fecha || todayISO(), registro }); }}>
        <Check size={18} strokeWidth={3} /> {tr("Confirmar venta")}
      </PrimaryBtn>
      <button onClick={() => setAbierto(false)} style={{ ...iconBtn, width: "100%", height: 38, marginTop: 8, fontSize: 12.5, color: C.textLo }}>{tr("Cancelar")}</button>
    </div>
  );
}
...
function OrgsPanel({ state, dispatch }) {
  ...
  return (
    <div>
      {list.length === 0 ? (
        <EmptyState face="happy" title={tr("Empresas y artistas")} sub={tr("Archiva las empresas y artistas que colaboran contigo: cuánto te cuestan, cuánto beneficio sacas, los eventos que habéis hecho y tu margen. Todo editable, con gráfica.")} cta={tr("Añadir")} onCta={() => setSheet(true)} />
      ) : (
        <>
          <div style={{ ...card, padding: 16, marginBottom: 12, background: C.tintG, border: "1px solid rgba(61,220,132,.28)", boxShadow: "0 1px 0 rgba(255,255,255,.06) inset, 0 8px 22px rgba(61,220,132,.1), 0 10px 26px rgba(0,0,0,.35)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", gap: 10 }}>
              <div style={{ minWidth: 0 }}>
                <div style={{ color: C.textLo, fontSize: 11.5, fontWeight: 700, letterSpacing: 1.6, textTransform: "uppercase" }}>{tr("Beneficio total")}</div>
              ...
              <div style={{ textAlign: "right", color: C.textLo, fontSize: 12, flexShrink: 0 }}>
                <div>{tr("Ingresos")} <b style={{ color: C.textHi }}>{money(totals.income)}</b></div>
                <div>{tr("Coste")} <b style={{ color: C.textHi }}>{money(totals.cost)}</b></div>
              </div>
            </div>
          </div>
          ...
          <div style={{ ...card, padding: 16, marginBottom: 12 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}><BarChart3 size={16} color={C.yellow} /><span style={{ color: C.textHi, fontWeight: 800, fontSize: 14 }}>{view === "beneficio" ? tr("Beneficio por colaborador") : tr("Margen por colaborador")}</span></div>
              <div style={{ display: "flex", gap: 6 }}>
      ...
      <div style={{ height: 122 }} />
      <FAB onClick={() => setSheet(true)} label={tr("Empresa / artista")} />
      <Sheet open={sheet} onClose={() => setSheet(false)} title={tr("Nueva empresa o artista")}>
...
function OrgBars({ list, view }) {
  ...
  const max = Math.max(1, ...data.map((d) => d.value));
  if (!data.length) return <div style={{ color: C.textLo, fontSize: 12.5 }}>{tr("Sin datos.")}</div>;
  return (
    <div style={{ display: "grid", gap: 10 }}>
      {data.map((d, i) => (
        <div key={d.label + i}>
...
function OrgDetail({ org, dispatch, onBack }) {
  ...
  return (
    <div>
      <button onClick={onBack} style={{ ...linkBtn, marginBottom: 12 }}><ChevronLeft size={16} /> {tr("Empresas y artistas")}</button>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 8 }}>
        <MiniStat label={tr("INGRESOS")} val={money(t.income)} col={C.good} />
        <MiniStat label={tr("COSTE")} val={money(t.cost)} col={C.textHi} />
        <MiniStat label={tr("BENEFICIO")} val={`${t.profit >= 0 ? "+" : ""}${money(t.profit)}`} col={t.profit >= 0 ? C.good : C.bad} />
        <MiniStat label={tr("MARGEN")} val={t.margin == null ? "—" : `${t.margin.toFixed(0)}%`} col={C.yellow} />
      </div>
      <div style={{ ...card, padding: 12, marginBottom: 14 }}>
        <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}><span style={{ color: C.textLo, fontSize: 12, fontWeight: 700 }}>{tr("Margen de beneficio")}</span><span style={{ color: t.profit >= 0 ? C.good : C.bad, fontSize: 12.5, fontWeight: 800 }}>{t.margin == null ? "sin ingresos" : `${t.margin.toFixed(1)}%`}</span></div>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
        ...
        <button onClick={() => { setEditEvent(null); setSheet("event"); }} style={{ ...iconBtn, width: "auto", paddingInline: 12, height: 36, gap: 6, fontSize: 12.5, fontWeight: 800, background: C.yellow, color: C.ink, borderColor: C.yellow }}><Plus size={15} strokeWidth={3} /> {tr("Evento")}</button>
      </div>
      {events.length === 0 ? (
        <div style={{ ...card, padding: 16, textAlign: "center", color: C.textLo, fontSize: 13 }}>{tr("Aún no hay eventos. Añade el primero para ver el beneficio y el margen.")}</div>
      ) : (
        <div style={{ display: "grid", gap: 8 }}>
      ...
      <Sheet open={sheet === "org"} onClose={() => setSheet(null)} title={tr("Editar")}>
        <OrgForm initial={org} onSave={(d) => { save(d); setSheet(null); }} onDelete={del} />
      </Sheet>
      <Sheet open={sheet === "event"} onClose={() => { setSheet(null); setEditEvent(null); }} title={editEvent ? tr("Editar evento") : tr("Nuevo evento")}>
...
function OrgForm({ initial, onSave, onDelete }) {
  ...
  return (
    <div>
      ...
      <Field label={tr("Nombre")}><input style={inputStyle} value={name} onChange={(e) => setName(e.target.value)} placeholder={type === "artista" ? tr("Ej. DJ Nébula") : tr("Ej. Sala Aurora")} autoFocus /></Field>
      <Field label={tr("Coste fijo / cuota (opcional €)")}><input type="number" inputMode="decimal" style={inputStyle} value={baseCost} onChange={(e) => setBaseCost(e.target.value)} placeholder="0,00" /></Field>
      <Field label={tr("Notas (opcional)")}><textarea style={{ ...inputStyle, minHeight: 60, resize: "vertical" }} value={note} onChange={(e) => setNote(e.target.value)} placeholder={tr("Contacto, condiciones, lo que sea…")} /></Field>
      <PrimaryBtn full color={C.purple} onClick={() => { if (name.trim()) onSave({ name: name.trim(), type, baseCost: Number(baseCost || 0), note: note.trim() }); }}><Check size={18} strokeWidth={3} color="#fff" /> <span style={{ color: "#fff" }}>{initial ? tr("Guardar") : tr("Crear")}</span></PrimaryBtn>
      {onDelete && <button onClick={onDelete} style={{ ...iconBtn, width: "100%", height: 46, marginTop: 10, gap: 8, fontWeight: 700, fontSize: 13.5, color: C.bad }}><Trash2 size={15} /> {tr("Borrar")}</button>}
    </div>
  );
}
...
function OrgEventForm({ initial, onSave, onDelete }) {
  ...
  return (
    <div>
      <Field label={tr("Evento / colaboración")}><input style={inputStyle} value={name} onChange={(e) => setName(e.target.value)} placeholder={tr("Ej. Fiesta aniversario")} autoFocus /></Field>
      <Field label={tr("Fecha")}><input type="date" style={inputStyle} value={date} onChange={(e) => setDate(e.target.value)} /></Field>
      <div style={{ display: "flex", gap: 10 }}>
        <div style={{ flex: 1 }}><Field label={tr("Ingreso (€)")}><input type="number" inputMode="decimal" style={inputStyle} value={income} onChange={(e) => setIncome(e.target.value)} placeholder="0,00" /></Field></div>
        <div style={{ flex: 1 }}><Field label={tr("Coste (€)")}><input type="number" inputMode="decimal" style={inputStyle} value={cost} onChange={(e) => setCost(e.target.value)} placeholder="0,00" /></Field></div>
      </div>
      <div style={{ ...card, padding: "10px 12px", background: C.well, marginBottom: 12, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <span style={{ color: C.textLo, fontSize: 12.5, fontWeight: 700 }}>{tr("Beneficio de este evento")}</span>
        <span style={{ color: p >= 0 ? C.good : C.bad, fontWeight: 900, fontSize: 15 }}>{p >= 0 ? "+" : ""}{money(p)}</span>
      </div>
      <Field label={tr("Nota (opcional)")}><input style={inputStyle} value={note} onChange={(e) => setNote(e.target.value)} placeholder={tr("Detalle…")} /></Field>
      <PrimaryBtn full onClick={() => { if (name.trim() || income || cost) onSave({ name: name.trim() || "Evento", date, income: Number(income || 0), cost: Number(cost || 0), note: note.trim() }); }}><Check size={18} strokeWidth={3} /> {initial ? tr("Guardar") : tr("Añadir evento")}</PrimaryBtn>
      {onDelete && <button onClick={onDelete} style={{ ...iconBtn, width: "100%", height: 46, marginTop: 10, gap: 8, fontWeight: 700, fontSize: 13.5, color: C.bad }}><Trash2 size={15} /> {tr("Borrar evento")}</button>}
    </div>
  );
}

/* ============================================================
   OBJETOS — tu cartera de cosas que cotizan de verdad.
   Reúne las piezas de todas las colecciones que tengan fuente de precio real
   (oro/plata al peso, cartas) y las trata como una cartera: valor total, cuánto
...
function ObjetosPanel({ state, dispatch, onIrAColecciones }) {
  ...
  const [msg, setMsg] = useState(null);
  const [orden, setOrden] = useState("valor"); // valor | movimiento

  const objetos = [];
  (state.collections || []).forEach((c) => (c.items || []).forEach((it) => {
    if (it.src) objetos.push({ ...it, colId: c.id, colName: c.name });
  }));

  const total = objetos.reduce((s, o) => s + Number(o.value || 0), 0);
  const base = objetos.reduce((s, o) => s + Number(o.prev != null ? o.prev : (o.value || 0)), 0);
  const delta = total - base;
  const pct = base ? (delta / base) * 100 : 0;
  const movDe = (o) => (o.value != null && o.prev != null && o.prev !== 0) ? ((o.value - o.prev) / o.prev) * 100 : 0;
  const lista = [...objetos].sort((a, b) => orden === "valor" ? Number(b.value || 0) - Number(a.value || 0) : Math.abs(movDe(b)) - Math.abs(movDe(a)));
  const ultima = objetos.map((o) => o.pricedAt).filter(Boolean).sort().slice(-1)[0];

  const actualizarTodo = async () => {
    setBusy(true); setMsg(null);
    // copia de trabajo: si leyera del estado en cada vuelta, cada dispatch pisaría
    // el anterior dentro de la misma colección
    const trabajo = (state.collections || []).map((c) => ({ ...c, items: (c.items || []).map((x) => ({ ...x })) }));
    const tocadas = new Set();
    let ok = 0; const fallos = [];
    for (let i = 0; i < objetos.length; i++) {
      const o = objetos[i];
      setProg({ i: i + 1, n: objetos.length, nombre: o.name });
      try {
        const r = await precioDeObjeto(o);
        const col = trabajo.find((c) => c.id === o.colId);
        const it = col && col.items.find((x) => x.id === o.id);
        if (it) {
          it.prev = it.value != null ? it.value : r.value;
          it.value = r.value;
          it.history = [...(it.history || []), { t: nowISO(), v: r.value }].slice(-30);
          it.pricedAt = nowISO();
          it.priceSrc = r.fuente;
          tocadas.add(col.id);
        }
        ok++;
      } catch (e) { fallos.push(o.name + ": " + String((e && e.message) || e)); }
      // las APIs sin clave capan las peticiones seguidas
      await new Promise((res) => setTimeout(res, 700));
    }
    tocadas.forEach((id) => {
      const col = trabajo.find((c) => c.id === id);
      dispatch({ type: "update", col: "collections", id, patch: { items: col.items } });
    });
    setProg(null); setBusy(false);
    setMsg(fallos.length
      ? { ok: false, t: tri("{a} de {b} actualizados. No pude con: {f}", { a: ok, b: objetos.length, f: fallos.slice(0, 2).join(" · ") }) }
      : { ok: true, t: tri("{n} objetos actualizados", { n: ok }) });
  };

  if (objetos.length === 0) {
    return <EmptyState face="neutral" title={tr("Aún no cotiza nada")}
      sub={tr("Entra en una pieza de tus colecciones, dale al lápiz y elige de dónde sale su precio: oro o plata al peso, carta Pokémon o carta Magic. Las que tengan fuente real aparecerán aquí.")}
      cta={tr("Ir a Colecciones")} onCta={onIrAColecciones} />;
  }

  return (
    <div>
      <div style={{ ...card, padding: 16, marginBottom: 12 }}>
        <div style={{ color: C.textLo, fontSize: 10.5, fontWeight: 700, letterSpacing: 1.5 }}>{tr("LO QUE VALE TODO")}</div>
        <div style={{ color: C.textHi, fontWeight: 900, fontSize: 30, lineHeight: 1.1, letterSpacing: -0.8 }}>{money(total)}</div>
        <div style={{ display: "flex", alignItems: "center", gap: 6, marginTop: 4 }}>
          <span style={{ color: delta > 0 ? C.good : delta < 0 ? C.bad : C.textLo, fontWeight: 800, fontSize: 13.5 }}>
            {delta > 0 ? "+" : ""}{money(delta)}{base ? ` (${delta >= 0 ? "+" : ""}${pct.toFixed(1)}%)` : ""}
          </span>
          <span style={{ color: C.textLo, fontSize: 11.5 }}>{tr("desde la última actualización")}</span>
        </div>
        {ultima && <div style={{ color: C.textLo, fontSize: 11, marginTop: 6 }}>{tri("Precios de {d}", { d: fmtDateTime ? fmtDateTime(ultima) : fmtDate(String(ultima).slice(0, 10)) })}</div>}
      </div>

      <button onClick={actualizarTodo} disabled={busy} style={{ ...iconBtn, width: "100%", height: 44, marginBottom: 12, gap: 8, fontWeight: 800, fontSize: 13.5, background: busy ? C.ink3 : C.yellow, color: busy ? C.textLo : C.ink, borderColor: busy ? C.line : C.yellow }}>
        <span style={{ display: "inline-flex", animation: busy ? "bspin 1s linear infinite" : "none" }}><RefreshCcw size={16} /></span>
        {busy && prog ? tri("Actualizando {i}/{n}…", { i: prog.i, n: prog.n }) : tr("Actualizar precios")}
      </button>
      {msg && <div style={{ color: msg.ok ? C.good : C.bad, fontSize: 12, fontWeight: 700, marginBottom: 12 }}>{msg.t}</div>}

      <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
        <Chip active={orden === "valor"} onClick={() => setOrden("valor")}>{tr("Por valor")}</Chip>
        <Chip active={orden === "movimiento"} onClick={() => setOrden("movimiento")}>{tr("Los que más se mueven")}</Chip>
      </div>
      ...
      <div style={{ display: "grid", gap: 10 }}>
        {lista.map((o) => {
          ...
          const sube = mv > 0;
          return (
            <div key={o.colId + ":" + o.id} style={{ ...card, padding: 13, display: "flex", alignItems: "center", gap: 12 }}>
              {o.photo ? <Thumb src={o.photo} size={42} radius={11} /> : <div style={{ width: 42, height: 42, borderRadius: 11, background: C.ink3, display: "grid", placeItems: "center", flexShrink: 0 }}><Star size={17} color={C.yellow} /></div>}
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ color: C.textHi, fontWeight: 700, fontSize: 14, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{o.name}</div>
                <div style={{ color: C.textLo, fontSize: 11.5, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{o.colName}{o.priceSrc ? " · " + o.priceSrc : ""}</div>
              </div>
              {(o.history || []).length > 1 && <Sparkline history={o.history} color={mv === 0 ? C.textLo : sube ? C.good : C.bad} w={48} h={24} />}
              <div style={{ textAlign: "right", flexShrink: 0 }}>
                <div style={{ color: C.yellow, fontWeight: 800, fontSize: 14.5 }}>{o.value != null ? money(o.value) : "—"}</div>
                {mv !== 0 && (
                  <div style={{ color: sube ? C.good : C.bad, fontSize: 11.5, fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 2 }}>
                    {sube ? <ArrowUp size={11} /> : <ArrowDown size={11} />}{Math.abs(mv).toFixed(1)}%
                  </div>
                )}
              </div>
            </div>
    ...
    <div style={{ padding: soloTab ? 0 : "20px 18px 0" }}>
      ...
      {tab === "colecciones" && (
        activeCol ? (
          ...
          <EmptyState face="happy" title={tr("Empieza tu colmena")} sub={tr("Crea colecciones para todo lo que guardas: vinilos, cartas, monedas, sneakers… lo que sea.")} cta={tr("Nueva colección")} onCta={() => setSheet("col")} />
        ) : (
          <div className="bstagger" style={{ display: "grid", gap: 12 }}>
            {state.collections.length > 1 && state.collections.some((c) => (c.items || []).some((i) => Number(i.value))) && (
              <HBars
                icon={BarChart3}
                title={tr("Valor por colección")}
      ...
      {tab === "fiados" && (
        state.fiados.length === 0 ? (
          <EmptyState face="neutral" title={tr("Nada prestado")} sub={tr("Apunta lo que dejas a alguien y a quién, para que nunca se te pierda.")} cta={tr("Apuntar fiado")} onCta={() => setSheet("fiado")} />
        ) : (
          <div style={{ display: "grid", gap: 10 }}>
              ...
              <div key={f.id} style={{ ...card, padding: 14, opacity: f.returned ? 0.5 : 1 }}>
                ...
                <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
                  <button onClick={() => dispatch({ type: "update", col: "fiados", id: f.id, patch: { returned: !f.returned } })} style={miniBtn(f.returned ? C.line : C.yellow, f.returned ? C.textLo : C.ink)}>
                    {f.returned ? tr("Devuelto ✓") : tr("Marcar devuelto")}
                  </button>
      ...
      <Sheet open={sheet === "col"} onClose={() => setSheet(null)} title={tr("Nueva colección")}>
        <SimpleNameForm placeholder={tr("Ej. Vinilos, sneakers, cartas…")} onSave={(name) => { dispatch({ type: "add", col: "collections", item: { id: uid(), name, items: [] } }); setSheet(null); }} />
      </Sheet>
      <Sheet open={sheet === "item"} onClose={() => setSheet(null)} title={tr("Añadir pieza")}>
        <ItemForm onSave={(d) => {
          const c = state.collections.find((c) => c.id === activeCol);
...
function ColDetail({ col, onBack, onAddItem, onDelItem, onUpdateItem, onRename, onDeleteCol }) {
  ...
  return (
    <div>
      <button onClick={onBack} style={{ ...linkBtn, marginBottom: 12 }}><ChevronLeft size={16} /> {tr("Colecciones")}</button>
      {renaming ? (
        <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
          <input autoFocus style={inputStyle} value={nm} onChange={(e) => setNm(e.target.value)} placeholder={tr("Nombre de la colección")} />
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 14, gap: 8 }}>
          ...
          <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
            <button onClick={() => { setNm(col.name); setRenaming(true); }} title={tr("Renombrar")} style={{ ...iconBtn, width: 44, height: 44 }}><Pencil size={17} /></button>
            <button onClick={() => { if (typeof window === "undefined" || window.confirm(`¿Borrar la colección "${col.name}" y todas sus piezas?`)) onDeleteCol(); }} title={tr("Borrar colección")} style={{ ...iconBtn, width: 44, height: 44, color: C.bad }}><Trash2 size={17} /></button>
      ...
      {(col.items || []).length === 0 ? (
        <EmptyState title={tr("Colección vacía")} sub={tr("Añade la primera pieza de esta colección.")} cta={tr("Añadir pieza")} onCta={onAddItem} />
      ) : (
        <div style={{ display: "grid", gap: 10 }}>
...
function CollectionItemCard({ item, onUpdate, onDel }) {
  ...
  // Editar la pieza. Antes había que borrarla y volver a crearla, perdiendo el histórico.
  const [editInfo, setEditInfo] = useState(false);
  const [eName, setEName] = useState(item.name || "");
  const [eNote, setENote] = useState(item.note || "");
  const [ePhoto, setEPhoto] = useState(item.photo || null);
  const [eSrc, setESrc] = useState(item.src || "");
  const [eRef, setERef] = useState(item.ref || {});
  const setRef = (k, v) => setERef((r) => ({ ...r, [k]: v }));
  const abrirEdicion = () => { setEName(item.name || ""); setENote(item.note || ""); setEPhoto(item.photo || null); setESrc(item.src || ""); setERef(item.ref || {}); setEditInfo(true); };
  const guardarEdicion = () => { if (!eName.trim()) return; onUpdate({ name: eName.trim(), note: eNote, photo: ePhoto, src: eSrc, ref: eRef }); setEditInfo(false); };
  ...
  const conFuente = !!item.src && item.src !== "ia";
  const estimate = async () => {
    setAiBusy(true); setAiMsg(null);
    try {
      const r = await precioDeObjeto({ ...item, src: item.src || "ia" });
      setValue(r.value);
      onUpdate({ pricedAt: nowISO(), priceSrc: r.fuente });
      setAiMsg({ ok: true, text: r.fuente + " · " + money(r.value) });
    } catch (e) {
      setAiMsg({ ok: false, text: String((e && e.message) || e) });
    } finally { setAiBusy(false); }
  };
  return (
    <div style={{ ...card, padding: 14 }}>
      ...
      {editing ? (
        <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
          <input type="number" inputMode="decimal" autoFocus style={{ ...inputStyle, padding: "9px 12px" }} value={val} onChange={(e) => setVal(e.target.value)} placeholder={tr("Nuevo valor en €")} />
        ...
        <div style={{ marginTop: 10 }}>
          <Field label={tr("Foto (opcional)")}><PhotoPicker value={ePhoto} onChange={setEPhoto} /></Field>
          <Field label={tr("Pieza")}><input style={inputStyle} value={eName} onChange={(e) => setEName(e.target.value)} placeholder={tr("Nombre de la pieza")} autoFocus /></Field>
          <Field label={tr("Nota / detalle")}><input style={inputStyle} value={eNote} onChange={(e) => setENote(e.target.value)} placeholder={tr("Estado, edición, año…")} /></Field>

          <Field label={tr("¿De dónde sale el precio?")}>
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
              {FUENTES_PRECIO.map((f) => <Chip key={f.k} active={eSrc === f.k} onClick={() => setESrc(f.k)}>{tr(f.l)}</Chip>)}
            </div>
          </Field>
          {eSrc === "metal" && (
            <>
              <div style={{ display: "flex", gap: 6, marginBottom: 10 }}>
                <Chip active={(eRef.metal || "XAU") === "XAU"} onClick={() => setRef("metal", "XAU")}>{tr("Oro")}</Chip>
                <Chip active={eRef.metal === "XAG"} onClick={() => setRef("metal", "XAG")}>{tr("Plata")}</Chip>
              </div>
              <Field label={tr("Gramos")}><input type="number" inputMode="decimal" style={inputStyle} value={eRef.grams || ""} onChange={(e) => setRef("grams", Number(e.target.value))} placeholder="100" /></Field>
              <Field label={tr("Ley")}>
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                  {LEYES.map((l) => <Chip key={l.k} active={(eRef.purity != null ? eRef.purity : 0.999) === l.k} onClick={() => setRef("purity", l.k)}>{l.l}</Chip>)}
                </div>
              </Field>
            </>
          )}
          {eSrc === "pokemon" && (
            <Field label={tr("Id de la carta")}>
              <input style={inputStyle} value={eRef.cardId || ""} onChange={(e) => setRef("cardId", e.target.value.trim())} placeholder="base1-4" />
            </Field>
          )}
          {eSrc === "mtg" && (
            <>
              <Field label={tr("Nombre exacto en inglés")}><input style={inputStyle} value={eRef.name || ""} onChange={(e) => setRef("name", e.target.value)} placeholder="Black Lotus" /></Field>
              <Field label={tr("Edición (opcional)")}><input style={inputStyle} value={eRef.set || ""} onChange={(e) => setRef("set", e.target.value.trim())} placeholder="2ed" /></Field>
            </>
          )}

          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={guardarEdicion} style={miniBtn(C.yellow, C.ink)}>{tr("Guardar")}</button>
            <button onClick={() => setEditInfo(false)} style={miniBtn(C.ink3, C.textLo)}>{tr("Cancelar")}</button>
          </div>
        </div>
      ) : (
        <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
          <button onClick={() => setEditing(true)} style={miniBtn(C.ink3, C.textHi)}>{tr("Actualizar valor")}</button>
          <button onClick={estimate} disabled={aiBusy} style={miniBtn(conFuente ? "rgba(61,220,132,.18)" : "rgba(144,19,254,.18)", conFuente ? C.good : C.purple)}>{aiBusy ? tr("Buscando…") : conFuente ? tr("Precio real") : tr("Estimar con IA")}</button>
          <button onClick={abrirEdicion} title={tr("Editar pieza")} style={{ ...iconBtn, width: 38, height: 38, color: C.textLo }}><Pencil size={14} /></button>
          <button onClick={onDel} style={{ ...iconBtn, width: 38, height: 38, color: C.textLo }}><Trash2 size={14} /></button>
        </div>
      )}
      {aiMsg && (
        <div style={{ marginTop: 8, fontSize: 11.5, fontWeight: 700, color: aiMsg.ok ? C.purple : C.bad }}>{aiMsg.text}</div>
      )}
    </div>
  );
}
...
function SimpleNameForm({ onSave, placeholder }) {
  ...
  return (
    <div>
      <Field label={tr("Nombre")}><input style={inputStyle} value={v} onChange={(e) => setV(e.target.value)} placeholder={placeholder} autoFocus /></Field>
      <PrimaryBtn full onClick={() => v.trim() && onSave(v.trim())}><Check size={18} strokeWidth={3} /> {tr("Crear")}</PrimaryBtn>
    </div>
  );
}
function ItemForm({ onSave }) {
  ...
  return (
    <div>
      <Field label={tr("Foto (opcional)")}><PhotoPicker value={photo} onChange={setPhoto} /></Field>
      <Field label={tr("Pieza")}><input style={inputStyle} value={name} onChange={(e) => setName(e.target.value)} placeholder={tr("Nombre de la pieza")} autoFocus /></Field>
      <Field label={tr("Nota / detalle")}><input style={inputStyle} value={note} onChange={(e) => setNote(e.target.value)} placeholder={tr("Estado, edición, año…")} /></Field>
      <Field label={tr("Valor estimado (€)")}><input type="number" inputMode="decimal" style={inputStyle} value={value} onChange={(e) => setValue(e.target.value)} placeholder={tr("Opcional")} /></Field>
      <PrimaryBtn full onClick={() => name.trim() && onSave({ name: name.trim(), note, value: Number(value || 0), photo })}><Check size={18} strokeWidth={3} /> {tr("Añadir")}</PrimaryBtn>
    </div>
  );
}
function FiadoForm({ initial, onSave, onDelete }) {
  ...
  return (
    <div>
      <Field label={tr("¿Qué prestas?")}><input style={inputStyle} value={item} onChange={(e) => setItem(e.target.value)} placeholder={tr("Objeto")} autoFocus /></Field>
      <Field label={tr("¿A quién?")}><input style={inputStyle} value={person} onChange={(e) => setPerson(e.target.value)} placeholder={tr("Nombre")} /></Field>
      <PrimaryBtn full onClick={() => item.trim() && person.trim() && onSave({ item: item.trim(), person: person.trim() })}><Check size={18} strokeWidth={3} /> {initial ? tr("Guardar cambios") : tr("Guardar")}</PrimaryBtn>
      {onDelete && <button onClick={onDelete} style={{ ...iconBtn, width: "100%", height: 44, marginTop: 10, gap: 8, fontWeight: 700, fontSize: 13.5, color: C.bad, borderColor: "rgba(255,92,92,.4)" }}><Trash2 size={16} /> {tr("Borrar fiado")}</button>}
    </div>
  );
}
  ...
  return (
    <div style={{ padding: "20px 18px 0" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <Title kicker={tr("Tu conciencia")}>{tr("Cerebro")}</Title>
        {ai && (
      ...
      {tab === "objetivos" && (
        state.goals.length === 0 ? (
          <EmptyState face="happy" title={tr("¿Hacia dónde vas?")} sub={tr("Define objetivos y divídelos en pasos. Brosin te lleva paso a paso.")} cta={tr("Nuevo objetivo")} onCta={() => setSheet("goal")} />
        ) : (
          <div className="bstagger" style={{ display: "grid", gap: 12 }}>
      ...
      {tab === "notas" && (
        state.notes.length === 0 ? (
          <EmptyState face="neutral" title={tr("Bóveda vacía")} sub={tr("Guarda aquí información valiosa: claves, ideas, lo que no quieres perder.")} cta={tr("Nueva nota")} onCta={() => setSheet("note")} />
        ) : (
          <div className="bstagger" style={{ display: "grid", gap: 10 }}>
      ...
      <div style={{ height: 122 }} />
      <FAB onClick={() => { setEditGoal(null); setSheet(tab === "notas" ? "note" : "goal"); }} label={tab === "notas" ? tr("Nota") : tr("Objetivo")} />
      ...
      <Sheet open={sheet === "note"} onClose={() => setSheet(null)} title={tr("Nueva nota")}>
...
function GoalCard({ g, dispatch, onEdit }) {
  ...
  const addStep = () => { if (!newStep.trim()) return; dispatch({ type: "update", col: "goals", id: g.id, patch: { steps: [...steps, { id: uid(), text: newStep.trim(), done: false, date: newStepDate || "", createdAt: nowISO() }] } }); setNewStep(""); setNewStepDate(""); };
  ...
  return (
    <div style={{ ...card, padding: 16 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 10 }}>
        <div style={{ flex: 1 }}>
          ...
          {g.deadline && (() => {
            ...
            const lbl = complete ? tr("conseguido") : dd < 0 ? (-dd === 1 ? tr("vencido hace 1 día") : tri("vencido hace {n} días", { n: -dd })) : dd === 0 ? tr("¡es hoy!") : (dd === 1 ? tr("falta 1 día") : tri("faltan {n} días", { n: dd }));
        ...
        <button onClick={() => dispatch({ type: "update", col: "goals", id: g.id, patch: { notif: g.notif === false } })} title={tr("Avisos de la meta")} style={{ ...iconBtn, width: 30, height: 30, color: g.notif === false ? C.textLo : C.purple, borderColor: g.notif === false ? C.line : C.purple }}><BellRing size={14} /></button>
        {onEdit && <button onClick={onEdit} title={tr("Editar objetivo")} style={{ ...iconBtn, width: 30, height: 30, color: C.textLo }}><Pencil size={14} /></button>}
      ...
      <div style={{ display: "grid", gap: 6 }}>
        {steps.map((s) => (
          <div key={s.id} style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            {s.date && (() => {
              const dd = daysFromNow(s.date);
              const col = s.done ? C.textLo : dd < 0 ? C.bad : dd === 0 ? C.yellow : dd <= 3 ? C.yellow : C.textLo;
              const lbl = dd === 0 ? tr("hoy") : dd === 1 ? tr("mañana") : fmtDate(s.date);
              return <span style={{ color: col, fontSize: 11, fontWeight: 700, whiteSpace: "nowrap", flexShrink: 0 }}>{lbl}</span>;
            })()}
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
        <input style={{ ...inputStyle, padding: "9px 12px", fontSize: 13.5, minWidth: 0 }} value={newStep} onChange={(e) => setNewStep(e.target.value)} onKeyDown={(e) => e.key === "Enter" && addStep()} placeholder={tr("Añadir paso…")} />
        <input type="date" title={tr("Día del paso")} style={{ ...inputStyle, padding: "9px 8px", fontSize: 12, width: 126, flexShrink: 0 }} value={newStepDate} onChange={(e) => setNewStepDate(e.target.value)} />
        <button onClick={addStep} style={{ ...iconBtn, width: 40, height: 40, flexShrink: 0, background: C.purple, borderColor: C.purple }}><Plus size={18} color="#fff" strokeWidth={3} /></button>
      </div>
    </div>
  );
}
...
function GoalForm({ initial, onSave }) {
  ...
  return (
    <div>
      <Field label={tr("Objetivo")}><input style={inputStyle} value={title} onChange={(e) => setTitle(e.target.value)} placeholder={tr("¿Qué quieres conseguir?")} autoFocus /></Field>
      <Field label={tr("Fecha meta (opcional)")}><input type="date" style={inputStyle} value={deadline} onChange={(e) => setDeadline(e.target.value)} /></Field>
      <Field label={tr("¿Por qué lo quieres? (opcional)")}><input style={inputStyle} value={why} onChange={(e) => setWhy(e.target.value)} placeholder={tr("Tu motivación en una frase")} /></Field>
      <button onClick={() => setNotif((n) => !n)} style={{
        ...
        <span style={{ flex: 1, textAlign: "left", color: C.textHi, fontWeight: 700, fontSize: 14 }}>{tr("Avisarme según se acerca la meta")}</span>
...
function NoteForm({ onSave }) {
  ...
  return (
    <div>
      <Field label={tr("Título")}><input style={inputStyle} value={title} onChange={(e) => setTitle(e.target.value)} placeholder={tr("Título de la nota")} autoFocus /></Field>
      <Field label={tr("Contenido")}>
        <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
          <textarea style={{ ...inputStyle, minHeight: 110, resize: "vertical" }} value={body} onChange={(e) => setBody(e.target.value)} placeholder={tr("Escribe o dicta con el micro…")} />
          <MicButton onResult={setBody} base={body} />
        </div>
      </Field>
      <Field label={tr("Foto (opcional)")}><PhotoPicker value={photo} onChange={setPhoto} /></Field>
      <PrimaryBtn full color={C.purple} onClick={() => title.trim() && onSave({ title: title.trim(), body, photo })}><Check size={18} strokeWidth={3} color="#fff" /> <span style={{ color: "#fff" }}>{tr("Guardar nota")}</span></PrimaryBtn>
    </div>
  );
}
...
function ChatSheet({ onClose, state, dispatch }) {
  ...
  const kbInset = useKeyboardInset();
  const z = useTopZ(true);

  useEffect(() => { if (scroller.current) scroller.current.scrollTop = scroller.current.scrollHeight; }, [msgs, loading]);
  // al abrirse el teclado, mantener el último mensaje a la vista
  useEffect(() => { if (scroller.current) scroller.current.scrollTop = scroller.current.scrollHeight; }, [kbInset]);
  ...
  return (
    <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.65)", zIndex: z, paddingBottom: kbInset, display: "flex", flexDirection: "column", justifyContent: "flex-end", backdropFilter: "blur(8px)", WebkitBackdropFilter: "blur(8px)" }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: `linear-gradient(180deg, ${glassTokens().sheen}, ${C.base === "light" ? "rgba(255,255,255,.72)" : "rgba(12,12,14,.62)"})`, backdropFilter: "blur(24px) saturate(1.8) brightness(1.06)", WebkitBackdropFilter: "blur(24px) saturate(1.8) brightness(1.06)", borderTopLeftRadius: 28, borderTopRightRadius: 28, borderTop: `1px solid ${glassTokens().edge}`, boxShadow: `inset 0 1px 0 ${glassTokens().hi}, 0 -18px 50px ${C.dropSheet}`, height: "88%", display: "flex", flexDirection: "column", animation: "bsheet .32s cubic-bezier(.32,1.25,.45,1)" }}>
        <div style={{ padding: "14px 18px", borderBottom: `1px solid ${C.line}`, display: "flex", alignItems: "center", gap: 12 }}>
          ...
          <div style={{ flex: 1 }}>
            <div style={{ color: C.textHi, fontWeight: 900, fontSize: 16 }}>{tr("La Mente Brosin")}</div>
            <div style={{ color: C.purple, fontSize: 11.5, fontWeight: 700 }}>{tr("Tu conciencia inteligente")}</div>
          </div>
        ...
        <div ref={scroller} style={{ flex: 1, overflowY: "auto", padding: 16, display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          {err && (
            <div style={{ alignSelf: "flex-start", background: "rgba(255,92,92,.12)", border: `1px solid ${C.bad}`, padding: "12px 14px", borderRadius: 16 }}>
              ...
              <button onClick={() => send(msgs[msgs.length - 1]?.role === "user" ? msgs[msgs.length - 1].content : null)} style={{ ...iconBtn, width: "auto", paddingInline: 12, gap: 6, fontWeight: 700, fontSize: 13 }}><RefreshCcw size={14} /> {tr("Reintentar")}</button>
            </div>
          )}
        </div>

        <div style={{ padding: "12px 14px", borderTop: `1px solid ${C.line}`, display: "flex", gap: 10, alignItems: "center" }}>
          <input style={{ ...inputStyle, borderRadius: 999 }} value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && send()} placeholder={tr("Habla con tu cerebro…")} />
  ...
  return (
    <div style={{ padding: "20px 18px 0" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
        ...
        <Title kicker={tr("Foto inteligente")}>{tr("Cámara")}</Title>
      </div>

      <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
        <Chip active={mode === "ticket"} onClick={() => { setMode("ticket"); reset(); }}>{tr("Ticket → gasto")}</Chip>
        <Chip active={mode === "comida"} onClick={() => { setMode("comida"); reset(); }} accent={C.purple}>{tr("Comida")}</Chip>
      </div>
      ...
      {mode === "comida" && meals.length > 0 && (
        <>
          <h3 style={{ ...sectionH, marginBottom: 10 }}>{tr("Lo que has comido")}</h3>
...
/* ============================================================
   PERFIL / AJUSTES
   ============================================================ */
/* Refrescar la app sin cerrarla y volver a abrirla.
   Fuerza al service worker a mirar si hay versión nueva y recarga. */
function UpdateRow() {
  const [phase, setPhase] = useState("idle"); // idle | checking | error
  useEffect(() => {
    if (!("serviceWorker" in navigator)) return;
    const onCtl = () => { try { window.location.reload(); } catch (e) {} };
    navigator.serviceWorker.addEventListener("controllerchange", onCtl);
    return () => navigator.serviceWorker.removeEventListener("controllerchange", onCtl);
  }, []);
  const check = async () => {
    setPhase("checking");
    try {
    ...
    setTimeout(() => { try { window.location.reload(); } catch (e) { setPhase("error"); } }, 700);
  };
  const build = (typeof document !== "undefined" && document.querySelector('meta[name="brosin-build"]')?.content) || "";
  return (
    <button onClick={check} disabled={phase === "checking"} style={settingRow}>
      <div style={{ width: 38, height: 38, borderRadius: 11, background: C.ink3, display: "grid", placeItems: "center" }}>
        <span style={{ display: "inline-flex", animation: phase === "checking" ? "bspin 1s linear infinite" : "none" }}><RefreshCcw size={18} color={C.yellow} /></span>
      </div>
      <span style={{ flex: 1, textAlign: "left", color: C.textHi, fontWeight: 700, fontSize: 14.5 }}>{phase === "checking" ? tr("Actualizando…") : tr("Actualizar la app")}</span>
      {build && <span style={{ color: C.textLo, fontSize: 12, fontWeight: 700, marginRight: 4 }}>{build}</span>}
      <ChevronRight size={18} color={C.textLo} />
    </button>
  );
}
...
function ProfileScreen({ state, dispatch, onChangePin, hasPin, onResetAll, onRestore, cloud }) {
  ...
  const [briefCfgOpen, setBriefCfgOpen] = useState(false);
  ...
  return (
    <div style={{ padding: "20px 18px 0" }}>
      <Title kicker={tr("Tu cuenta")}>{tr("Perfil")}</Title>
      ...
      <div style={{ display: "grid", gap: 10 }}>
        ...
        {cloud && (
          <button onClick={() => setCloudOpen(true)} style={{ ...settingRow, borderColor: cloud.user ? C.good : C.line }}>
            ...
            <span style={{ flex: 1, textAlign: "left", color: C.textHi, fontWeight: 700, fontSize: 14.5 }}>{tr("Cuenta en la nube")}</span>
        ...
        <button onClick={() => setNotifOpen(true)} style={settingRow}>
          ...
          <span style={{ flex: 1, textAlign: "left", color: C.textHi, fontWeight: 700, fontSize: 14.5 }}>{tr("Avisos y notificaciones")}</span>
        ...
        <button onClick={() => setPushOpen(true)} style={{ ...settingRow, borderColor: state.profile.pushOn ? C.good : C.line }}>
          ...
          <span style={{ flex: 1, textAlign: "left", color: C.textHi, fontWeight: 700, fontSize: 14.5 }}>{tr("Avisos en el móvil (app cerrada)")}</span>
        ...
        <button onClick={() => setLangOpen(true)} style={settingRow}>
          <div style={{ width: 38, height: 38, borderRadius: 11, background: C.ink3, display: "grid", placeItems: "center" }}><MessageCircle size={18} color={C.yellow} /></div>
          <span style={{ flex: 1, textAlign: "left", color: C.textHi, fontWeight: 700, fontSize: 14.5 }}>{tr("Idioma")}</span>
          <span style={{ color: C.textLo, fontSize: 12, fontWeight: 700, marginRight: 4 }}>{(() => { const l = LANGS.find((x) => x.k === (state.profile.lang || "auto")); return l && l.k !== "auto" ? `${l.flag} ${l.label}` : `🌐 ${CUR_LANG.toUpperCase()}`; })()}</span>
          <ChevronRight size={18} color={C.textLo} />
        </button>
        <button onClick={() => setBriefCfgOpen(true)} style={settingRow}>
          <div style={{ width: 38, height: 38, borderRadius: 11, background: "rgba(255,212,0,.13)", display: "grid", placeItems: "center" }}><Sun size={18} color={C.yellow} /></div>
          <span style={{ flex: 1, textAlign: "left", color: C.textHi, fontWeight: 700, fontSize: 14.5 }}>{tr("Tu día en 30 segundos")}</span>
          <span style={{ color: state.profile.briefOn === false ? C.textLo : C.good, fontSize: 12, fontWeight: 700, marginRight: 4 }}>{state.profile.briefOn === false ? "OFF" : (state.profile.briefTime || "08:30")}</span>
          <ChevronRight size={18} color={C.textLo} />
        </button>
        <button onClick={() => setThemeOpen(true)} style={settingRow}>
          <div style={{ width: 38, height: 38, borderRadius: 11, background: C.ink3, display: "grid", placeItems: "center" }}>{THEMES[state.profile.theme] && THEMES[state.profile.theme].base === "light" ? <Sun size={18} color={C.yellow} /> : <Moon size={18} color={C.yellow} />}</div>
          <span style={{ flex: 1, textAlign: "left", color: C.textHi, fontWeight: 700, fontSize: 14.5 }}>{tr("Tema de la app")}</span>
          <span style={{ color: C.textLo, fontSize: 12, fontWeight: 700, marginRight: 4 }}>{(() => { const cur = THEME_LIST.find((x) => x.k === state.profile.theme) || THEME_LIST[0]; return `${cur.emoji} ${tr(cur.name)}`; })()}</span>
          <ChevronRight size={18} color={C.textLo} />
        </button>
        ...
        <button onClick={() => setBackupOpen(true)} style={settingRow}>
          ...
          <span style={{ flex: 1, textAlign: "left", color: C.textHi, fontWeight: 700, fontSize: 14.5 }}>{tr("Copia de seguridad")}</span>
        ...
        <button onClick={() => setAiKeyOpen(true)} style={settingRow}>
          ...
          <span style={{ flex: 1, textAlign: "left", color: C.textHi, fontWeight: 700, fontSize: 14.5 }}>{tr("Conectar IA (app instalada)")}</span>
          {(() => {
        ...
        <button onClick={() => setFeedbackOpen(true)} style={settingRow}>
          ...
          <span style={{ flex: 1, textAlign: "left", color: C.textHi, fontWeight: 700, fontSize: 14.5 }}>{tr("Enviar feedback")}</span>
          <ChevronRight size={18} color={C.textLo} />
        </button>
        <button onClick={onResetAll} style={settingRow}>
          ...
          <span style={{ flex: 1, textAlign: "left", color: C.bad, fontWeight: 700, fontSize: 14.5 }}>{tr("Borrar todos mis datos")}</span>
          <ChevronRight size={18} color={C.textLo} />
        </button>
      </div>
      ...
      <Sheet open={notifOpen} onClose={() => setNotifOpen(false)} title={tr("Avisos y notificaciones")}>
        <NotifSettings notif={state.profile.notif || {}} onChange={(patch) => dispatch({ type: "profile", payload: { notif: { ...(state.profile.notif || {}), ...patch } } })} />
      </Sheet>

      <Sheet open={aiKeyOpen} onClose={() => setAiKeyOpen(false)} title={tr("Conectar IA")}>
        <AiKeyForm value={state.profile.aiKey || ""} onSave={(k, status) => { dispatch({ type: "profile", payload: { aiKey: k, aiStatus: status } }); }} />
      </Sheet>

      {cloud && (
        <Sheet open={cloudOpen} onClose={() => setCloudOpen(false)} title={tr("Cuenta en la nube")}>
          <CloudPanel cloud={cloud} />
        </Sheet>
      )}

      <Sheet open={backupOpen} onClose={() => setBackupOpen(false)} title={tr("Copia de seguridad")}>
      ...
      <Sheet open={feedbackOpen} onClose={() => setFeedbackOpen(false)} title={tr("Enviar feedback")}>
        <FeedbackPanel name={state.profile.name} />
      </Sheet>

      <Sheet open={langOpen} onClose={() => setLangOpen(false)} title={tr("Idioma")}>
        <div style={{ display: "grid", gap: 8, marginBottom: 14 }}>
          <button onClick={() => dispatch({ type: "profile", payload: { lang: "auto" } })} style={{ ...card, padding: "12px 14px", display: "flex", alignItems: "center", gap: 10, cursor: "pointer", borderColor: (state.profile.lang || "auto") === "auto" ? C.yellow : C.line }}>
            <span style={{ fontSize: 18 }}>🌐</span>
            <span style={{ flex: 1, textAlign: "left", color: C.textHi, fontWeight: 700, fontSize: 14 }}>{tr("Automático (idioma del móvil)")}</span>
            {(state.profile.lang || "auto") === "auto" && <Check size={16} color={C.yellow} strokeWidth={3} />}
          </button>
          {LANGS.filter((l) => l.k !== "auto").map((l) => (
            <button key={l.k} onClick={() => dispatch({ type: "profile", payload: { lang: l.k } })} style={{ ...card, padding: "12px 14px", display: "flex", alignItems: "center", gap: 10, cursor: "pointer", borderColor: state.profile.lang === l.k ? C.yellow : C.line }}>
              <span style={{ fontSize: 18 }}>{l.flag}</span>
              <span style={{ flex: 1, textAlign: "left", color: C.textHi, fontWeight: 700, fontSize: 14 }}>{l.label}</span>
              {state.profile.lang === l.k && <Check size={16} color={C.yellow} strokeWidth={3} />}
            </button>
          ))}
        </div>
        <div style={{ color: C.textLo, fontSize: 11.5, lineHeight: 1.5 }}>{tr("La interfaz principal está traducida; el resto de textos irán llegando en próximas versiones.")}</div>
      </Sheet>
      <Sheet open={briefCfgOpen} onClose={() => setBriefCfgOpen(false)} title={tr("Tu día en 30 segundos")}>
        <div style={{ color: C.textLo, fontSize: 13.5, lineHeight: 1.55, marginBottom: 14 }}>
          {tr("Cada mañana, Brosin te prepara")} <b style={{ color: C.textHi }}>{tr("tu día en una tarjeta")}</b>{tr(": agenda, hábitos para marcar al toque, dinero, cumpleaños y un consejo. Se abre sola la primera vez del día y te llega un aviso a la hora que elijas (si tienes los avisos activados).")}
        </div>
        <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
          <Chip active={state.profile.briefOn !== false} onClick={() => dispatch({ type: "profile", payload: { briefOn: true } })}>{tr("Encendido")}</Chip>
          <Chip active={state.profile.briefOn === false} onClick={() => dispatch({ type: "profile", payload: { briefOn: false } })}>{tr("Apagado")}</Chip>
        </div>
        {state.profile.briefOn !== false && (
          <Field label={tr("Hora del aviso matinal")}>
            <input type="time" style={inputStyle} value={state.profile.briefTime || "08:30"} onChange={(e) => dispatch({ type: "profile", payload: { briefTime: e.target.value || "08:30" } })} />
          </Field>
        )}
        <div style={{ color: C.textLo, fontSize: 11.5, lineHeight: 1.5 }}>{tr("También puedes abrirla cuando quieras tocando la fecha grande del inicio.")}</div>
      </Sheet>
      <Sheet open={themeOpen} onClose={() => setThemeOpen(false)} title={tr("Tema de la app")}>
        {(() => {
          const bestS = Math.max(state.profile.streakBest || 0, hiveStreak(state).count, hiveBestStreak(state));
          const curK = THEMES[state.profile.theme] ? state.profile.theme : "dark";
          return (
            <div style={{ display: "grid", gap: 8, marginBottom: 14 }}>
              {THEME_LIST.map((th) => {
                const unlocked = bestS >= th.req;
                const pal = THEMES[th.k];
                return (
                  <button key={th.k} onClick={() => { if (unlocked) dispatch({ type: "profile", payload: { theme: th.k } }); }} style={{ ...card, padding: "12px 14px", display: "flex", alignItems: "center", gap: 10, cursor: unlocked ? "pointer" : "default", opacity: unlocked ? 1 : 0.55, borderColor: curK === th.k ? C.yellow : C.line }}>
                    <span style={{ fontSize: 18 }}>{th.emoji}</span>
                    <span style={{ width: 34, height: 22, borderRadius: 7, flexShrink: 0, border: `1px solid ${pal.line}`, background: `linear-gradient(135deg, ${pal.bg0} 0%, ${pal.bg0} 45%, ${pal.ink3} 46%, ${pal.ink3} 75%, ${pal.yellow} 76%, ${pal.yellow} 100%)` }} />
                    <span style={{ flex: 1, textAlign: "left" }}>
                      <span style={{ display: "block", color: C.textHi, fontWeight: 700, fontSize: 14 }}>{tr(th.name)}</span>
                      {!unlocked && <span style={{ display: "block", color: C.textLo, fontSize: 11, fontWeight: 600, marginTop: 2 }}>🔒 {tr("Se desbloquea con")} {th.req} {tr("días de racha")}</span>}
                    </span>
                    {unlocked && curK === th.k && <Check size={16} color={C.yellow} strokeWidth={3} />}
                  </button>
                );
              })}
            </div>
        ...
        <div style={{ color: C.textLo, fontSize: 11.5, lineHeight: 1.5 }}>{tr("Consigue rachas más largas para desbloquear temas nuevos. Tu mejor racha cuenta para siempre.")}</div>
      </Sheet>
      <Sheet open={pushOpen} onClose={() => setPushOpen(false)} title={tr("Avisos en el móvil")}>
...
function PushPanel({ cloud, on, onOn }) {
  ...
  return (
    <div>
      <div style={{ color: C.textLo, fontSize: 13.5, lineHeight: 1.55, marginBottom: 14 }}>
        {tr("Activa esto para que Brosin te avise de eventos, cumpleaños y metas")} <b style={{ color: C.textHi }}>{tr("aunque tengas la app cerrada o el móvil bloqueado")}</b>{tr(". Necesitas tener la")} <b style={{ color: C.textHi }}>{tr("Cuenta en la nube")}</b> {tr("activada.")}
      </div>
      <PrimaryBtn full color={status === "ok" ? C.good : C.purple} onClick={enable}>
        {status === "working" ? <><span style={{ display: "inline-flex", animation: "bspin 1s linear infinite" }}><RefreshCcw size={18} /></span> {tr("Activando…")}</> : status === "ok" ? <><Check size={18} strokeWidth={3} /> <span>{tr("Avisos activados ✓")}</span></> : <><BellRing size={18} /> <span style={{ color: "#fff" }}>{tr("Activar avisos en el móvil")}</span></>}
      </PrimaryBtn>
      {msg && <div style={{ marginTop: 12, color: status === "ok" ? C.good : C.textLo, fontSize: 13, lineHeight: 1.5 }}>{msg}</div>}
      {status === "ok" && (
        <button onClick={enable} style={{ ...iconBtn, width: "auto", paddingInline: 14, height: 38, marginTop: 12, gap: 8, fontSize: 13, color: C.textLo }}><RefreshCcw size={14} /> {tr("Reactivar en este móvil")}</button>
      )}
      <div style={{ color: C.textLo, fontSize: 11.5, marginTop: 14, lineHeight: 1.5 }}>{tr("Consejo: haz esto en el móvil donde quieras recibir los avisos (uno por móvil). Ojo: si borras y reinstalas la app, hay que volver a activarlos.")}</div>
      <PushDoctor cloud={cloud} />
    </div>
  );
}

/* Médico de avisos: comprueba toda la cadena desde ESTE móvil y dice qué falla */
function PushDoctor({ cloud }) {
  const [rows, setRows] = useState(null); // [{label, ok, fix}]
  const [busy, setBusy] = useState(false);
  const [test, setTest] = useState(null); // null | sending | sent | err
  const run = async () => {
    setBusy(true); setRows(null); setTest(null);
    const out = [];
    const add = (label, ok, fix) => out.push({ label, ok, fix: ok ? "" : fix });
    try {
      ...
      add("Abierta como app instalada", !!standalone, "Ábrela desde su icono de la pantalla de inicio (no desde Safari). Si no la tienes: Safari → Compartir → Añadir a pantalla de inicio.");
      // 2) soporte push
      add("El móvil soporta avisos push", pushSupported(), "Este navegador no puede recibir push. En iPhone necesitas iOS 16.4 o superior y abrirla instalada.");
      // 3) permiso
      const perm = typeof Notification !== "undefined" ? Notification.permission : "unsupported";
      add("Permiso de notificaciones concedido", perm === "granted", perm === "denied" ? "Está BLOQUEADO: Ajustes del iPhone → Notificaciones → Brosin → Permitir. Luego vuelve aquí." : "Aún no lo has dado: pulsa «Activar avisos» arriba y acepta.");
      // 4) service worker
      let reg = null;
      try { reg = await navigator.serviceWorker.getRegistration(); } catch (e) {}
      add("Motor de avisos (service worker) activo", !!(reg && reg.active), "Cierra la app del todo, ábrela con internet y repite la revisión.");
      // 5) suscripción de ESTE móvil
      let sub = null;
      try { sub = reg ? await reg.pushManager.getSubscription() : null; } catch (e) {}
      add("Este móvil tiene suscripción de avisos", !!sub, "No hay suscripción (se pierde al reinstalar la app). Pulsa «Activar avisos» arriba y repite la revisión.");
      // 6) nube iniciada
      const user = cloud && cloud.user;
      add("Cuenta en la nube iniciada", !!user, "Sin nube no hay avisos: en Perfil → Cuenta en la nube, entra con tu email, y vuelve.");
      // 7-8) la nube responde + este móvil registrado + recordatorios futuros
      if (user) {
        try {
          const sb = await getSupa();
          const { data: subsData, error: e1 } = await sb.from("push_subs").select("endpoint").eq("user_id", user.id);
          if (e1) throw e1;
          add("La nube responde", true, "");
          const registered = !!(sub && (subsData || []).some((r) => r.endpoint === sub.endpoint));
          add("Este móvil está registrado en la nube", registered, "Pulsa «Activar avisos» arriba para registrarlo y repite.");
          const { count, error: e2 } = await sb.from("reminders").select("id", { count: "exact", head: true }).eq("user_id", user.id).eq("sent", false).gt("fire_at", new Date().toISOString());
          if (e2) throw e2;
          add(`Recordatorios futuros en cola (${count || 0})`, (count || 0) > 0, "No hay nada programado: crea un evento futuro con aviso, o usa el botón de prueba de abajo.");
        } catch (e) {
          add("La nube responde", false, "La base de datos no contesta. Suele ser el proyecto de Supabase dormido: entra en supabase.com → tu proyecto → botón «Restore/Resume», espera 2 min y repite.");
        }
      }
      // 9-10) servidor de brosinos.com
      try {
        const r = await fetch("/api/claude", { method: "GET" });
        const j = await r.json().catch(() => null);
        add("Servidor de la app en marcha", !!(r.ok && j && j.ok), "brosinos.com/api no responde bien. Dímelo a mí (Claude) y lo reviso en el servidor.");
      } catch (e) { add("Servidor de la app en marcha", false, "Sin conexión con brosinos.com/api. ¿Tienes internet? Si sí, dímelo y lo reviso."); }
      try {
        const r2 = await fetch("/api/pending", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ endpoint: (sub && sub.endpoint) || "diag" }) });
        const j2 = await r2.json().catch(() => null);
        add("Canal de mensajes de avisos listo", !!(r2.ok && j2 && Array.isArray(j2.items)), "El canal /api/pending falla. Dímelo y lo arreglo en el servidor.");
      } catch (e) { add("Canal de mensajes de avisos listo", false, "El canal /api/pending no responde. Dímelo y lo reviso."); }
    } catch (e) {}
    setRows(out); setBusy(false);
  };
  const sendTest = async () => {
    if (!cloud || !cloud.user) { setTest("err"); return; }
    setTest("sending");
    try {
      const sb = await getSupa();
      const { error } = await sb.from("reminders").insert({ user_id: cloud.user.id, rkey: "test:" + uid(), fire_at: new Date(Date.now() + 5000).toISOString(), title: "🐝 ¡Los avisos funcionan!", body: "Prueba enviada desde el diagnóstico de Brosinos." });
  ...
  return (
    <div style={{ marginTop: 18, borderTop: `1px solid ${C.line}`, paddingTop: 16 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
        ...
        <span style={{ color: C.textHi, fontWeight: 800, fontSize: 14, flex: 1 }}>{tr("¿No te llegan? Diagnóstico")}</span>
      </div>
      <button onClick={busy ? undefined : run} style={{ ...iconBtn, width: "100%", height: 44, gap: 8, fontWeight: 800, fontSize: 13.5, background: C.ink3, opacity: busy ? 0.6 : 1 }}>
        {busy ? <><span style={{ display: "inline-flex", animation: "bspin 1s linear infinite" }}><RefreshCcw size={15} /></span> {tr("Revisando…")}</> : <><Activity size={15} /> {tr("Revisar el sistema de avisos")}</>}
      </button>
      {rows && (
        <div style={{ marginTop: 12 }}>
          <div style={{ ...card, padding: "10px 12px", marginBottom: 10, background: fails === 0 ? "rgba(61,220,132,.1)" : "rgba(255,92,92,.08)", border: `1px solid ${fails === 0 ? C.good : C.bad}`, color: C.textHi, fontSize: 13, fontWeight: 700 }}>
            {fails === 0 ? "Todo en orden ✓ — lanza el aviso de prueba de abajo." : `${fails} problema${fails === 1 ? "" : "s"} encontrado${fails === 1 ? "" : "s"} — mira las ✗ y sigue su instrucción.`}
          </div>
          <div style={{ display: "grid", gap: 6 }}>
            {rows.map((r, i) => (
              <div key={i} style={{ ...card, padding: "9px 12px", background: C.ink }}>
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  {r.ok ? <Check size={15} color={C.good} strokeWidth={3} /> : <X size={15} color={C.bad} strokeWidth={3} />}
                  <span style={{ color: r.ok ? C.textLo : C.textHi, fontSize: 12.5, fontWeight: 700, flex: 1 }}>{r.label}</span>
                </div>
                {!r.ok && r.fix && <div style={{ color: C.textLo, fontSize: 12, marginTop: 5, lineHeight: 1.45, paddingLeft: 23 }}>{r.fix}</div>}
              </div>
            ))}
          </div>
        </div>
      )}
      <button onClick={test === "sending" ? undefined : sendTest} style={{ ...iconBtn, width: "100%", height: 44, marginTop: 12, gap: 8, fontWeight: 800, fontSize: 13.5, background: "rgba(144,19,254,.18)", borderColor: C.purple, color: C.textHi }}>
        {test === "sending" ? "Enviando…" : <><BellRing size={15} color={C.purple} /> {tr("Enviar aviso de prueba (~1 min)")}</>}
      </button>
      {test === "sent" && <div style={{ color: C.good, fontSize: 12.5, marginTop: 8, lineHeight: 1.5, fontWeight: 700 }}>Enviado. CIERRA la app del todo y BLOQUEA el móvil: el aviso debe llegar en ~1 minuto. Si no llega, vuelve y pasa la revisión.</div>}
      {test === "err" && <div style={{ color: C.bad, fontSize: 12.5, marginTop: 8 }}>No pude programarlo. ¿Tienes la nube iniciada? Pasa la revisión de arriba.</div>}
    </div>
  );
}
...
function FeedbackPanel({ name }) {
  ...
  return (
    <div>
      ...
      <Field label={tr("Tu mensaje")}><textarea style={{ ...inputStyle, minHeight: 100, resize: "vertical" }} value={text} onChange={(e) => setText(e.target.value)} placeholder={tr("Lo que quieras contarme…")} /></Field>
    ...
    <div>
      ...
      <div style={{ opacity: notif.enabled ? 1 : 0.45, pointerEvents: notif.enabled ? "auto" : "none" }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 18 }}>
          {leadOptions.map((o) => <Chip key={o.v} active={leads.includes(o.v)} onClick={() => toggleLead(o.v)}>{tr(o.l)}</Chip>)}
        </div>

        <div style={{ ...card, padding: 12, background: C.well, fontSize: 12.5, color: C.textLo, lineHeight: 1.5 }}>
          {tr("El recordatorio insistente ya no es general: se activa en cada evento cuando lo creas (opción \"recordatorio insistente\"). Así solo insisten las cosas que tú marques.")}
        </div>
...
const settingRow = { ...glassSurface(18), padding: 12, display: "flex", alignItems: "center", gap: 12, cursor: "pointer" };
...
function BackupConfirm({ data, onCancel, onRestore }) {
  return (
    <div>
      ...
      <button onClick={onCancel} style={{ ...iconBtn, width: "100%", height: 44, marginTop: 10, gap: 8, fontWeight: 700, fontSize: 13.5, color: C.textLo }}>{tr("Cancelar")}</button>
    </div>
  );
}
  ...
  return (
    <div>
      <div style={{ display: "flex", gap: 8, marginBottom: 16, overflowX: "auto" }}>
        <Chip active={tab === "export"} onClick={() => { setTab("export"); setPending(null); }}>{tr("Guardar")}</Chip>
        <Chip active={tab === "restore"} onClick={() => { setTab("restore"); setPending(null); }}>{tr("Restaurar")}</Chip>
        <Chip active={tab === "auto"} onClick={() => { setTab("auto"); setPending(null); refreshAutos(); }}>{tr("Automáticas")}</Chip>
      </div>

      {tab === "export" && (
        <div>
          <p style={{ color: C.textLo, fontSize: 13.5, lineHeight: 1.55, marginTop: 0 }}>
            {tr("Guarda TODO lo de la app (agenda, dinero, colecciones, empresas, equipo, objetivos…). Ahora mismo hay")} <b style={{ color: C.textHi }}>{totalPieces}</b> {tr("cosas guardadas.")}
          </p>
        ...
        <div>
          {!pending ? (
            <>
              ...
              <Field label={tr("…o pega el código")}>
...
function CloudPanel({ cloud }) {
  ...
  return (
    <div>
      <p style={{ color: C.textLo, fontSize: 13.5, lineHeight: 1.55, marginTop: 0 }}>
        {tr("Crea una cuenta (o entra) para")} <strong style={{ color: C.textHi }}>{tr("guardar tus datos en la nube")}</strong> {tr("y sincronizarlos entre el móvil y el ordenador. Gratis.")}
      </p>
      <Field label={tr("Email")}><input style={inputStyle} value={email} onChange={(e) => setEmail(e.target.value)} placeholder={tr("tucorreo@email.com")} type="email" autoFocus /></Field>
      <Field label={tr("Contraseña")}><input style={inputStyle} value={pw} onChange={(e) => setPw(e.target.value)} placeholder={tr("mínimo 6 caracteres")} type="password" /></Field>
      {cloud.msg && (
...
function AiKeyForm({ value, onSave }) {
  ...
  return (
    <div>
      ...
      <Field label={tr("Clave de Anthropic (sk-ant-api03-…)")}>
...
function ScreenTimeScreen({ state, dispatch, back }) {
  ...
  return (
    <div style={{ padding: "20px 18px 0" }}>
      <button onClick={back} style={{ ...linkBtn, marginBottom: 10 }}><ChevronLeft size={16} /> {tr("Inicio")}</button>
      <Title kicker={tr("Bienestar digital")}>Pantalla</Title>
      ...
      {apps.length === 0 ? (
        <EmptyState face="neutral" title={tr("Controla tu pantalla")} sub={tr("Anota cuánto usas cada app (lo ves en Ajustes › Tiempo de uso) y ponte límites. Brosin te avisa si te pasas.")} cta={tr("Añadir app")} onCta={() => setSheet(true)} />
      ) : (
        <>
          <HBars icon={BarChart3} title={tr("Reparto por app")} sub={fmtMin(total)} accent={C.yellow}
      ...
      <div style={{ height: 122 }} />
      <FAB onClick={() => setSheet(true)} label={tr("App")} />

      <Sheet open={sheet} onClose={() => setSheet(false)} title={tr("Añadir app")}>
...
function AppRow({ a, onSet, onDel }) {
  ...
  return (
    <div style={{ ...card, padding: 14 }}>
      ...
      {editing && (
        <div style={{ display: "flex", gap: 8, marginTop: 10 }}>
          <input type="number" inputMode="numeric" autoFocus style={{ ...inputStyle, padding: "9px 12px" }} value={v} onChange={(e) => setV(e.target.value)} placeholder={tr("Minutos de hoy")} />
...
function AppForm({ onSave }) {
  ...
  return (
    <div>
      <Field label={tr("App")}><input style={inputStyle} value={name} onChange={(e) => setName(e.target.value)} placeholder={tr("Nombre de la app")} autoFocus /></Field>
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12 }}>
        {quick.map((q) => <Chip key={q} active={name === q} onClick={() => setName(q)}>{q}</Chip>)}
      </div>
      <div style={{ display: "flex", gap: 10 }}>
        <div style={{ flex: 1 }}><Field label={tr("Minutos hoy")}><input type="number" inputMode="numeric" style={inputStyle} value={minutes} onChange={(e) => setMinutes(e.target.value)} placeholder={tr("Ej. 45")} /></Field></div>
        <div style={{ flex: 1 }}><Field label={tr("Límite (min/día)")}><input type="number" inputMode="numeric" style={inputStyle} value={limitMin} onChange={(e) => setLimitMin(e.target.value)} placeholder={tr("Opcional")} /></Field></div>
      </div>
      <PrimaryBtn full onClick={() => name.trim() && onSave({ name: name.trim(), minutes: Number(minutes || 0), limitMin: Number(limitMin || 0) })}><Check size={18} strokeWidth={3} /> {tr("Guardar")}</PrimaryBtn>
    </div>
  );
}
...
function DrivingScreen({ state, dispatch, back, toast }) {
  ...
  return (
    <div style={{ padding: "20px 18px 0" }}>
      <button onClick={back} style={{ ...linkBtn, marginBottom: 10 }}><ChevronLeft size={16} /> {tr("Inicio")}</button>
      <Title kicker={tr("Al volante, sin distracciones")}>Conducción</Title>
      ...
      <div style={{ ...card, padding: 16, marginBottom: 14 }}>
        ...
        {editMsg ? (
          <div>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
              <button onClick={() => { dispatch({ type: "drive", payload: { defaultMsg: draft } }); setEditMsg(false); }} style={miniBtn(C.yellow, C.ink)}>{tr("Guardar")}</button>
              <button onClick={() => setEditMsg(false)} style={miniBtn(C.ink3, C.textLo)}>{tr("Cancelar")}</button>
            </div>
          </div>
        ) : (
      ...
      <Sheet open={sheet} onClose={() => setSheet(false)} title={tr("Mensaje personalizado")}>
...
function DriveContactForm({ onSave }) {
  ...
  return (
    <div>
      <Field label={tr("¿Para quién?")}><input style={inputStyle} value={name} onChange={(e) => setName(e.target.value)} placeholder={tr("Nombre")} autoFocus /></Field>
      <Field label={tr("Su mensaje")}><textarea style={{ ...inputStyle, minHeight: 90, resize: "vertical" }} value={msg} onChange={(e) => setMsg(e.target.value)} placeholder={tr("Ej. Mi amor, conduciendo. En cuanto pare te llamo ❤️")} /></Field>
      <PrimaryBtn full color={C.purple} onClick={() => name.trim() && msg.trim() && onSave({ name: name.trim(), msg: msg.trim() })}><Check size={18} strokeWidth={3} color="#fff" /> <span style={{ color: "#fff" }}>{tr("Guardar")}</span></PrimaryBtn>
    </div>
  );
}
...
function TeamScreen({ state, dispatch, back, me }) {
  ...
  return (
    <div style={{ padding: "20px 18px 0" }}>
      <button onClick={back} style={{ ...linkBtn, marginBottom: 10 }}><ChevronLeft size={16} /> {tr("Inicio")}</button>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <Title kicker={tr("Juntos llega más lejos")}>Equipo</Title>
        <button onClick={() => setSheet("import")} style={{ ...iconBtn, width: 46, height: 46 }}><Download size={20} /></button>
      </div>

      {projects.length === 0 ? (
        <EmptyState face="happy" title={tr("Sin proyectos aún")} sub={tr("Crea un proyecto compartido con horarios, tareas y tablón — o importa uno que te hayan pasado.")} cta={tr("Crear proyecto")} onCta={() => setSheet("new")} />
      ) : (
        <div style={{ display: "grid", gap: 12 }}>
          {projects.map((p) => {
      ...
      <div style={{ height: 122 }} />
      {projects.length > 0 && <FAB onClick={() => setSheet("new")} label={tr("Proyecto")} />}

      <Sheet open={sheet === "new"} onClose={() => setSheet(null)} title={tr("Nuevo proyecto")}>
      ...
      <Sheet open={sheet === "import"} onClose={() => setSheet(null)} title={tr("Importar proyecto")}>
...
function ProjectForm({ onSave, me }) {
  ...
  return (
    <div>
      <Field label={tr("Nombre del proyecto")}><input style={inputStyle} value={name} onChange={(e) => setName(e.target.value)} placeholder={tr("Ej. Reforma local, Viaje, Negocio…")} autoFocus /></Field>
      <PrimaryBtn full color={C.purple} onClick={() => name.trim() && onSave({ name: name.trim(), members: [{ id: uid(), name: me || "Yo" }] })}>
        <Check size={18} strokeWidth={3} color="#fff" /> <span style={{ color: "#fff" }}>{tr("Crear")}</span>
      </PrimaryBtn>
    </div>
  );
}
function ImportForm({ onImport }) {
  ...
  return (
    <div>
      <Field label={tr("Pega el código del proyecto")}><textarea style={{ ...inputStyle, minHeight: 100, resize: "vertical", fontFamily: "monospace", fontSize: 12 }} value={code} onChange={(e) => { setCode(e.target.value); setErr(false); }} placeholder={tr("BRSN1:…")} autoFocus /></Field>
...
function ProjectDetail({ project, back, patch, onDelete, me }) {
  ...
  return (
    <div style={{ padding: "20px 18px 0" }}>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
        <Chip active={tab === "tareas"} onClick={() => setTab("tareas")} accent={C.purple}>{tr("Tareas")}</Chip>
        <Chip active={tab === "agenda"} onClick={() => setTab("agenda")} accent={C.purple}>{tr("Agenda")}</Chip>
        <Chip active={tab === "tablon"} onClick={() => setTab("tablon")} accent={C.purple}>{tr("Tablón")}</Chip>
      </div>

      {tab === "tareas" && (
        (project.tasks || []).length === 0 ? (
          <EmptyState face="neutral" title={tr("Sin tareas")} sub={tr("Reparte el trabajo: cada tarea con su responsable y su fecha.")} cta={tr("Añadir tarea")} onCta={() => setSheet("task")} />
        ) : (
          <div style={{ display: "grid", gap: 10 }}>
      ...
      {tab === "agenda" && (
        (project.events || []).length === 0 ? (
          <EmptyState face="sleep" title={tr("Agenda compartida vacía")} sub={tr("Pon las citas y horarios del equipo en un solo sitio.")} cta={tr("Añadir cita")} onCta={() => setSheet("event")} />
        ) : (
          <div style={{ display: "grid", gap: 10 }}>
      ...
      {tab === "tablon" && (
        (project.notes || []).length === 0 ? (
          <EmptyState face="happy" title={tr("Tablón en blanco")} sub={tr("Ideas, enlaces, recados — todo lo que el equipo deba ver.")} cta={tr("Escribir")} onCta={() => setSheet("note")} />
        ) : (
          <div style={{ display: "grid", gap: 10 }}>
      ...
      <Sheet open={sheet === "member"} onClose={() => setSheet(null)} title={tr("Añadir miembro")}>
        <SimpleNameForm placeholder={tr("Nombre del miembro")} onSave={(name) => { patch({ members: [...members, { id: uid(), name }] }); setSheet(null); }} />
      </Sheet>
      <Sheet open={sheet === "task"} onClose={() => setSheet(null)} title={tr("Nueva tarea")}>
        <TeamTaskForm members={members} onSave={(d) => { patch({ tasks: [{ id: uid(), done: false, ...d }, ...(project.tasks || [])] }); setSheet(null); }} />
      </Sheet>
      <Sheet open={sheet === "event"} onClose={() => setSheet(null)} title={tr("Nueva cita")}>
        <TeamEventForm members={members} onSave={(d) => { patch({ events: [{ id: uid(), ...d }, ...(project.events || [])] }); setSheet(null); }} />
      </Sheet>
      <Sheet open={sheet === "note"} onClose={() => setSheet(null)} title={tr("Nueva nota del tablón")}>
        <TeamNoteForm members={members} me={me} onSave={(d) => { patch({ notes: [{ id: uid(), ...d }, ...(project.notes || [])] }); setSheet(null); }} />
      </Sheet>
      <Sheet open={sheet === "share"} onClose={() => setSheet(null)} title={tr("Compartir proyecto")}>
...
function TeamTaskForm({ members, onSave }) {
  ...
  return (
    <div>
      <Field label={tr("Tarea")}><input style={inputStyle} value={text} onChange={(e) => setText(e.target.value)} placeholder={tr("¿Qué hay que hacer?")} autoFocus /></Field>
      <Field label={tr("Responsable")}><MemberPicker members={members} value={assignee} onChange={setAssignee} /></Field>
      <Field label={tr("Fecha límite (opcional)")}><input type="date" style={inputStyle} value={due} onChange={(e) => setDue(e.target.value)} /></Field>
      <PrimaryBtn full color={C.purple} onClick={() => text.trim() && onSave({ text: text.trim(), assignee, due })}><Check size={18} strokeWidth={3} color="#fff" /> <span style={{ color: "#fff" }}>{tr("Añadir")}</span></PrimaryBtn>
    </div>
  );
}
function TeamEventForm({ members, onSave }) {
  ...
  return (
    <div>
      <Field label={tr("Cita")}><input style={inputStyle} value={title} onChange={(e) => setTitle(e.target.value)} placeholder={tr("Título")} autoFocus /></Field>
      <div style={{ display: "flex", gap: 10 }}>
        <div style={{ flex: 1 }}><Field label={tr("Fecha")}><input type="date" style={inputStyle} value={date} onChange={(e) => setDate(e.target.value)} /></Field></div>
        <div style={{ flex: 1 }}><Field label={tr("Hora")}><input type="time" style={inputStyle} value={time} onChange={(e) => setTime(e.target.value)} /></Field></div>
      </div>
      <Field label={tr("Quién")}><MemberPicker members={members} value={assignee} onChange={setAssignee} /></Field>
      <PrimaryBtn full color={C.purple} onClick={() => title.trim() && onSave({ title: title.trim(), date, time, assignee })}><Check size={18} strokeWidth={3} color="#fff" /> <span style={{ color: "#fff" }}>{tr("Añadir")}</span></PrimaryBtn>
    </div>
  );
}
function TeamNoteForm({ members, me, onSave }) {
  ...
  return (
    <div>
      <Field label={tr("Nota")}><textarea style={{ ...inputStyle, minHeight: 90, resize: "vertical" }} value={text} onChange={(e) => setText(e.target.value)} placeholder={tr("Escribe para el equipo…")} autoFocus /></Field>
      <Field label={tr("De parte de")}><MemberPicker members={members} value={author} onChange={setAuthor} /></Field>
...
function TabBar({ active, onChange, ai }) {
  ...
  const tabs = [
    ...
    { key: "agenda", label: tr("Agenda"), icon: CalendarDays },
    { key: "dinero", label: tr("Dinero"), icon: Wallet },
    { key: "juego", label: tr("Juego"), icon: Flag },
...
export default function BrosinOS() {
  ...
  useEffect(() => {
    if (!state.ready) return;
    let alive = true;
    const want = resolveLang(state.profile && state.profile.lang);
    if (want === CUR_LANG && want !== "es") return;
    loadLang(want).then((changed) => { if (alive && (changed || CUR_LANG !== "es")) setLangTick((x) => x + 1); });
    return () => { alive = false; };
  }, [state.ready, state.profile && state.profile.lang]);
  const [cloudBusy, setCloudBusy] = useState(false);
  const [cloudMsg, setCloudMsg] = useState(null);
  const stateRef = useRef(state);
  const cloudSkip = useRef(false);
  const syncedUserRef = useRef(null);
  const localSavedAt = useRef(null);
  const cloudUserRef = useRef(null);
  useEffect(() => { stateRef.current = state; });
  useEffect(() => { cloudUserRef.current = cloudUser; }, [cloudUser]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const nubeTieneAlgo = !!(cloudData && (cloudData.profile || cloudData.events));
      if (nubeTieneAlgo) {
        // FUSIÓN, no elegir una copia entera: se queda la versión más nueva de cada
        // cosa y los borrados mandan sobre versiones anteriores. Así lo apuntado en
        // este móvil y lo apuntado en otro conviven en vez de pisarse.
        const { ready, ...persistLocal } = stateRef.current;
        const fusion = fusionarEstados({ ...persistLocal, savedAt: localSavedAt.current || null }, cloudData);
        cloudSkip.current = true;
        const { savedAt, ...rest } = fusion;
        dispatch({ type: "hydrate", payload: { ...EMPTY, ...rest, ready: true } });
        const sellada = stamp(rest);
        localSavedAt.current = sellada.savedAt;
        const subido = await cloudSave(sb, cloudUser.id, sellada);
        setCloudMsg(subido ? { ok: true, t: "Sincronizado ✓" } : { ok: false, t: "Fusionado en el móvil, pero no pude subirlo." });
      } else {
        const { ready, ...persist } = stateRef.current;
        const ok = await cloudSave(sb, cloudUser.id, stamp(persist));
        if (ok) setCloudMsg({ ok: true, t: nubeTieneAlgo ? "Este móvil tenía lo más nuevo: subido ✓" : "Sincronizado ✓" });
        else setCloudMsg({ ok: false, t: "No se pudo guardar en la nube. Lo tuyo sigue a salvo en el móvil." });
      }
    })();
  ...
  useEffect(() => {
    ...
    const id = setTimeout(async () => {
      ...
      const ok = await cloudSave(sb, cloudUser.id, stamp(persist));
      if (!ok) setCloudMsg({ ok: false, t: "No se pudo guardar en la nube. Lo tuyo sigue a salvo en el móvil." });
    }, 900);
    return () => clearTimeout(id);
  }, [state, cloudUser]);

  /* Guardar de golpe al cerrar la app o mandarla a segundo plano.
     Antes se esperaba 400 ms (local) y 900 ms (nube): si cerrabas o iOS suspendía
     la app antes, lo último apuntado no llegaba a ningún lado. */
  useEffect(() => {
    const flush = () => {
      const s = stateRef.current;
      if (!s || !s.ready) return;
      const { ready, ...persist } = s;
      const sellado = stamp(persist);
      localSavedAt.current = sellado.savedAt;
      try { store.set(STATE_KEY, sellado); } catch (e) {}
      const u = cloudUserRef.current;
      if (u) { try { getSupa().then((sb) => { if (sb) cloudSave(sb, u.id, sellado); }).catch(() => {}); } catch (e) {} }
    };
  ...
  useEffect(() => {
    ...
    return () => clearTimeout(id);
  }, [state.events, state.people, state.goals, state.habits, state.profile.briefOn, state.profile.briefTime, cloudUser, state.ready]);
  ...
  useEffect(() => {
    ...
    const check = () => {
      ...
      state.events.forEach((e) => {
        ...
        if (now >= start && now - start < 12 * 3600 * 1000) {
          ...
          if (now - last >= e.nag * 60000) {
            ...
            pushToast(tri("Sigue pendiente: {t}", { t: e.title }), tri("Márcalo como hecho cuando lo tengas. Te aviso cada {n}.", { n: e.nag % 60 === 0 ? e.nag / 60 + " h" : e.nag + " min" }));
          }
        }
      });
      ...
      (state.goals || []).forEach((g) => {
        ...
        pushToast(dd === 0 ? tr("Hoy es la meta") + ": " + g.title : tr("Se acerca") + ": " + g.title, `${dd === 0 ? tr("Es hoy") : (dd === 1 ? tr("Falta 1 día") : tri("Faltan {n} días", { n: dd }))}${left ? " · " + tri("{n} paso(s) pendientes", { n: left }) : ""}. ${g.why ? tr("Recuerda") + ": " + g.why : tr("A por ello.")}`);
      });
      ...
      if (lastSleep && lastSleep.hours < smin && sleepNagRef.current !== todayISO()) {
        ...
        pushToast(tr("A descansar, bro"), tri("Vienes de {h} h de sueño. Necesitas descansar más — hazte un favor hoy.", { h: lastSleep.hours }));
      }
    };
    check();
  ...
  useEffect(() => {
    (async () => {
      ...
      if (saved && typeof saved === "object") {
        localSavedAt.current = saved.savedAt || null;
  ...
  useEffect(() => {
    ...
    const id = setTimeout(async () => {
      ...
      const sellado = stamp(persist);
      localSavedAt.current = sellado.savedAt;
      const okLocal = await store.set(STATE_KEY, sellado);
      if (!okLocal) pushToast("No he podido guardar", "La memoria del móvil está llena. Borra fotos de la app o libera espacio.", false);
  ...
  /* racha de la colmena: congelador automático, premios y mejor marca */
  useEffect(() => {
    if (!state.ready) return;
    const p = state.profile || {};
    const now = new Date();
    const yesterday = new Date(now); yesterday.setDate(now.getDate() - 1);
    const yIso = dayISOOf(yesterday);
    const before = new Date(now); before.setDate(now.getDate() - 2);
    const patch = {};
    // 1) consumir congelador: ayer fallado, anteayer (o cadena) activo, y hay congeladores
    const saved = p.streakSaved || [];
    if (!wasActiveOn(state, yIso) && !saved.includes(yIso) && (p.streakFreezes || 0) > 0 && (wasActiveOn(state, dayISOOf(before)) || saved.includes(dayISOOf(before)))) {
      patch.streakFreezes = (p.streakFreezes || 0) - 1;
      patch.streakSaved = [...saved, yIso].slice(-30);
    }
    // 2) calcular racha con el posible perdón aplicado
    const tmpState = patch.streakSaved ? { ...state, profile: { ...p, streakSaved: patch.streakSaved } } : state;
    const st = hiveStreak(tmpState, now);
    // 3) ganar 1 congelador cada 7 días de racha (máx. 2 guardados)
    const milestone = Math.floor(st.count / 7);
    if (milestone > (p.streakMilestone || 0) && (patch.streakFreezes != null ? patch.streakFreezes : (p.streakFreezes || 0)) < 2) {
      patch.streakFreezes = Math.min(2, (patch.streakFreezes != null ? patch.streakFreezes : (p.streakFreezes || 0)) + 1);
      patch.streakMilestone = milestone;
    } else if (milestone > (p.streakMilestone || 0)) {
      patch.streakMilestone = milestone;
    }
    // 4) mejor marca
  ...
  }, [state.ready, state.events, state.habits]);
  ...
  const _thK = state.profile && state.profile.theme;
  ...
  const styleTag = (
    <style>{CSS_TACTO + `
      ...
      * { -webkit-tap-highlight-color: transparent; box-sizing: border-box; }
      ...
      /* ================= LIQUID GLASS =================
         El lienzo con color va en el marco de la app (ambientBg), no aquí:
         el shell exterior es negro a propósito para que en escritorio la app
         siga pareciendo un móvil. */

      /* Sin backdrop-filter (WebViews viejas): sube el relleno o el texto se pierde. */
      @supports not ((backdrop-filter: blur(1px)) or (-webkit-backdrop-filter: blur(1px))) {
        [style*="backdrop-filter"], [style*="backdropFilter"] {
          background: ${C.base === "light" ? "rgba(255,255,255,.94)" : "rgba(16,16,18,.94)"} !important;
        }
      }
      @media (prefers-reduced-transparency: reduce) {
        body { background: ${C.bg0} !important; }
      }
      /* Regla del kit: no transformar lo que hay ENCIMA de un cristal — la capa
         de debajo se queda sin fondo que muestrear y parpadea en negro. Por eso
         la entrada escalonada pasa a ser solo opacidad. */
      @keyframes blistin {from{opacity:0}to{opacity:1}}
      ::-webkit-scrollbar { width: 0; height: 0; }
      input::placeholder, textarea::placeholder { color: ${C.base === "light" ? "#a39a83" : "#5a5a5a"}; }
      input[type="date"], input[type="time"] { color-scheme: ${C.base}; }
      ...
      .b3d-dark {
        ...
        box-shadow: 0 1px 0 rgba(255,255,255,${C.base === "light" ? ".9" : ".08"}) inset, 0 -2px 0 ${C.keycapIn} inset,
      ...
      .bkeycap {
        box-shadow: 0 1px 0 rgba(255,255,255,${C.base === "light" ? ".85" : ".09"}) inset, 0 -3px 0 ${C.keycapIn} inset,
      ...
      .bkeycap:active {
        ...
        box-shadow: 0 1px 0 rgba(255,255,255,.04) inset, 0 -1px 0 ${C.keycapIn} inset,
          0 1px 3px ${C.dropBtn};
        filter: brightness(${C.base === "light" ? ".97" : ".92"});
      }
      /* --- FAB: pop al montar --- */
      ...
      .broadfx { position: absolute; left: 7px; right: 7px; bottom: 10px; height: 2px; border-radius: 2px;
        background: repeating-linear-gradient(90deg, ${C.base === "light" ? "rgba(60,50,20,.4)" : "rgba(255,255,255,.55)"} 0 6px, transparent 6px 13px);
  ...
  return (
    <div style={{ minHeight: "100vh", background: C.outer, display: "flex", justifyContent: "center", fontFamily: "'Inter','SF Pro Display',system-ui,-apple-system,'Segoe UI',Roboto,sans-serif" }}>
      ...
      <div key={C.mode + "·" + CUR_LANG + "·" + langTick} style={{ position: "relative", width: "100%", maxWidth: 440, minHeight: "100vh", background: ambientBg(), overflow: "hidden", boxShadow: C.base === "light" ? "0 0 60px rgba(120,100,40,.25)" : "0 0 60px rgba(0,0,0,.6)" }}>
        ...
        <div style={{ position: "relative", height: "100vh", overflowY: "auto", paddingTop: "env(safe-area-inset-top)", paddingBottom: "calc(72px + env(safe-area-inset-bottom))" }}>
          {!booted ? (
            ...
            <div key={tab} style={{ animation: "bpagein .3s cubic-bezier(.2,.9,.3,1)" }}>
              ...
              {tab === "juego" && <GameScreen state={state} dispatch={dispatch} toast={pushToast} ai={ai} />}
